...
uses
  ...
  DirectShow9, ActiveX, UPlayer, System.json, uwebserv;
...
uses umain, ucommon, ugrid, uimportfiles, ugrtimelines, utimeline, udrawtimelines,
     uairdraw, umyfiles, UIMGButtons, umymessage, umyundo, uplaylists, umytexttable;
...
procedure SetButtonsPredNext;
begin
  if Form1.MySynhro.Checked then exit;
...
begin
  ...
  for i:=1 to grid.RowCount-1 do begin
    tmstr:=trim((grid.Objects[0,i] as tgridrows).MyCells[3].ReadPhrase('StartTime'));
    if tmstr<>'' then begin
...
//  MyStartPlay  := -1;    // ¬рем€ старта клипа, при chnone не используетс€, -1 врем€ не установлено.
  ...
  form1.lbTypeTC.Caption :='';// Trim((Form1.GridActPlayList.Objects[0,GridPlayerRow] as TGridRows).MyCells[3].ReadPhrase('StartTime'));
...
//  MediaSetPosition(TLParameters.Position, false);
  ...
  WriteLog('MAIN', 'UActPlayList.LoadDefaultClipToPlayer ѕустой клип загружен.');
  SetMediaButtons;
except
...
//
  ...
  if GridPlayer=grPlayList then begin
    if Not (Form1.GridActPlayList.Objects[0,GridPlayerRow] is TGridRows) then exit;
    Form1.lbusesclpidlst.Caption:='ѕлей-лист: ' + Form1.ListBox1.Items.Strings[Form1.ListBox1.ItemIndex];
    ...
    Form1.Label2.Caption:=(Form1.GridActPlayList.Objects[0,GridPlayerRow] as TGridRows).MyCells[3].ReadPhrase('Clip');
    //Form1.lbClipName.Caption:=(Form1.GridActPlayList.Objects[0,GridPlayerRow] as TGridRows).MyCells[3].ReadPhrase('Clip');
    pntlprep.SetText('ClipName',(Form1.GridActPlayList.Objects[0,GridPlayerRow] as TGridRows).MyCells[3].ReadPhrase('Clip'));
    //Form1.lbSongName.Caption:=(Form1.GridActPlayList.Objects[0,GridPlayerRow] as TGridRows).MyCells[3].ReadPhrase('Song');
    pntlprep.SetText('SongName',(Form1.GridActPlayList.Objects[0,GridPlayerRow] as TGridRows).MyCells[3].ReadPhrase('Song'));
    //Form1.lbSongSinger.Caption:=(Form1.GridActPlayList.Objects[0,GridPlayerRow] as TGridRows).MyCells[3].ReadPhrase('Singer');
    pntlprep.SetText('SingerName',(Form1.GridActPlayList.Objects[0,GridPlayerRow] as TGridRows).MyCells[3].ReadPhrase('Singer'));
    Form1.lbPlayerFile.Caption:=(Form1.GridActPlayList.Objects[0,GridPlayerRow] as TGridRows).MyCells[3].ReadPhrase('File');
    form1.lbTypeTC.Caption := Trim((Form1.GridActPlayList.Objects[0,GridPlayerRow] as TGridRows).MyCells[3].ReadPhrase('StartTime'));
    ...
    pntlprep.SetText('ClipName',(Form1.GridClips.Objects[0,GridPlayerRow] as TGridRows).MyCells[3].ReadPhrase('Clip'));
    //Form1.lbSongName.Caption:=(Form1.GridClips.Objects[0,GridPlayerRow] as TGridRows).MyCells[3].ReadPhrase('Song');
    pntlprep.SetText('SongName',(Form1.GridClips.Objects[0,GridPlayerRow] as TGridRows).MyCells[3].ReadPhrase('Song'));
    //Form1.lbSongSinger.Caption:=(Form1.GridClips.Objects[0,GridPlayerRow] as TGridRows).MyCells[3].ReadPhrase('Singer');
    pntlprep.SetText('SingerName',(Form1.GridClips.Objects[0,GridPlayerRow] as TGridRows).MyCells[3].ReadPhrase('Singer'));
    Form1.lbPlayerFile.Caption:=(Form1.GridClips.Objects[0,GridPlayerRow] as TGridRows).MyCells[3].ReadPhrase('File');
    form1.lbTypeTC.Caption := Trim((Form1.GridClips.Objects[0,GridPlayerRow] as TGridRows).MyCells[3].ReadPhrase('StartTime'));
...
//  MyRemainTime := -1;  //врем€ оставшеес€ до запуска
  ...
  //MediaPause;
  AddToJsonStore('TLEDITOR',TLZone.TLEditor.SaveToJSONObject);
  SetMediaButtons;
except
  on E: Exception do begin
...
begin
  With Form1 do begin
        ...
        if (GridClips.Objects[0,i] as TGridRows).ID<>0 then begin
          if (GridClips.Objects[0,i] as TGridRows).MyCells[1].Mark then begin
            ...
            if not ClipExists(GridActPlayList,(GridClips.Objects[0,i] as TGridRows).MyCells[3].ReadPhrase('ClipID')) then begin
              ...
              (GridActPlayList.Objects[0,rw] as TGridRows).MyCells[3].UpdatePhrase('StartTime','');
    ...
    if cnt=0
      then MyTextMessage('—ообщение','¬ плей-лист не загружен ни один клип.',1)
      else MyTextMessage('—ообщение','¬ыделено клипов ' + inttostr(ttl) + ' загруженно в плей-лист ' + inttostr(cnt) + '.',1);
  end; //width
end;


procedure LoadClipsToActPlayList(PLName : string);
var i, cnt, rw, ttl, ps, plid : integer;
    lst : tstrings;
    clid, sttm : string;
begin
  With Form1 do begin
    ...
    try
      ...
      for i:=0 to lst.Count-1 do begin
        ...
        if ps > 0 then begin
          ...
          (GridActPlayList.Objects[0,rw] as TGridRows).MyCells[0].Used:=false;
          (GridActPlayList.Objects[0,rw] as TGridRows).MyCells[1].Mark:=false;
          (GridActPlayList.Objects[0,rw] as TGridRows).MyCells[3].UpdatePhrase('StartTime',sttm);
          sttm:=(GridActPlayList.Objects[0,rw] as TGridRows).MyCells[3].ReadPhrase('StartTime');
          rw:=rw+1;
        end;
      end;
    finally
    ...
    try
      for i:=1 to GridActPlayList.RowCount-1 do begin
        if (GridActPlayList.Objects[0,i] is TGridRows) then begin
          clipid := (GridActPlayList.Objects[0,i] as TGridRows).MyCells[3].ReadPhrase('ClipID');
          starttime := (GridActPlayList.Objects[0,i] as TGridRows).MyCells[3].ReadPhrase('StartTime');
  ...
  with form1 do begin
    for i:=GridActPlayList.RowCount-1 downto 1 do begin
      if GridActPlayList.Objects[0,i] is TGridRows then begin
        ...
        starttime :=  trim((GridActPlayList.Objects[0,i] as TGridRows).MyCells[3].ReadPhrase('StartTime'));
        ps := uplaylists.findclipposition(GridClips,txt);
        if ps>0 then begin
          (GridActPlayList.Objects[0,i] as TGridRows).Assign((GridClips.Objects[0,ps] as TGridRows));
          (GridActPlayList.Objects[0,i] as TGridRows).MyCells[3].UpdatePhrase('StartTime',starttime);
          (GridClips.Objects[0,ps] as TGridRows).MyCells[3].UpdatePhrase('StartTime',starttime);
        end else MyGridDeleteRow(GridActPlayList, i, RowGridClips);
      end;
    end;
//    for i:=1 to GridActPlayList.RowCount-1 do begin
//      if GridActPlayList.Objects[0,i] is TGridRows then begin
