...
begin
  ...
  For i:=0 to 31 do DevEvents[i].ReadFromStream(F);
  ...
  With Form1.GridTimeLines do begin
    for i:=1 to RowCount-1
      do if Objects[0,i] is TTimelineOptions
           then if (Objects[0,i] as TTimelineOptions).TypeTL=TypeTL
...
begin
  result := 0;
        case TypeTL of
  tldevice : result := CountTypeTLTimeline(tldevice);
  tltext   : result := 20 + CountTypeTLTimeline(tltext);
  tlmedia  : result := 40 + CountTypeTLTimeline(tltext);
        end;
end;
  ...
  If FEditTimeline.ModalResult=mrOk then begin
    ...
    if ARow=-1 then begin
      ...
      (Form1.GridTimeLines.Objects[0,cellpos] as TTimelineOptions).IDTimeline
          :=AddIDTimeline((Form1.GridTimeLines.Objects[0,cellpos] as TTimelineOptions).TypeTL);
...
//                       - imgButtonsControlProj.Height - GridTimeLines.Height - 40;
    ...
    //IDTL:=IDTL + 1;
    (GridTimeLines.Objects[0,1] as TTimelineOptions).IDTimeline:=SetIDTimeline(1,tldevice);
    ...
    //IDTL:=IDTL + 1;
    (GridTimeLines.Objects[0,2] as TTimelineOptions).IDTimeline:=SetIDTimeline(2,tltext);
    ...
    //IDTL:=IDTL + 1;
    (GridTimeLines.Objects[0,3] as TTimelineOptions).IDTimeline:=SetIDTimeline(3,tlmedia);
...
begin
 ...
 //               + #13#10 +'Настройка оборудования не доступна',1);
  if OPTTimeline.TypeTL=tlDevice
    then SetProtocol(OPTTimeline)
    else MyTextMessage('Сообщение','Отсутсвуют протоколы поддерживающие данный тип тайм-линий.',1);
end;
