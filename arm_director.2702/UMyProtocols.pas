  ...
  with TypeDevices[index] do begin
    ...
    with Vendors[index] do begin
      ...
      with FirmDevices[index] do begin
        ...
        if index=-1 then index:=0;
        with ListProtocols[index] do begin
...
begin
  ...
  try
    ...
    GetListString(buff);
    buff.Add('</' + TypeProtocols + '>');
    if ansilowercase(TypeProtocols)='tldevices' then buff.Text:=buff.Text + stxt + smedia
    else if ansilowercase(TypeProtocols)='tltext' then buff.Text:=sdev + buff.Text + smedia
    else if ansilowercase(TypeProtocols)='tlmedia' then buff.Text:=sdev + stxt + buff.Text;
...
begin
   ...
   try
     ...
     tmp.DrawText('Производитель:',Vendors[index].rt2,DT_VCENTER or DT_SINGLELINE);
     tmp.DrawText(Vendors[index].Vendor,Vendors[index].rtv,DT_VCENTER or DT_SINGLELINE);

     if Vendors[index].index=-1 then Vendors[index].index:=-0;
     ...
     with Vendors[index] do begin
       ...
       tmp.DrawText('Устройство:',FirmDevices[index].rt3,DT_VCENTER or DT_SINGLELINE);
       tmp.DrawText(FirmDevices[index].Device,FirmDevices[index].rtd,DT_VCENTER or DT_SINGLELINE);


       if FirmDevices[index].index=-1 then FirmDevices[index].index:=-0;

       with FirmDevices[index] do begin
         ListProtocols[index].rt4.Left:=rt1.Left;
         ListProtocols[index].rt4.Top:=rt3.Bottom;
         ListProtocols[index].rt4.Right:=rt1.Left+ps;
         ListProtocols[index].rt4.Bottom:=ListProtocols[index].rt4.Top+hghtrw;
         ...
         tmp.DrawText('Протокол:',ListProtocols[index].rt4,DT_VCENTER or DT_SINGLELINE);
         tmp.DrawText(ListProtocols[index].Protocol,ListProtocols[index].rtp,DT_VCENTER or DT_SINGLELINE);
       end;
     end;
...
begin
  ...
  if exist422 then port422.GetListString(lst);
  if existip then portip.GetListString(lst);
  lst.Add(setspace(10) + '</Ports>');
end;
...
begin
  ...
  if trim(scount)<>'' then begin
    ...
    for i:=0 to cnt-1 do begin
      ...
      Count:=Count+1;
      setlength(List,Count);
      List[Count-1] := TOneStringTable.create(res.Name, res.Text, res.VarText);
    end;
  end;
end;
...
begin
  result:=-1;
  for i:=0 to Count-1 do begin
    if (X>List[i].rttxt.Left) and (X<List[i].rttxt.Right) and (Y>List[i].rttxt.Top)
       and (Y<List[i].rttxt.Bottom) then
    begin
      result := i;
      exit;
    end;
...
begin
   ...
   try
     ...
     if crw=0 then crw:=1;
     ccl := 32 div crw;
     if (32 mod crw) > 0 then ccl := ccl + 1;
     ...
     for i:=0 to Count-1 do begin
       ...
       tmp.SetFontEx(FormsFontName,trunc(w1 / length(List[i].Name)),MTFontSize,1,false,false,false);
       tmp.DrawText(List[i].Name,List[i].rtnm,DT_VCENTER or DT_SINGLELINE);
       tmp.SetFont(FormsFontName, MTFontSize);
       tmp.DrawText(List[i].Text,List[i].rttxt,DT_VCENTER or DT_SINGLELINE);
       //top := List[i].rtnm.Bottom;
