...
begin
  ...
  End;
    ...
    on E: Exception do WriteLog('MAIN', 'Uplaylists.SetPlaylistBlocking ARow=' + inttostr(ARow) + ' | ' + E.Message);
  end;
end;
...
begin
  ...
  with Form1, Form1.GridLists do begin
    ...
    for i:=1 to GridLists.RowCount-1 do begin
      plnm := (GridLists.Objects[0,i] as TGridRows).MyCells[3].ReadPhrase('Name');
      ...
      WriteLog('MAIN', 'Uplaylists.PlaylistToPanel Name=' + plnm + '  ClipID=' + (ListBox1.Items.Objects[j] as TMyListBoxObject).ClipId);
    end;
    ListBox1.ItemIndex:=ARow-1;
    pntlapls.SetText('CommentText',(Objects[0,ARow] as TGridRows).MyCells[3].ReadPhrase('Comment'));
    pntlapls.Draw(imgpldata.Canvas);
    imgpldata.Repaint;
    //lbPLComment.Caption:=(Objects[0,ARow] as TGridRows).MyCells[3].ReadPhrase('Comment');
  end; //with
  except
    on E: Exception do WriteLog('MAIN', 'Uplaylists.PlaylistToPanel ARow=' + inttostr(ARow) + ' | ' + E.Message);
  end;
end;
...
begin
  ...
  with Form1.GridClips do begin
    ...
    for i:=1 to RowCount-1 do begin
       txt := (Form1.GridClips.Objects[0,i] as TGridRows).MyCells[3].ReadPhrase('Clip');
       ...
       WriteLog('MAIN', 'Uplaylists.ReadListClips Clip=' + txt + '  ClipID=' + (FPlayLists.ListBox1.Items.Objects[ps] as TMyListBoxObject).ClipId);
    end; //for
  end;
  except
    on E: Exception do WriteLog('MAIN', 'Uplaylists.ReadListClips | ' + E.Message);
  end;
end;
...
begin
  try
  WriteLog('MAIN', 'Start Uplaylists.findclipposition Grid='
            + Grid.Name + ' ClipID=' + ClipID);
  result:=-1;
  for i:=1 to Grid.RowCount-1 do begin
    if trim((Grid.Objects[0,i] as TGridRows).MyCells[3].ReadPhrase('ClipID'))=trim(ClipID) then begin
      result:=i;
      WriteLog('MAIN', 'Finish Uplaylists.findclipposition ClipID='
                + ClipID + ' Position=' + inttostr(i));
      exit;
    end;
  end;
  except
    on E: Exception do WriteLog('MAIN', 'Uplaylists.findclipposition Grid='
           + Grid.Name + '  ClipID=' + ClipID + ' | ' + E.Message);
  end;
end;
...
begin
  ...
  with FPlayLists do begin
    ...
    for i:=0 to ListBox2.Items.Count-1 do begin
      ...
      starttime := trim((ListBox2.Items.Objects[i] as TMyListBoxObject).StartTime);
      if clipid<>'' then lst.Add(clipid+'|'+starttime);
    end;
    ...
    on E: Exception do WriteLog('MAIN', 'Uplaylists.WriteClipsToPlayLists | ' + E.Message);
  end;
end;
...
begin
  ...
  with FPlayLists do begin
    ...
    for i:=0 to lst.Count-1 do begin
      ...
      if psi>0 then begin
        clp:=(Form1.GridClips.Objects[0,psi] as TGridRows).MyCells[3].ReadPhrase('Clip');
        clpid:=(Form1.GridClips.Objects[0,psi] as TGridRows).MyCells[3].ReadPhrase('ClipId');
      ...
      WriteLog('MAIN', 'Uplaylists.LoadClipsFromPlayLists Clip=' + clp + ' ClipID=' + clpid);
    end;
    ...
    WriteLog('MAIN', ' Finish Uplaylists.LoadClipsFromPlayLists Name=' + PLName);
  end;
  except
    on E: Exception do WriteLog('MAIN', 'Uplaylists.LoadClipsFromPlayLists Name=' + PLName + ' | ' + E.Message);
  end;
end;
...
begin
  ...
  if FPlayLists.ModalResult = mrOk then begin
    if ARow=-1 then begin
      ...
      for i:=0 to Form1.GridLists.RowCount-1 do (Form1.GridLists.Objects[0,i] as TGridRows).MyCells[2].Mark:=false;
    ...
    on E: Exception do WriteLog('MAIN', 'Uplaylists.EditPlayList ARow=' + inttostr(ARow) + ' | ' + E.Message);
  end;
end;
...
begin
  if Trim(edNamePL.Text)<>'' then begin
...
begin
  ...
  with FPlayLists do begin
    ...
    for i:=0 to ListBox2.Items.Count-1 do begin
      if ListBox2.Items.Objects[i] is TMyListBoxObject then begin
        if (ListBox2.Items.Objects[i] as TMyListBoxObject).ClipID=ClipID then begin
          WriteLog('MAIN', 'Uplaylists.ClipExists ClipID=' + ClipID + ' Position=' + inttostr(i));
          Result := true;
          exit;
        end;
      end;
    ...
    on E: Exception do WriteLog('MAIN', 'Uplaylists.ClipExists ClipID=' + ClipID + ' | ' + E.Message);
  end;
end;
...
begin
  ...
  for i:=ListBox2.Items.Count-1 downto 0 do if trim(ListBox2.Items.Strings[i])='' then ListBox2.Items.Delete(i);
      ...
      if ListBox1.Selected[i] then begin
        if ListBox1.Items.Objects[i] is TMyListBoxObject then begin
          if not ClipExists((ListBox1.Items.Objects[i] as TMyListBoxObject).ClipID) then begin
            ...
            WriteLog('MAIN', 'TFPlayLists.SpeedButton5Click Clip=' + clp + ' ClipID=' + (ListBox2.Items.Objects[ps] as TMyListBoxObject).ClipId);
          end;
        end;
      end;
    ...
    on E: Exception do WriteLog('MAIN', 'TFPlayLists.SpeedButton5Click | ' + E.Message);
  end;
end;
