...
//    procedure GridListsMouseUpGRTemplates(X, Y: Integer);
    ...
    procedure ImgMainMenuMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure ImgMainMenuMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure sbProjectMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure sbClipsMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure sbPlayListMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure sbMainMenuMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    ...
    procedure ImgListFilesMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure ImgListFilesMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure ImgStartWinBtnMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure ImgStartWinBtnMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure PanelStartWindowMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure imgClpFindStrMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    ...
    procedure imgClpFindTimeMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure imgClpFindTimeMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure imgAPlFindStrMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure imgAPlFindStrMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure imgAPlFindTimeMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure imgAPlFindTimeMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure imgTempMenuMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure imgTempMenuMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure GridClipsMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure GridActPlayListMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure Label2MouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure PanelControlClipMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure sbPredClipMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure sbNextClipMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure pnMovieMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure Image3MouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure lbClipNameMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure imgEventsMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure imgPLTlCurrMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure imgPLTlCurrMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure GridTimeLinesMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure imgpntlprojMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure Timer2Timer(Sender: TObject);
    procedure GridListsMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
...
uses UInitForms, UGrid, UProject, UIMGButtons, UDelGridRow, UTimeline,
     ...
     umyltc, usettc, UMyTextTemplate, umymenu, UStartWindow, ufrhotkeys, UMyOptions,
...
// Если запуск воспроизведения выполнен то отображаем движение тайм-линий.
   If MyTimer.Enable then begin
     ...
     if (not fileexists(Form1.lbPlayerFile.Caption)) or (vlcplayer.p_mi=nil) then begin
...
procedure TForm1.sbProjectMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if pnMainMenu.Visible then pnMainMenu.Visible:=False;
  if pnTempMenu.Visible then pnTempMenu.Visible:=false;
end;
...
procedure TForm1.sbClipsMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if pnMainMenu.Visible then pnMainMenu.Visible:=False;
  if pnTempMenu.Visible then pnTempMenu.Visible:=false;
...
procedure TForm1.sbPlayListMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if pnMainMenu.Visible then pnMainMenu.Visible:=False;
  if pnTempMenu.Visible then pnTempMenu.Visible:=false;
...
procedure TForm1.lbClipNameMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if pnTempMenu.Visible then pnTempMenu.Visible:=false;
end;
...
begin
  ...
  if MySynhro.Checked and (oldlist1index<>ListBox1.ItemIndex) then begin
    MyTextMessage('Предупреждение','Внимание режим синхронизации будет отключен.', 1);
  ...
  pntlapls.SetText('CommentText',(GridLists.Objects[0,listbox1.ItemIndex+1] as TGridRows).MyCells[3].ReadPhrase('Comment'));
  ...
  if FileExists(PathPlayLists + '\' + FLName) then LoadClipsToActPlayList(PathPlayLists + '\' + FLName);

  GridActvePLToPanel(GridActPlayList.Row);
  if PanelPlayList.Visible then ActiveControl:=GridActPlayList;

  if i<>-1
    then pntlclips.SetText('APlayList',ListBox1.Items.Strings[listbox1.ItemIndex])  //Form1.lbClipActPL.Caption := ListBox1.Items.Strings[listbox1.ItemIndex]
    else pntlclips.SetText('APlayList',''); //Form1.lbClipActPL.Caption := '';
...
//Сохраняем текущий плей-лист
  if ListBox1.ItemIndex<>-1
      then SavePlayListFromGridToFile((ListBox1.Items.Objects[ListBox1.ItemIndex] as TMyListBoxObject).ClipId);
//Закрываем панели проектов, клипов, и плей-листа
  if trim(lbActiveClipID.Caption)=''
     then LoadDefaultClipToPlayer;
...
procedure TForm1.GridTimeLinesMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if pnMainMenu.Visible then pnMainMenu.Visible:=false;
end;
...
procedure TForm1.pnMovieMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if pnTempMenu.Visible then pnTempMenu.Visible:=false;
end;
...
procedure TForm1.imgpntlprojMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if pnMainMenu.Visible then pnMainMenu.Visible:=false;
end;

procedure TForm1.ImgStartWinBtnMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
...
procedure TForm1.imgEventsMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if pnTempMenu.Visible then pnTempMenu.Visible := false;
  if pnmainmenu.Visible then pnmainmenu.Visible:=false;
...
//  CoUninitialize;// деинициализировать OLE
  ...
  if MyMainMenu<>nil then MyMainMenu.FreeInstance;
  if TempMenu<>nil then TempMenu.FreeInstance;
  if InputPopUpMenu<>nil then InputPopUpMenu.FreeInstance;
  if MenuListFiles<>nil then MenuListFiles.FreeInstance;
  UnmapViewOfFile(Compartido);
  CloseHandle(FicheroM);
  if ClpText<>nil then ClpText.FreeInstance;
  if ClpTime<>nil then ClpTime.FreeInstance;
  if APlText<>nil then APlText.FreeInstance;
  if APlTime<>nil then AplTime.FreeInstance;
end;
...
procedure TForm1.imgPLTlCurrMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
...
procedure TForm1.imgClpFindStrMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
...
begin
  ...
  0 : begin
        ...
        if s='клип' then SortGridAlphabet(GridClips, 1, 3, 'Clip', ClpText.direction)
        else if s='песня' then SortGridAlphabet(GridClips, 1, 3, 'Song', ClpText.direction)
        else if s='исполнитель' then SortGridAlphabet(GridClips, 1, 3, 'Singer', ClpText.direction);
      end;
  1 : begin
        TempMenu.clear;
        pnTempMenu.Top:=PanelControl.Height+pnClpFindStr.Top+pnClpFindStr.Height
                         -(pnAPlFindStr.Height-18) div 2;
...
procedure TForm1.imgClpFindTimeMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
...
begin
  ...
  0 : begin
        ...
        s:=ClpTime.Text;
        s:=ansilowercase(trim(s));
        if s='время старта' then SortGridStartTime(GridClips, not ClpTime.direction)
        else if s='хр-ж медиа' then SortGridTime(GridClips, 1, 3, 'Duration', ClpTime.direction)
        else if s='хр-ж воспр.' then SortGridTime(GridClips, 1, 3, 'Dur', ClpTime.direction);
      end;
  1 : begin
        TempMenu.clear;
        pnTempMenu.Top:=PanelControl.Height+pnClpFindTime.Top+pnClpFindTime.Height
                         -(pnAPlFindStr.Height-18) div 2;
        TempMenu.rowheight:=22;
        pnTempMenu.Height:=TempMenu.rowheight*3+2;
  ...
  0,1: (GridClips.Objects[0,GridClips.Row] as TGridRows).MyCells[i].Mark:=not (GridClips.Objects[0,GridClips.Row] as TGridRows).MyCells[i].Mark;
    2: begin
         ...
         if MySynhro.Checked then begin
           if MyTextMessage('Предупреждение','Установлен режим синхронизации по времени.' + #13#10 +
                            'В случае продолжения режим синхронизации будет отменен.' + #13#10 +
                            #13#10 + 'Продолжить?',2)
           then begin
...
begin
  ...
  if MySynhro.Checked then begin
    if MyTextMessage('Предупреждение','Установлен режим синхронизации.' +#13#10
         + 'В случае продолжения установки клипа режим синхранизации будет отменен.'
         + #13#10 + 'Продолжить?',2) then begin
      if trim(Form1.lbActiveClipID.Caption)='' then exit;
...
procedure TForm1.sbPredClipMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
var s, cl, tm : string;
    rw : integer;
begin
  if panelprepare.Visible then begin
    if not pnTempMenu.Visible then begin
      ...
      s:='';
      if not MySynhro.Checked then begin
        if GridPlayer=grPlayList then begin
          s:='Плей-лист: ';
          if GridPlayerRow<=0 then s:=s+'Не выбрано ни одного клипа из списка.';
          if (GridPlayerRow<=GridActPlayList.RowCount-1) and (GridPlayerRow>1)
            then s:=s+(GridActPlayList.Objects[0,GridPlayerRow-1] as TGridRows).MyCells[3].ReadPhrase('Clip');
        end else begin
          s:='Список клипов: ';
          if GridPlayerRow<=0 then s:=s+'Не выбрано ни одного клипа из списка.';
          if (GridPlayerRow<=GridClips.RowCount-1) and (GridPlayerRow>1)
            then s:=s+(GridClips.Objects[0,GridPlayerRow-1] as TGridRows).MyCells[3].ReadPhrase('Clip');
        end;
      end else begin
        s:='Плей-лист: ';
        rw := FindPredClipTime(GridActPlayList, now);
        if rw>0 then begin
          cl := (GridActPlayList.Objects[0,rw] as TGridRows).MyCells[3].ReadPhrase('Clip');
          tm := (GridActPlayList.Objects[0,rw] as TGridRows).MyCells[3].ReadPhrase('StartTime');
          s:=s+'['+tm+'] ' +cl;
        end else s:=s + 'Предыдущий клип не обнаружен';
      end;
      TempMenu.clear;
      TempMenu.Add(s,100);
      TempMenu.Draw(imgTempMenu.Canvas);
...
procedure TForm1.sbNextClipMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
var s, cl, tm : string;
    rw : integer;
begin
    ...
    if not pnTempMenu.Visible then begin
      ...
      if not MySynhro.Checked then begin
        if GridPlayer=grPlayList then begin
          s:='Плей-лист: ';
          if GridPlayerRow<=0 then s:=s+'Не выбрано ни одного клипа из списка.';
          if (GridPlayerRow<=GridActPlayList.RowCount-2) and (GridPlayerRow>0)
            then s:=s+(GridActPlayList.Objects[0,GridPlayerRow+1] as TGridRows).MyCells[3].ReadPhrase('Clip');
        end else begin
          s:='Список клипов: ';
          if GridPlayerRow<=0 then s:=s+'Не выбрано ни одного клипа из списка.';
        ...
        if rw>0 then begin
          ...
          tm := (GridActPlayList.Objects[0,rw] as TGridRows).MyCells[3].ReadPhrase('StartTime');
          s:=s+'['+tm+'] ' +cl;
        end else s:=s + 'Следующий клип не обнаружен';
      end;
      TempMenu.clear;
      TempMenu.Add(s,100);
      TempMenu.Draw(imgTempMenu.Canvas);
...
begin
  ...
  while wnd <> 0 do begin // Не показываем:
    ...
    begin
      ...
      if not WindowInVisibleList(StrPas(buff)) then SetForegroundWindow(wnd);
      application.ProcessMessages;
...
procedure TForm1.ImgListFilesMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if (X<5) or (X>(ImgListFiles.Width-5))
    then MenuListFiles.SetNotSelet
    else MenuListFiles.MouseMove(ImgListFiles.Canvas, X, Y);
...
procedure TForm1.ImgMainMenuMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if X>=imgMainMenu.Width-10 then begin
     pnMainMenu.visible:=false;
     exit;
  end;
  if X<=5 then begin
  ...
  if Y>=imgMainMenu.Height-10 then begin
    pnMainMenu.visible:=false;
    exit;
  end;
...
begin
  ...
  pnMainMenu.Visible:=false;
     case res of
  0 : ButtonsControlProjects(2);
  1 : ButtonsControlProjects(3);
  2 : ButtonsControlProjects(4);
  3 : ButtonsControlProjects(5);
  4 : ButtonsControlProjects(0);
  5 : ButtonsControlProjects(1);
  6 : ButtonControlLists(0);
  7 : ButtonControlLists(1);
  8 : ButtonControlLists(2);
  9 : EditListHotKeys;
 10 : Close;
 11 : SetOptions;
 12 : ButtonsControlProjects(6);
     end;
  //pnMainMenu.Visible:=false;
end;
...
procedure TForm1.imgAPlFindStrMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
...
begin
  ...
  0 : begin
        ...
        s:=APlText.Text;
        s:=ansilowercase(trim(s));
        if s='клип' then SortGridAlphabet(GridActPlayList, 1, 3, 'Clip', APlText.direction)
        else if s='песня' then SortGridAlphabet(GridActPlayList, 1, 3, 'Song', APlText.direction)
        else if s='исполнитель' then SortGridAlphabet(GridActPlayList, 1, 3, 'Singer', APlText.direction);
      end;
  1 : begin
        TempMenu.clear;
        pnTempMenu.Top:=PanelControl.Height+pnAPlFindStr.Top+pnAPlFindStr.Height
                         -(pnAPlFindStr.Height-18) div 2;
        TempMenu.rowheight:=22;
        pnTempMenu.Height:=TempMenu.rowheight*3+2;
...
procedure TForm1.imgAPlFindTimeMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
var res : integer;
begin
...
begin
  ...
  0 : begin
        ...
        s:=APlTime.Text;
        s:=ansilowercase(trim(s));
        if s='время старта' then SortGridStartTime(GridActPlayList, not APlTime.direction)
        else if s='хр-ж медиа' then SortGridTime(GridActPlayList, 1, 3, 'Duration', APlTime.direction)
        else if s='хр-ж воспр.' then SortGridTime(GridActPlayList, 1, 3, 'Dur', APlTime.direction);
      end;
  1 : begin
        TempMenu.clear;
        pnTempMenu.Top:=PanelControl.Height+pnAPlFindTime.Top+pnAPlFindTime.Height
                         -(pnAPlFindStr.Height-18) div 2;
        TempMenu.rowheight:=22;
        pnTempMenu.Height:=TempMenu.rowheight*3+2;
...
begin
  ...
  MediaStop;
  VLCPlayer.Stop;
  while VLCPlayer.PlayerReady do begin
    FrSaveProject.ProgressBar1.Position:=FrSaveProject.ProgressBar1.Position + 2;
    FrSaveProject.Show;
    application.ProcessMessages;
  end;
  VLCPlayer.Release;
  ...
  ClearUNDO;
  WriteMyIniFile;
  if IsProjectChanges or (not (trim(FileNameProject)=''))
    then if MyTextMessage('Вопрос','Сохранить текущий проект '+ FileNameProject +'?',2)
           then SaveProject;
...
//++++++++++++++++++++++++++++++
  if Button=mbRight then begin
    if (rw <> -1) and (rw <> 0) then begin
      ...
      txt := (GridActPlayList.Objects[0,GridActPlayList.Row] as TGridRows).MyCells[3].ReadPhrase('StartTime');
      clpid := (GridActPlayList.Objects[0,GridActPlayList.Row] as TGridRows).MyCells[3].ReadPhrase('ClipID');
      txt := trim(SetTimeCode(txt));
      (GridActPlayList.Objects[0,GridActPlayList.Row] as TGridRows).MyCells[3].updatephrase('StartTime',txt);
      ...
      (GridActPlayList.Objects[0,GridActPlayList.Row] as TGridRows).MyCells[3].updatephrase('StartTime',txt);
      ps := (GridActPlayList.Objects[0,GridActPlayList.Row] as TGridRows).MyCells[3].PositionName('StartTime');
      ...
      For i:=1 to GridClips.RowCount-1 do begin
        if Trim((GridClips.Objects[0,i] as TGridRows).MyCells[3].ReadPhrase('ClipID'))=trim(clpid) then begin
          (GridClips.Objects[0,i] as TGridRows).MyCells[3].updatephrase('StartTime',txt);
          break;
        end;
      end;
    end;
    ...
    2: if (GridActPlayList.Objects[0,GridActPlayList.Row] is TGridRows) then begin
         ...
         if MySynhro.Checked then begin
           ...
           then begin
             ...
             if (GridActPlayList.Objects[0,GridActPlayList.Row] is TGridRows) then PlayClipFromActPlaylist;
           end;
         end else begin
           GridPlayerRow := GridActPlayList.Row;
           if (GridActPlayList.Objects[0,GridActPlayList.Row] is TGridRows) then PlayClipFromActPlaylist;
...
begin
  try
    ...
    if PanelPrepare.Visible or PanelAir.Visible then RZD:=4
    else if PanelClips.Visible then RZD:=2
    else if PanelProject.Visible then RZD:=1
    else if PanelPlayList.Visible then RZD:=3;
...
//Клавиши переключения доступные во всех окнах
                 ...
                 case CMD of
  1   : begin //0|1|Открыть панель «Проекты»|SHIFT+Q
  ...
  2   : begin //0|2|Открыть панель «Клипы»|SHIFT+W
          WriteLog('MAIN', 'TForm1.FormKeyUp Form1.Visible Key=' + ShiftToStr(Shift, Key));
          if Form1.ActiveControl = RichEdit1 then exit;
          SetMainGridPanel(clips); //Shift + W
          exit;
        end;
  3   : begin//0|3|Открыть панель «Активный плей-лист»|SHIFT+E
          WriteLog('MAIN', 'TForm1.FormKeyUp Form1.Visible Key=' + ShiftToStr(Shift, Key));
          if Form1.ActiveControl = RichEdit1 then exit;
          sbPlayListClick(nil);
          exit; //
        end;
  4   : begin//0|4|Открыть панель «Подготовка»|SHIFT+R
          if Form1.ActiveControl = RichEdit1 then exit;
  ...
  5   : begin //0|5|Открыть панель «Эфир»|SHIFT+T
          if Form1.ActiveControl = RichEdit1 then exit;
...
//Клавиши доступные во окне подготовки
  if PanelPrepare.Visible then begin
    ...
    if ActiveControl=RichEdit1 then exit;
                  case CMD of
  29  : begin //'4|29|Запустить/Остановить воспроизведение клипа|SPACE'
  ...
  30  : begin //'4|30|На один кадр влево|LEFT'
          if vlcmode<>play then ControlPlayerFastSlow(1);
          exit;
        end;
  31  : begin //'4|31|На один кадр вправо|RIGHT'
          if vlcmode<>play then ControlPlayerFastSlow(2);
          exit;
        end;
  32  : begin //'4|32|В начало предыдущего клипа|SHIFT+LEFT'
          ControlPlayerTransmition(1);
          exit;
        end;
  33  : begin //'4|33|В начало следующего клипа|SHIFT+RIGHT'
          ControlPlayerTransmition(2);
          exit;
        end;
  34  : begin //'4|34|На десять кадров влево|CTRL+LEFT'
          if vlcmode<>play then ControlPlayerFastSlow(0);
          exit;
        end;
  35  : begin //'4|35|На десять кадров вправо|CTRL+RIGHT'
          if vlcmode<>play then ControlPlayerFastSlow(3);
          exit;
        end;
  36  : begin //'4|36|Перейти к точке начала воспроизведения|HOME'
          ControlPlayerTransmition(0);
          exit;
        end;
  37  : begin //'4|37|Перейти в точку окончания воспроизведения|END'
          ControlPlayerTransmition(3);
          exit;
        end;
  38  : begin //'4|38|Перейти в начало клипа|SHIFT+HOME'
          SaveToUNDO;
  ...
  39  : begin //'4|39|Перейти в конец клипа |SHIFT+END'
          SaveToUNDO;
  ...
  40  : begin //'4|40|Отменить предыдущее действие|CTRL+Z'
          ControlButtonsPrepare(10); //Ctrl + Z
          exit;
        end;
  41  : begin //'4|41|Уменьшить скорость воспроизведения в 2 раза|CTRL+<'
          if vlcmode=play then ControlPlayerFastSlow(1);
          exit;
        end;
  42  : begin //'4|42|Увеличить скорость воспроизведения в 2 раза|CTRL+>'
          if vlcmode=play then ControlPlayerFastSlow(2);
          exit;
        end;
  43  : begin //'4|43|Уменьшить скорость воспроизведения в 4 раза|SHIFT+<'
          if vlcmode=play then ControlPlayerFastSlow(0);
          exit;
        end;
  44  : begin //'4|44|Увеличить скорость воспроизведения в 4 раза|SHIFT+>'
          if vlcmode=play then ControlPlayerFastSlow(3);
          exit;
        end;
  45  : begin //'4|45|Подтянуть границу события находящуюся слева от курсора|C'
          ControlButtonsPrepare(0);
          exit;
        end;
  46  : begin //'4|46|Подтянуть границу события находящуюся справа от курсора|V'
          ControlButtonsPrepare(1);
          exit;
        end;
  47  : begin //'4|47|Сдвинуть тайм-линию/линии на заданное количество кадров|CTRL+S'
          ControlButtonsPrepare(2);
          exit;
        end;
  48  : begin //'4|48|Установить короткие номера событий|CTRL+D'
          ControlButtonsPrepare(3);
          exit;
        end;
  49  : begin //'4|49|Вырезать событие/события в буфер обмена|CTRL+X'
          ControlButtonsPrepare(6);
          exit;
        end;
  50  : begin //'4|50|Копировать событие/события в буфер обмена|CTRL+C'
          ControlButtonsPrepare(7);
          exit;
        end;
  51  : begin //'4|51|Вставить событие/события из буфера обмена|CTRL+V'
          ControlButtonsPrepare(8);
          exit;
        end;
  52  : begin //'4|52|Удалить событие/события без возможности восстановления|DELETE');
          ControlButtonsPrepare(9);
          exit;
        end;
  320 : begin //'4|320|Вывести данные на печать|SHIFT+P');
          ControlButtonsPrepare(4);
          exit;
        end;
  321 : begin //'4|321|Сохранить редактируемую тайм-линию в файл|SHIFT+W');
          ControlButtonsPrepare(5);
          exit;
        end;
  322 : begin//'4|322|Загрузить данные из файла на редактируемую тайм-линию|SHIFT+R');
          ControlButtonsPrepare(11);
          exit;
        end;
  323 : begin//'4|324|Проверить времена запуска клипов в текущем плей-листе|CTRL+T');
          if listbox1.ItemIndex<0 then exit;
          ButtonsControlPlayList(4);
          exit;
        end;
  53  : begin //'4|53|Загрузить предыдущее событие из списка (клипы, плей-лист)|SHIFT+A'
          if Form1.ActiveControl = RichEdit1 then exit;
          sbPredClipClick(nil);
          exit;
        end;
  54  : begin //'4|54|Загрузить следующее событие из списка (клипы, плей-лист)|SHIFT+S'
          if Form1.ActiveControl = RichEdit1 then exit;
          sbNextClipClick(nil);
          exit;
        end;
  55  : begin //'4|55|Открыть/Закрыть список графических шаблонов|SHIFT+G'
          if Form1.ActiveControl = RichEdit1 then exit;
          CheckBox1.Checked:= not CheckBox1.Checked;
          CheckBox1Click(nil);
          exit;
        end;
  56  : begin //'4|56|Открыть панель воспроизведения видео|SHIFT+D'
          if Form1.ActiveControl = RichEdit1 then exit;
          MyMediaSwitcher.Select:=0;
          SwitcherVideoPanels(0);
          exit;
        end;
  57  : begin //'4|57|Открыть панель воспроизведения шаблонов|SHIFT+F'
          if Form1.ActiveControl = RichEdit1 then exit;
          MyMediaSwitcher.Select:=1;
          SwitcherVideoPanels(1);
          exit;
        end;
  58  : begin //'4|58|Установить начальную точку воспроизведения|I'
          if TLParameters.Position < TLParameters.Finish then begin
            SaveToUNDO;
  ...
  59  : begin //'4|59|Установить конечную точку воспроизведения|O'
  ...
  70  : begin //'4|70|Установить нулевую точку|CTRL+O');
          if vlcmode<>play then ButtonsControlMedia(1);
          exit;
        end;
  71  : begin //'4|71|Установить время запуска клипа|CTRL+ALT+M');
          if vlcmode<>play then SetLTC(2);
          exit;
        end;
  80  : begin //'4|80|Выделить все события на тайм-линии редактирования|CTRL+ALT+S'
          if vlcmode = play then exit;
          if TLZone.TLEditor.Count > 0 then begin
            SaveToUNDO;
            for i:=0 to TLZone.TLEditor.Count-1 do TLZone.TLEditor.Events[i].Select:=true;
            TLZone.TLEditor.DrawEditor(bmptimeline.Canvas,TLZone.TLEditor.FindEventPos(TLParameters.ScreenStartFrame));
            TLZone.DrawTimelines(Form1.imgtimelines.Canvas,bmptimeline);
          end;
          exit;
        end;
  81  : begin //'4|81|Отменить все выделенные события|CTRL+ALT+D'
          if vlcmode = play then exit;
          if TLZone.TLEditor.Count > 0 then begin
            SaveToUNDO;
            for i:=0 to TLZone.TLEditor.Count-1 do TLZone.TLEditor.Events[i].Select:=false;
            TLZone.TLEditor.DrawEditor(bmptimeline.Canvas,TLZone.TLEditor.FindEventPos(TLParameters.ScreenStartFrame));
            TLZone.DrawTimelines(Form1.imgtimelines.Canvas,bmptimeline);
          end;
          exit;
        end;
  101 : begin //'4|101|Установить событие для устройства 1|1|NUMPAD1'
          SaveToUNDO;
          InsertEventToEditTimeline(0);
          exit;
        end;
  102 : begin //'4|102|Установить событие для устройства 2|2|NUMPAD2'
          SaveToUNDO;
          InsertEventToEditTimeline(1);
          exit;
        end;
  103 : begin //'4|103|Установить событие для устройства 3|3|NUMPAD3'
          SaveToUNDO;
          InsertEventToEditTimeline(2);
          exit;
        end;
  104 : begin //'4|104|Установить событие для устройства 4|4|NUMPAD4'
          SaveToUNDO;
          InsertEventToEditTimeline(3);
          exit;
        end;
  105 : begin //'4|105|Установить событие для устройства 5|5|NUMPAD5'
          SaveToUNDO;
          InsertEventToEditTimeline(4);
          exit;
        end;
  106 : begin //'4|106|Установить событие для устройства 6|6|NUMPAD6'
          SaveToUNDO;
          InsertEventToEditTimeline(5);
          exit;
        end;
  107 : begin //'4|107|Установить событие для устройства 7|7|NUMPAD7'
          SaveToUNDO;
          InsertEventToEditTimeline(6);
          exit;
        end;
  108 : begin //'4|108|Установить событие для устройства 8|8|NUMPAD8'
          SaveToUNDO;
          InsertEventToEditTimeline(7);
          exit;
        end;
  109 : begin //'4|109|Установить событие для устройства 9|9|NUMPAD9'
          SaveToUNDO;
          InsertEventToEditTimeline(8);
          exit;
        end;
  110 : begin //'4|110|Установить событие для устройства 10|0|NUMPAD0'
          SaveToUNDO;
          InsertEventToEditTimeline(9);
          exit;
        end;
  111 : begin //'4|111|Установить событие для устройств 11|CTRL+1|CTRL+NUMPAD1'
          SaveToUNDO;
          InsertEventToEditTimeline(10);
          exit;
        end;
  112 : begin //'4|112|Установить событие для устройств 12|CTRL+2|CTRL+NUMPAD2'
          SaveToUNDO;
          InsertEventToEditTimeline(11);
          exit;
        end;
  113 : begin //'4|113|Установить событие для устройств 13|CTRL+3|CTRL+NUMPAD3'
          SaveToUNDO;
          InsertEventToEditTimeline(12);
          exit;
        end;
  114 : begin //'4|114|Установить событие для устройств 14|CTRL+4|CTRL+NUMPAD4'
          SaveToUNDO;
          InsertEventToEditTimeline(13);
          exit;
        end;
  115 : begin //'4|115|Установить событие для устройств 15|CTRL+5|CTRL+NUMPAD5'
          SaveToUNDO;
          InsertEventToEditTimeline(14);
          exit;
        end;
  116 : begin //'4|116|Установить событие для устройств 16|CTRL+6|CTRL+NUMPAD6'
          SaveToUNDO;
          InsertEventToEditTimeline(15);
          exit;
        end;
  117 : begin //'4|117|Установить событие для устройств 17|CTRL+7|CTRL+NUMPAD7'
          SaveToUNDO;
          InsertEventToEditTimeline(16);
          exit;
        end;
  118 : begin //'4|118|Установить событие для устройств 18|CTRL+8|CTRL+NUMPAD8'
          SaveToUNDO;
          InsertEventToEditTimeline(17);
          exit;
        end;
  119 : begin //'4|119|Установить событие для устройств 19|CTRL+9|CTRL+NUMPAD9'
          SaveToUNDO;
          InsertEventToEditTimeline(18);
          exit;
        end;
  120 : begin //'4|120|Установить событие для устройств 20|CTRL+0|CTRL+NUMPAD0'
          SaveToUNDO;
          InsertEventToEditTimeline(19);
          exit;
        end;
  121 : begin //'4|121|Установить событие для устройств 21|SHIFT+1'
          SaveToUNDO;
          InsertEventToEditTimeline(20);
          exit;
        end;
  122 : begin //'4|122|Установить событие для устройств 22|SHIFT+2'
          SaveToUNDO;
          InsertEventToEditTimeline(21);
          exit;
        end;
  123 : begin //'4|123|Установить событие для устройств 23|SHIFT+3'
          SaveToUNDO;
          InsertEventToEditTimeline(22);
          exit;
        end;
  124 : begin //'4|124|Установить событие для устройств 24|SHIFT+4'
          SaveToUNDO;
          InsertEventToEditTimeline(23);
          exit;
        end;
  125 : begin //'4|125|Установить событие для устройств 25|SHIFT+5'
          SaveToUNDO;
          InsertEventToEditTimeline(24);
          exit;
        end;
  126 : begin //'4|126|Установить событие для устройств 26|SHIFT+6'
          SaveToUNDO;
          InsertEventToEditTimeline(25);
          exit;
        end;
  127 : begin //'4|127|Установить событие для устройств 27|SHIFT+7'
          SaveToUNDO;
          InsertEventToEditTimeline(26);
          exit;
        end;
  128 : begin //'4|128|Установить событие для устройств 28|SHIFT+8'
          SaveToUNDO;
          InsertEventToEditTimeline(27);
          exit;
        end;
  129 : begin //'4|129|Установить событие для устройств 29|SHIFT+9'
          SaveToUNDO;
          InsertEventToEditTimeline(28);
          exit;
        end;
  130 : begin //'4|130|Установить событие для устройств 30|SHIFT+0'
          SaveToUNDO;
          InsertEventToEditTimeline(29);
          exit;
        end;
  131 : begin //'4|131|Установить событие для устройств 31|ALT+1|ALT+NUMPAD1'
          SaveToUNDO;
          InsertEventToEditTimeline(30);
          exit;
        end;
  132 : begin //'4|132|Установить событие для устройств 32|ALT+2|ALT+NUMPAD2'
          SaveToUNDO;
          InsertEventToEditTimeline(31);
          exit;
        end;
  60  : begin //'4|60|Текстовая тайм-линия: Разделить событие на части по курсору|ALT+С'
...
//          exit;
//        end;
  62  : begin //'4|62|Медиа тайм-линия: Установить маркер|CTRL+M'
          if pnMediaTL.Visible then begin
            ButtonsControlMedia(4);
            exit;
          end;
        end;
  63  : begin //'4|63|Медиа тайм-линия: Удалить маркер|ALT+M'
          if pnMediaTL.Visible then begin
            ButtonsControlMedia(5);
            exit;
          end;
        end;
  200 : begin //'4|200|Увеличить размер тайм-линий по вертикали|SHIFT+PLUS|SHIFT+NUMPADPLUS'
          TLHeights.StepPlus;
         //TLHeights.StepMinus;
          TLZone.TLEditor.DrawEditor(bmptimeline.Canvas,0);
          TLZone.DrawBitmap(bmptimeline);
  ...
  201 : begin //'4|201|Уменьшить размер тайм-линий по вертикали|SHIFT+MINUS|SHIFT+NUMPADMUNUS'
          TLHeights.StepMinus;
          TLZone.TLEditor.DrawEditor(bmptimeline.Canvas,0);
          TLZone.DrawBitmap(bmptimeline);
          Form1.imgTimelines.Canvas.Lock;
  ...
  202 : begin //4|202|Увеличить масштаб тайм-линий по горизонтали|CTRL+PLUS|CTRL+NUMPADPLUS'
          TLZone.PlusHoriz;
          TLZone.TLEditor.DrawEditor(bmptimeline.Canvas,0);
          TLZone.DrawBitmap(bmptimeline);
          Form1.imgTimelines.Canvas.Lock;
  ...
  203 : begin //4|203|Уменьшить масштаб тайм-линий по горизонтали|CTRL+MINUS|CTRL+NUMPADMUNUS'
          TLZone.MinusHoriz;
          TLZone.TLEditor.DrawEditor(bmptimeline.Canvas,0);
          TLZone.DrawBitmap(bmptimeline);
          Form1.imgTimelines.Canvas.Lock;
...
//Кнопки панели Список клипов
  if PanelClips.Visible then begin
    WriteLog('MAIN', 'TForm1.FormKeyUp PanelClips.Visible Key=' + ShiftToStr(Shift, Key));
               case CMD of
22    : begin //'2|22|Импортировать клипы в систему|CTRL+I'
          ButtonsControlClipsPanel(0);
          exit;
        end;
23    : begin //'2|23|Удалить клип/клипы из списка|DELETE'
          ButtonsControlClipsPanel(5);          exit;        end;24    : begin //'2|24|Загрузить выбранный клип в окно подготовки|CTRL+P'          ButtonsControlClipsPanel(2);          exit;        end;25    : begin //'2|25|Сортировать список клипов|CTRL+S'          ButtonsControlClipsPanel(3);
          exit;
        end;
26    : begin //'2|26|Загрузить клип/клипы в активный плей-лист|CTRL+L'
          ButtonsControlClipsPanel(4);
          exit;
        end;
72    : begin //'2|72|Создать новый клип|CTRL+N'
...
//Кнопки панели Активный плей-лист
  ...
  if PanelPlayList.Visible then begin
    WriteLog('MAIN', 'TForm1.FormKeyUp PanelPlayList.Visible Key=' + ShiftToStr(Shift, Key));
           Case CMD of
  27  : begin //'3|27|Удалить клип/клипы из система|DELETE'
          ButtonsControlPlayList(5);
          exit;
        end;
  28  : begin //'3|28|Загрузить выбранный клип в окно подготовки|CTRL+P'
          ButtonsControlPlayList(2);
          exit;
        end;
  73  : begin //'3|73|Создать новый плей лист|CTRL+N'
          ButtonsControlPlayList(0);
          exit;
        end;
  74  : begin //'3|74|Редактировать список клипов текущего плей листа|CTRL+R'
          ButtonsControlPlayList(1);
          exit;
        end;
  310 : begin //'3|310|Сортировать клипы в текущем плей-листе|CTRL+S'
          ButtonsControlPlayList(3);
          exit;
        end;
  311 : begin//'3|311|Проверить установку времени старта в плей-листе|CTRL+T'
          ButtonsControlPlayList(4);
          exit;
        end;
           End;
...
//Кнопки панели Проектов
  ...
  if panelproject.Visible then begin
    WriteLog('MAIN', 'TForm1.FormKeyUp panelproject.Visible Key=' + ShiftToStr(Shift, Key));
            case CMD of
  6   : begin //'1|6|Создать новый проект|SHIFT+N'
          ButtonsControlProjects(2);
          exit;
        end;
  7   : begin //'1|7|Редактировать выбранный плей-лист|CTRL+R'
          if GridLists.Row > 0
            then if (GridLists.Objects[0,GridLists.Row] is TGridRows)
                   then if (GridLists.Objects[0,GridLists.Row] as TGridRows).ID=0
                          then EditPlayList(-1)
                          else EditPlayList(GridLists.Row);
          exit;
        end;
  8   : begin //'1|8|Открыть проект|CTRL+O'
          if vlcmode<>play then ButtonsControlProjects(3);          exit;        end;  9   : begin //'1|9|Сохранить проект как|SHIFT+ S'          if vlcmode<>play then ButtonsControlProjects(5);
          exit;
        end;
  10  : begin //'1|10|Блокировать/разблокировать текущий проект|SHIFT+B'
          //imgBlockProjectsClick(nil);
          exit;
        end;
//  11  : begin //'1|11|Переключение между списками окна проектов|TAB'
////        end;  12  : begin //'1|12|Создать новую тайм-линию|CTRL+PLUS'          ButtonControlLists(0);
          exit;
        end;
  13  : begin //'1|13|Удалить выбранную тайм-линию|CTRL+MINUS'
          ButtonControlLists(1);
          exit;
        end;
  14  : begin //'1|14|Редактировать выбранную тайм-линию|CTRL+T'
          GridTimeLinesDblClick(nil);
          exit;
        end;
  15  : begin //'1|15|Создать новый плей-лист|CTRL+N'
          ButtonPlaylLists(0);
          exit;
        end;
  16  : begin //'1|16|Удалить плей-лист|CTRL+D'
          ButtonPlaylLists(1);
          exit;
        end;
  17  : begin //'1|17|Сохранить изменения сделанные в окне проекта|CTRL+S'
          if vlcmode<>play then ButtonsControlProjects(4);
          //ButtonControlLists(6);
          exit;
        end;
  18  : begin //'1|18|Сортировать элементы списка (плей-листы, шаблоны)|ALT+S'
          ButtonPlaylLists(2);          exit;        end;  //19  : begin //'1|19|Открыть список «Плей-листы»|ALT+P'  //
  //      end;
  20  : begin //'1|20|Открыть список «Графические шаблоны»|ALT+G'
          EditImageTamplate;
          exit;
        end;
  21  : begin //'1|21|Открыть список «Текстовые шаблоны»|ALT+T'
...
procedure TForm1.imgTempMenuMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  ...
  if MySynhro.Checked then begin
    ...
    //if trim(lbTypeTC.Caption)<>'' then begin
      ...
      //                  + 'Загрузить очередной клип из плей-листа [Нажмите - Нет]', 2)  then begin
        ...
        if lbActiveClipID.Caption<> '' then begin
           if GridPlayer=grPlayList then begin
             rw := FindClipinGrid(GridActPlaylist, lbActiveClipID.Caption);
             if rw>0
               then lbTypeTC.Caption := (GridActPlaylist.Objects[0,rw] as tgridrows).MyCells[3].ReadPhrase('StartTime');
           end else begin
             rw := FindClipinGrid(GridClips, lbActiveClipID.Caption);
             if rw>0
               then lbTypeTC.Caption := (GridClips.Objects[0,rw] as tgridrows).MyCells[3].ReadPhrase('StartTime');
           end;
           if trim(lbTypeTC.Caption)<>'' then MyStartPlay:=StrTimeCodeToFrames(lbTypeTC.Caption);
        end;
      //end;
    //end;
  end else begin
...
procedure TForm1.GridListsMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if pnmainmenu.Visible then pnmainmenu.Visible:=false;
end;
...
procedure TForm1.GridClipsMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if pnTempMenu.Visible then pnTempMenu.Visible := false;
  if pnmainmenu.Visible then pnmainmenu.Visible:=false;
...
procedure TForm1.Image4MouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if (X<5) or (X>(image4.Width - 10))
    then InputPopUpMenu.SetNotSelet
    else InputPopUpMenu.MouseMove(Image4.Canvas, X, Y);
  InputPopUpMenu.Draw(Image4.Canvas);
  Image4.Repaint;
end;
...
begin
  ...
  0 : begin
        PanelStartWindow.Visible := false;
        if not CreateProject(-1) then begin
          MyStartWindow;
          exit;
        end;
        EnableProgram;
...
procedure TForm1.sbMainMenuMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if (X<5) or ((X<5) and (Y>sbMainMenu.Height-5)) or (Y<3)
  then pnMainMenu.Visible:=false
  else begin
    if not pnMainMenu.Visible then begin
      myMainMenu.Draw(imgMainMenu.Canvas);
      if ListBox1.ItemIndex<>-1
        then SavePlayListFromGridToFile((ListBox1.Items.Objects[ListBox1.ItemIndex]
