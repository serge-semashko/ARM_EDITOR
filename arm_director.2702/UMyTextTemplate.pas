  ...
  for i:=0 to frMyTextTemplate.ComponentCount-1 do begin
    if lowercase(frMyTextTemplate.Components[i].ClassName)='tlistbox' then begin
      ...
      if lowercase(trim((frMyTextTemplate.Components[nl] as TLabel).Caption))=lowercase(trim(cph)) then begin
        ...
        (frMyTextTemplate.Components[nl] as TLabel).Font.Color:=SmoothColor(FormsFontColor,32);
        (frMyTextTemplate.Components[nl] as TLabel).Font.Style:=(frMyTextTemplate.Components[nl] as TLabel).Font.Style - [fsBold];
      end;
    end;
  end;
...
begin
  ...
  for i:=frMyTextTemplate.ComponentCount-1 downto 0 do begin
    if lowercase(frMyTextTemplate.Components[i].ClassName)='tlistbox' then begin
  ...
  for i:=frMyTextTemplate.ComponentCount-1 downto 0 do begin
    if lowercase(frMyTextTemplate.Components[i].ClassName)='tlabel' then begin
      snm := lowercase(trim((frMyTextTemplate.Components[i] as TLabel).Name));
      snm := copy(snm,1,2);
      if snm='lb' then begin
        frMyTextTemplate.Components[i].Free;
        //frMyTextTemplate.Components[i]:=nil;
      end;
    end;
...
procedure SaveTextTemplateToStream(Stream : TFileStream);
var nm, cph, renm, FileName : string;
    i, j, nl, cnt, cntlst, cntcol : integer;
begin
  cntlst := nom;
  cntcol := frMyTextTemplate.SpinEdit1.Value;
...
procedure LoadTextTemplateFromStream(Stream : TFileStream);
var nm, cph, nnm, renm, FileName : string;
    i, j, nl, cnt, cntlst, cntcol : integer;
begin
  //FileName:=pathtemp + '\TextTemplates.lst';
  //if not FileExists(FileName) then exit;
...
begin
  with frMyTextTemplate do begin
    ...
    Panel1.Visible := false;
    ShowModal;
    if ModalResult=mrOk then SaveTextTemplateToFile;
  end;
end;
...
begin
  ...
  with frMyTextTemplate do begin
    ...
    if ModalResult=mrOk then begin
      if CheckBox1.Checked then result := '#' + trim(Edit1.Text) else result := trim(Edit1.Text);
    end;
  end;
end;
