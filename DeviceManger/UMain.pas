...
// Если запуск воспроизведения выполнен то отображаем движение тайм-линий.
   If MyTimer.Enable then begin
     ...
     if Form1.label1.Caption<>'' then TLParameters.Position :=TLParameters.Preroll + StrTimeCodeToFrames(Form1.label1.Caption);
     ...
     if OldList1Index<>crpos.Number then begin
       form1.memo1.Lines.Insert(0, inttostr(crpos.Number) + ')  Текущее - ' + curr + '  Следующее - ' + next +
