...
begin
  ...
  if trim(sv)<>'' then sv:=' v: '+trim(sv);
  sa:=vlcplayer.AudioCodec;
  if trim(sa)<>'' then sa:=' a: '+trim(sa);

  mediadata:=inttostr(vlcplayer.Width)+':'+inttostr(vlcplayer.Height) + sv + sa;
  vlcplayer.tracks:='';
  MediaPause;
end;
...
begin
  ...
  vlcmode := play;
   if VLCPlayer.p_mi <> nil then vlcplayer.play;
