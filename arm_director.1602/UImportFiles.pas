...
// Заносим результат в список клипов
    ...
    If ModalResult=mrOk then begin
      ...
      with (Form1.GridClips.Objects[0,rw] as TGridRows).MyCells[3] do begin
        ...
        //snch := '';
        if CheckBox1.Checked then UpdatePhrase('StartTime',edStartTime.Text) else UpdatePhrase('StartTime','');
...
begin
  ...
  with (Form1.GridClips.Objects[0,ARow] as TGridRows).MyCells[3] do begin
    ...
    pntlclips.SetText('TotalTime',FramesToStr(dur)); //Form1.lbclipfulldur.Caption := FramesToStr(dur);
  end;
