...
begin
  ...
  Strike       := MTT.Strike;
  Flags        := MTT.Flags;
  For i:=0 to MTT.Count-1 do begin
    Count:=Count+1;
...
begin
  ...
  Rows[Count-1] := TMyTextRow.Create;
  if Count=1
    then Rows[0].Rect.Top:=HeightTitle + offsettop
    else Rows[Count-1].Rect.Top:=Rows[Count-2].Rect.Bottom + RowInterval;
...
var i : integer;
begin
  For i:=ARow to Count-2 do begin
    Rows[i].Clear;
...
begin
  for i:=0 to Count-1 do begin
    for j:=0 to Rows[i].Count-1 do begin
      if lowercase(trim(TName))=lowercase(trim(Rows[i].Columns[j].Name)) then begin
    ...
    if Count>0 then begin
      for i:=0 to count-1 do begin
        ...
        for j:=0 to Rows[i].Count-1 do begin
          ...
          if Rows[i].Columns[j].WordWrap
            then flg := Rows[i].Columns[j].Flags or DT_WORDBREAK
            else flg := Rows[i].Columns[j].Flags;
...
begin
  for i:=0 to Count-1 do begin
    if i=0
      then Rows[0].Rect.Top:=HeightTitle + offsettop
      else Rows[i].Rect.Top:=Rows[i-1].Rect.Bottom + RowInterval;
...
begin
  ...
  for i:=0 to TTR.Count-1 do begin
    Count:=Count+1;
...
begin
  ...
  if Columns[Count-1].Rect.Right>Rect.Right then Columns[Count-1].Rect.Right:=Rect.Right;
  Columns[Count-1].Rect.Bottom := Rect.Bottom;
  result := Count-1;
end;
...
var i : integer;
begin
  if ACol>=Count then exit;
  for i:=ACol to Count-2 do Columns[i].Assign(Columns[i+1]);
