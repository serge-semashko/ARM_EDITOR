...
//Параметры синхронизации программы
    ini.WriteFloat('SYNH','Shift',MyShift);
        case MySinhro of
    chltc    : ini.WriteInteger('SINH','Type',0);
    chsystem : ini.WriteInteger('SINH','Type',1);
...
//  Общие настройки программы
    ...
    ini.WriteString('MAIN','ProgrammHintBtnFontName',ProgrammHintBtnFontName);
    ini.WriteInteger('MAIN','ProgrammHintBTNSFontColor',ProgrammHintBTNSFontColor);
    ini.WriteInteger('MAIN','ProgrammHintBTNSFontSize',ProgrammHintBTNSFontSize);
...
//    ini.WriteInteger('GRIDS','GridMainFontColor',GridFontColor);
    ...
    ini.WriteInteger('GRIDS','RowGridGrTemplateSelect',RowGridGrTemplateSelect);
...
// Цвета по умолчанию для событий
    For i:=0 to 31 do ini.WriteInteger('DefaultColors','Dev' + inttostr(i+1),DefaultColors[i]);
...
//Размеры щрифтов для таблиц панелей
  ...
  WriteLog('MAIN', 'Окончание записи IniFile: ' + AppPath + AppName + '.ini');
    ...
    on E: Exception do WriteLog('MAIN', 'WriteMyIniFile(' + AppPath + AppName + '.ini) | ' + E.Message);
  end;
end;
...
//           end;
    ...
    zn := ini.ReadInteger('MAIN','MainGrid',0);
         case zn of
    0 : MainGrid := projects;
    1 : MainGrid := playlists;
    2 : MainGrid := clips;
    3 : MainGrid := actplaylist;
    4 : MainGrid := grtemplate;
...
//         end;//case
    ...
    ProgrammCommentColor := ini.ReadInteger('MAIN','ProgrammCommentColor',ProgrammCommentColor);
    ProgrammFontColor := ini.ReadInteger('MAIN','ProgrammFontColor',ProgrammFontColor);
    ProgrammFontName := ini.ReadString('MAIN','ProgrammFontName',ProgrammFontName);
    ProgrammFontSize := ini.ReadInteger('MAIN','ProgrammFontSize',ProgrammFontSize);
    ProgrammEditColor := ini.ReadInteger('MAIN','ProgrammEditColor',ProgrammEditColor);
    ProgrammEditFontColor := ini.ReadInteger('MAIN','ProgrammEditFontColor',ProgrammEditFontColor);
    ProgrammEditFontName := ini.ReadString('MAIN','ProgrammEditFontName',ProgrammEditFontName);
    ProgrammEditFontSize := ini.ReadInteger('MAIN','ProgrammEditFontSize',ProgrammEditFontSize);

    ProgrammBtnFontSize := ini.ReadInteger('MAIN','ProgrammBtnFontSize',ProgrammBtnFontSize);
    ProgrammHintBtnFontName := ini.ReadString('MAIN','ProgrammHintBtnFontName',ProgrammHintBtnFontName);
    ProgrammHintBTNSFontColor := ini.ReadInteger('MAIN','ProgrammHintBTNSFontColor',ProgrammHintBTNSFontColor);
    ProgrammHintBTNSFontSize := ini.ReadInteger('MAIN','ProgrammHintBTNSFontSize',ProgrammHintBTNSFontSize);

    StepMouseWheel := ini.ReadInteger('MAIN','StepMouseWheel',StepMouseWheel);
    MainWindowStayOnTop := ini.ReadBool('MAIN','MainWindowStayOnTop',MainWindowStayOnTop);
    ...
    InputWithoutUsers := ini.ReadBool('MAIN','InputWithoutUsers',InputWithoutUsers);
    //Основные параметры вспомогательных форм
    FormsColor := ini.ReadInteger('FORMS','FormsColor',FormsColor);
    FormsFontColor := ini.ReadInteger('FORMS','FormsFontColor',FormsFontColor);
    FormsFontSize := ini.ReadInteger('FORMS','FormsFontSize',FormsFontSize);
    FormsSmallFont := ini.ReadInteger('FORMS','FormsSmallFont',FormsSmallFont);
    FormsFontName := ini.ReadString('FORMS','FormsFontName',FormsFontName);
    FormsEditColor := ini.ReadInteger('FORMS','FormsEditColor',FormsEditColor);
    FormsEditFontColor := ini.ReadInteger('FORMS','FormsEditFontColor',FormsEditFontColor);
    FormsEditFontSize := ini.ReadInteger('FORMS','FormsEditFontSize',FormsEditFontSize);
    FormsEditFontName := ini.ReadString('FORMS','FormsEditFontName',FormsEditFontName);

    //Основные параметры гридов
    GridBackGround := ini.ReadInteger('GRIDS','GridBackGround',GridBackGround);
...
//    GridMainFontColor := ini.ReadInteger('GRIDS','GridMainFontColor',GridMainFontColor);
    ...
    GridColorSelection := ini.ReadInteger('GRIDS','GridColorSelection',GridColorSelection);
    RowGridGrTemplateSelect := ini.ReadInteger('GRIDS','RowGridGrTemplateSelect',RowGridGrTemplateSelect);

    GridTitleFontName := ini.ReadString('GRIDS','GridTitleFontName',GridTitleFontName);
    GridTitleFontColor := ini.ReadInteger('GRIDS','GridTitleFontColor',GridTitleFontColor);
    GridTitleFontSize := ini.ReadInteger('GRIDS','GridTitleFontSize',GridTitleFontSize);
    GridTitleFontBold := ini.ReadBool('GRIDS','GridTitleFontBold',GridTitleFontBold);
    GridTitleFontItalic := ini.ReadBool('GRIDS','GridTitleFontItalic',GridTitleFontItalic);
    GridTitleFontUnderline := ini.ReadBool('GRIDS','GridTitleFontUnderline',GridTitleFontUnderline);
    ...
    GridFontUnderline := ini.ReadBool('GRIDS','GridFontUnderline',GridFontUnderline);
    GridSubFontName := ini.ReadString('GRIDS','GridSubFontName',GridSubFontName);
    GridSubFontColor := ini.ReadInteger('GRIDS','GridSubFontColor',GridSubFontColor);
    GridSubFontSize := ini.ReadInteger('GRIDS','GridSubFontSize',GridSubFontSize);
    GridSubFontBold := ini.ReadBool('GRIDS','GridSubFontBold',GridSubFontBold);
    GridSubFontItalic := ini.ReadBool('GRIDS','GridSubFontItalic',GridSubFontItalic);
    GridSubFontUnderline := ini.ReadBool('GRIDS','GridSubFontUnderline',GridSubFontUnderline);
    ProjectHeightTitle := ini.ReadInteger('GRIDS','ProjectHeightTitle',ProjectHeightTitle);
    ProjectHeightRow := ini.ReadInteger('GRIDS','ProjectHeightRow',ProjectHeightRow);
    ProjectRowsTop := ini.ReadInteger('GRIDS','ProjectRowsTop',ProjectRowsTop);
    ProjectRowsHeight := ini.ReadInteger('GRIDS','ProjectRowsHeight',ProjectRowsHeight);
    ProjectRowsInterval := ini.ReadInteger('GRIDS','ProjectRowsInterval',ProjectRowsInterval);
    ...
    PLRowsInterval := ini.ReadInteger('GRIDS','PLRowsInterval',PLRowsInterval);
    ClipsHeightTitle := ini.ReadInteger('GRIDS','ClipsHeightTitle',ClipsHeightTitle);
    ClipsHeightRow := ini.ReadInteger('GRIDS','ClipsHeightRow',ClipsHeightRow);
    ClipsRowsTop := ini.ReadInteger('GRIDS','ClipsRowsTop',ClipsRowsTop);
    ClipsRowsHeight := ini.ReadInteger('GRIDS','ClipsRowsHeight',ClipsRowsHeight);
    ClipsRowsInterval := ini.ReadInteger('GRIDS','ClipsRowsInterval',ClipsRowsInterval);
    ListTxtHeightTitle := ini.ReadInteger('GRIDS','ListTxtHeightTitle',ListTxtHeightTitle);
    ListTxtHeightRow := ini.ReadInteger('GRIDS','ListTxtHeightRow',ListTxtHeightRow);
    ListTxtRowsTop := ini.ReadInteger('GRIDS','ListTxtRowsTop',ListTxtRowsTop);
    ListTxtRowsHeight := ini.ReadInteger('GRIDS','ListTxtRowsHeight',ListTxtRowsHeight);
    ListGRHeightTitle := ini.ReadInteger('GRIDS','ListGRHeightTitle',ListGRHeightTitle);
    ListGRHeightRow := ini.ReadInteger('GRIDS','ListGRHeightRow',ListGRHeightRow);
    ListGRRowsTop := ini.ReadInteger('GRIDS','ListGRRowsTop',ListGRRowsTop);
    ListGRRowsHeight := ini.ReadInteger('GRIDS','ListGRRowsHeight',ListGRRowsHeight);
    ListGRRowsInterval := ini.ReadInteger('GRIDS','ListGRRowsInterval',ListGRRowsInterval);
    PhraseErrorColor := ini.ReadInteger('GRIDS','PhraseErrorColor',PhraseErrorColor);
    PhrasePlayColor := ini.ReadInteger('GRIDS','PhrasePlayColor',PhrasePlayColor);

    MyCellColorTrue := ini.ReadInteger('GRIDS','MyCellColorTrue',MyCellColorTrue);
    MyCellColorFalse := ini.ReadInteger('GRIDS','MyCellColorFalse',MyCellColorFalse);
    MyCellColorSelect := ini.ReadInteger('GRIDS','MyCellColorSelect',MyCellColorSelect);

    //Основные параметры Тайм-линий
    TLBackGround := ini.ReadInteger('TLS','TLBackGround',TLBackGround);
    TLZoneNamesColor := ini.ReadInteger('TLS','TLZoneNamesColor',TLZoneNamesColor);
    TLZoneNamesFontSize := ini.ReadInteger('TLS','TLZoneNamesFontSize',TLZoneNamesFontSize);
    TLZoneNamesFontColor := ini.ReadInteger('TLS','TLZoneNamesFontColor',TLZoneNamesFontColor);
    TLZoneFontColorSelect := ini.ReadInteger('TLS','TLZoneFontColorSelect',TLZoneFontColorSelect);
    TLZoneNamesFontName := ini.ReadString('TLS','TLZoneNamesFontName',TLZoneNamesFontName);
    ...
    AirColorTimeLine := ini.ReadInteger('TLS','AirColorTimeLine',AirColorTimeLine);
    DevBackGround := ini.ReadInteger('TLS','DevBackGround',DevBackGround);
    TimeForeGround := ini.ReadInteger('TLS','TimeForeGround',TimeForeGround);
    TimeSecondColor := ini.ReadInteger('TLS','TimeSecondColor',TimeSecondColor);
    AirFontComment := ini.ReadInteger('TLS','AirFontComment',AirFontComment);
    DefaultMediaColor := ini.ReadInteger('TLS','DefaultMediaColor',DefaultMediaColor);
    DefaultTextColor := ini.ReadInteger('TLS','DefaultTextColor',DefaultTextColor);
    StartColorCursor := ini.ReadInteger('TLS','StartColorCursor',StartColorCursor);
    EndColorCursor := ini.ReadInteger('TLS','EndColorCursor',EndColorCursor);
    ...
    TLMaxFrameSize := ini.ReadInteger('TLS','TLMaxFrameSize',TLMaxFrameSize);
    TLPreroll := ini.ReadInteger('TLS','TLPreroll',TLPreroll);
    TLPostroll := ini.ReadInteger('TLS','TLPostroll',TLPostroll);
    TLFlashDuration := ini.ReadInteger('TLS','TLFlashDuration',TLFlashDuration);
    //TLFontColor := ini.ReadInteger('TLS','TLFontColor',TLFontColor);
    //Основные параметры кнопок
    ProgBTNSFontName := ini.ReadString('BUTTONS','ProgBTNSFontName',ProgBTNSFontName);
    ProgBTNSFontColor := ini.ReadInteger('BUTTONS','ProgBTNSFontColor',ProgBTNSFontColor);
    ProgBTNSFontSize := ini.ReadInteger('BUTTONS','ProgBTNSFontSize',ProgBTNSFontSize);
    HintBTNSFontName := ini.ReadString('BUTTONS','HintBTNSFontName',HintBTNSFontName);
    HintBTNSFontColor := ini.ReadInteger('BUTTONS','HintBTNSFontColor',HintBTNSFontColor);
    HintBTNSFontSize := ini.ReadInteger('BUTTONS','HintBTNSFontSize',HintBTNSFontSize);

    // Цвета по умолчанию для событий
    For i:=0 to 31 do DefaultColors[i]:=ini.ReadInteger('DefaultColors','Dev' + inttostr(i+1),DefaultColors[i]);
    DefaultMediaColor:=ini.ReadInteger('DefaultColors','DefaultMediaColor',DefaultMediaColor);
    DefaultTextColor:=ini.ReadInteger('DefaultColors','DefaultTextColor',DefaultTextColor);
    ...
    PrintSpaceLeft      := ini.ReadFloat('PRINT','PrintSpaceLeft',PrintSpaceLeft);
    PrintSpaceRight     := ini.ReadFloat('PRINT','PrintSpaceRight',PrintSpaceRight);
    PrintSpaceTop       := ini.ReadFloat('PRINT','PrintSpaceTop',PrintSpaceTop);
    PrintSpaceBottom    := ini.ReadFloat('PRINT','PrintSpaceBottom',PrintSpaceBottom);
    PrintHeadLineTop    := ini.ReadFloat('PRINT','PrintHeadLineTop',PrintHeadLineTop);
    PrintHeadLineBottom := ini.ReadFloat('PRINT','PrintHeadLineBottom',PrintHeadLineBottom);
    ...
    PrintDeviceName     := ini.ReadString('PRINT','PrintDeviceName',PrintDeviceName);
    ...
    WriteLog('MAIN', 'Окончание чтения IniFile: ' + AppPath + AppName + '.ini');
    ...
    on E: Exception do WriteLog('MAIN', 'ReadMyIniFile(' + AppPath + AppName + '.ini) | ' + E.Message);
  end;
end;
