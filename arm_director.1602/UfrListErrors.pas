...
type
  ...
  TFrListErrors = class(TForm)
    ...
    procedure ImgUpDownMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure ImgUpDownMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure FormCreate(Sender: TObject);
    procedure imgBtnMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
...
begin
 // for i:=0 to 99 do ListErrors.Add('Строка ' + inttostr(i+1));
  ...
  if ScrollUpDown=nil then ScrollUpDown := TMyUpDown.Create(FrListErrors.ImgUpDown.Canvas);
  ...
  if FrListErrors.ModalResult=mrOk then begin
...
begin
  ...
  try
    ...
    if ScrollUpDown.RowCount-1<=ScrollUpDown.VisualRowCount then begin
       sz:=ScrollUpDown.Rowcount-1;
       ScrollUpDown.Position:=0;
    end else begin
      sz := ScrollUpDown.Position+ScrollUpDown.VisualRowCount+1;
      if sz>ScrollUpDown.Rowcount-1 then begin
        sz:=ScrollUpDown.Rowcount-1;
    ...
    ps := cv.ClipRect.Top;
    for i:=ScrollUpDown.Position to sz do begin
...
var sz, shght, AHGHT, szlift, dlt : integer;
    step : Real;
begin
  AHGHT := RowHeight * RowCount;
  shght := rtdown.Bottom - rtup.Top;
  sz := rtdown.Top-rtup.Bottom+2;
  if shght mod RowHeight = 0
    then VisualRowCount :=shght div RowHeight
    else VisualRowCount := trunc(shght / RowHeight)+1;
  if RowCount<=VisualRowCount then begin
    rtlift.Top:=rtup.Bottom+1;
    rtlift.Bottom:=rtdown.Top-1;
    exit;
  end;
  ...
  if OldPos=Y then begin
    if rtlift.Top+szlift>rtdown.Top-1 then begin
      rtlift.Bottom:=rtdown.Top-1;
  ...
  rtlift.Bottom:=rtlift.Top+szlift;
  if rtlift.Top <rtup.Bottom+1 then begin
    rtlift.Top:=rtup.Bottom+1;
    rtlift.Bottom:=rtlift.Top+szlift;
  end;
  if rtlift.Bottom > rtdown.Top-1 then begin
    rtlift.Bottom:=rtdown.Top-1;
    rtlift.Top:=rtlift.Bottom-szlift;
  end;
  ...
  if Position>=Rowcount-visualrowcount then Position:=Rowcount-visualrowcount;
...
begin
  ...
  if posi<VisualRowCount then begin
    rtlift.Top:=rtup.Bottom+1;
    rtlift.Bottom:=rtlift.Top+szlift;
    Position := 0;
  end else begin
    if posi+VisualRowCount>RowCount-1
      then dlt:=RowCount-VisualRowCount+1
      else dlt:=posi-VisualRowCount+1;
...
begin
  ...
  try
    ...
    if select1
      then tmp.SetBrush(bs_solid,0,colortorgb(smoothcolor(FormsColor,64)))
...
begin
  result := -1;
  if (X>rtlift.Left) and (X<rtlift.Right) and (Y>rtlift.Top) and (Y<rtlift.Bottom) and (Not Down)
  then begin
    Down:=true;
    OldPos:=Y;
    //liftposition(Y);
...
begin
  ...
  if (X>rtup.Left) and (X<rtup.Right) and (Y>rtup.Top) and (Y<rtup.Bottom) then begin
    select1:=true;
    select2:=false;
    exit;
  end;
  if (X>rtdown.Left) and (X<rtdown.Right) and (Y>rtdown.Top) and (Y<rtdown.Bottom) then begin
    select1:=false;
    select2:=true;
    exit;
  end;
...
begin
  result := Position;
  if (X>rtup.Left) and (X<rtup.Right) and (Y>rtup.Bottom+1) and (Y<rtdown.Top-1)
  then begin
    liftposition(Y);
    result := Position;
  end;
...
procedure TFrListErrors.imgBtnMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
...
begin
  ...
  if res=0 then FrListErrors.Close;
end;
