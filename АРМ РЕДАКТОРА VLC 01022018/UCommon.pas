...
begin
  ...
  if fileexists(fl) then begin
    ...
    for i:=0 to ListEditedProjects.Count-1
      do Form1.ListBox2.Items.Add(extractfilename(ListEditedProjects.Strings[i]));
  ...
  for i := 1 to Form1.GridClips.RowCount-1 do begin
    if Form1.GridClips.Objects[0,i] is TGridRows then begin
       ...
       if clpid=trim(ClipID) then begin
         (Form1.GridClips.Objects[0,i] as TGridRows).MyCells[3].SetPhraseColor('Clip', PhrasePlayColor);
         (Form1.GridClips.Objects[0,i] as TGridRows).MyCells[2].Mark:=false;
       end else begin
         (Form1.GridClips.Objects[0,i] as TGridRows).MyCells[3].SetPhraseColor('Clip', GridFontColor);
