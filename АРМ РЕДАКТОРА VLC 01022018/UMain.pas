...
var curpos, delttm, mycpos1, mycpos2 : longint;
   ...
   If MyTimer.Enable then begin
     ...
     if not fileexists(Form1.lbPlayerFile.Caption) then begin
       ...
       if MyDoubleToFrame(pStart1)*40 > db2 then begin
         ...
         WriteLog('TCPlayer', '.      1) DBO=' + floattostr(db0));
       end else begin
         if db1 < db2 then begin
           db0:=db1;
           PredDT:=MyTimer.ReadTimer;
           WriteLog('TCPlayer','.      2) DBO=' + floattostr(db0));
         end else begin
           Rate:=libvlc_media_player_get_rate(vlcplayer.p_mi);;
           db0 :=db2 + MyDoubleToFrame((MyTimer.ReadTimer - PredDt) * Rate)*40;//SpeedMultiple;
           WriteLog('TCPlayer', '.      3) DBO=' + floattostr(db0));
         end;
       end;
       if db0>db3 then db0:=db3;
       WriteLog('TCPlayer', '      4) DBO=' + floattostr(db0));
       mycpos2 := TLParameters.Preroll + (db0 div 40);
       if vlcmode=play then begin
         if mycpos2<mycpos1 then mycpos2:=mycpos1;
         if mycpos2>mycpos1+1 then mycpos2:=mycpos1+1
       end;
       TLParameters.Position:=mycpos2;//TLParameters.Preroll + (db0 div 40);
       WriteLog('TCPlayer', 'end)      DBO=' + floattostr(db0));
       WriteLog('TCPlayer', '3) DrawTimelines Position=' + inttostr(TLParameters.Position));
     end;
...
//     if OLDTCPosition<TLParameters.Position then begin
     //if (db0-OldTCTime > 0.02) then begin
       ...
       //application.ProcessMessages;
       WriteLog('TCPlayer', '4) DrawTimelines Position=' + inttostr(TLParameters.Position));
     ...
     //if OLDTCPosition<>TLParameters.Position then begin
       ...
       //dbld1 := MyTimer.ReadTimer; //=======
       WriteLog('TCPlayer', '5****) DrawTimelines Position=' + inttostr(TLParameters.Position));
...
//     OLDTCPosition:=TLParameters.Position;
     ...
     if not fileexists(Form1.lbPlayerFile.Caption) then begin
       if TLParameters.Position >= TLparameters.Finish then begin
         ...
         MediaPause;
         SetMediaButtons;
         WriteLog('TCPlayer', '6) DrawTimelines Position=' + inttostr(TLParameters.Position));
         //MyStartReady:=false;
         //Form1.MySynhro.Checked := false;
         TLZone.DrawTimelines(Form1.imgtimelines.Canvas,bmptimeline);
         exit;
...
begin
  if (ListBox2.ItemIndex>=0) and (ListBox2.ItemIndex<ListBox2.Count) then begin
    //ReadProjectFromFile(ListEditedProjects.Strings[ListBox2.ItemIndex]);
    FileNameProject:=ListEditedProjects.Strings[ListBox2.ItemIndex];
    ReadProjectFromFile(ListEditedProjects.Strings[ListBox2.ItemIndex]);
...
begin
  ...
  res:=ClickMainMenu(Y);
     case res of
  0 : ButtonsControlProjects(0);
  1 : ButtonsControlProjects(1);
  2 : ButtonsControlProjects(2);
  3 : ButtonsControlProjects(3);
  4 : ButtonsControlProjects(4);
  5 : ButtonsControlProjects(5);
...
////        end;  12  : begin //'1|12|Создать новую тайм-линию|CTRL+PLUS'          ButtonControlLists(0);
  ...
  17  : begin //'1|17|Сохранить изменения сделанные в окне проекта|CTRL+S'
          if vlcmode<>play then ButtonsControlProjects(2);
...
begin
  ...
  then pnMainMenu.Visible:=false
    ...
    if not pnMainMenu.Visible then CreateMainMenu(imgMainMenu,-1);
    pnMainMenu.Visible:=True;
  end;
