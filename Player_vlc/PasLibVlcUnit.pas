...
procedure libvlc_dynamic_dll_init_with_path(vlc_install_path: ansistring);
 ...
 * id, name (description ansistring) and pointer to next record.
 *)
type
...
type
  ...
  libvlc_audio_output_device_t = record
    ...
    psz_device      : PAnsiChar;                        // Device identifier ansistring
 ...
 * thread. The resulting ansistring is valid until another error occurs (at least
 ...
 * param fmt the format ansistring
 * param ap the arguments
 * return  a nul terminated ansistring in any case
 *)
...
(**
 * Sets the application name. LibVLC passes this as the user agent ansistring
 ...
 *
 * return  a ansistring containing the libvlc version
 *)
 ...
 *
 * return  a ansistring containing the libvlc compiler version
 *)
 ...
 * Example: "aa9bce0bc4"
 *
 * return  a ansistring containing the libvlc changeset
 *)
 ...
 * param fmt printf() format ansistring (as defined by ISO C11)
 * param args variable argument list for the format
 * note Log message handlers MUST be thread-safe.
 * warning The message context pointer, the format ansistring parameters and the
 *          variable arguments are only valid until the callback returns.
 *)

(* fmt format ansistrings

http://docwiki.embarcadero.com/RADStudio/Berlin/en/Format_Specifiers_in_C/C%2B%2B

Format Specifiers in C/C++

d, i - Decimal or integer. The argument must be an integer value. The value is converted to a ansistring of decimal digits.
       If the format ansistring contains a precision specifier, it indicates that the resulting ansistring must contain at least
       the specified number of digits; if the value has less digits, the resulting ansistring is left-padded with zeros.
u    - Unsigned int. Similar to "d" and "i", but it has no sign.
o    - Octal. The argument must be an integer value. The value is converted to a ansistring of octal digits.
       If the format ansistring contains a precision specifier, it indicates that the resulting ansistring must contain at least
       the specified number of digits; if the value has less digits, the resulting ansistring is left-padded with zeros.
x,X  - Hexadecimal. The argument must be an integer value. The value is converted to a ansistring of decimal digits.
       If the format ansistring contains a precision specifier, it indicates that the resulting ansistring must contain at least
       the specified number of digits; if the value has less digits, the resulting ansistring is left-padded with zeros.
f    - Floating point. The argument must be a floating-point value. The value is converted to a ansistring of the form "-ddd.ddd...".
       The resulting ansistring starts with a minus sign if the number is negative. The number of digits after the decimal point is given by
       the precision specifier in the format ansistring; a default of 2 decimal digits is assumed if no precision specifier is present.
e    - Scientific. The argument must be a floating-point value. The value is converted to a ansistring of the form "-d.ddd...E+ddd".
       The resulting ansistring starts with a minus sign if the number is negative. One digit always precedes the decimal point.
       The total number of digits in the resulting ansistring (including the one before the decimal point) is given by the precision specifier
       in the format ansistring; a default precision of 15 is assumed if no precision specifier is present. The "E" exponent character
       in the resulting ansistring is always followed by a plus or minus sign and at least 3 digits.
g    - Double. The argument must be a floating-point value. The value is converted to the shortest possible decimal ansistring using fixed or scientific format.
       The number of significant digits in the resulting ansistring is given by the precision specifier in the format ansistring;
       a default precision of 15 is assumed if no precision specifier is present. Trailing zeros are removed from the resulting ansistring, and a decimal point
       appears only if necessary. The resulting ansistring uses fixed point format if the number of digits to the left of the decimal point in the value is less
       than or equal to the specified precision, and if the value is greater than or equal to 0.00001. Otherwise the resulting ansistring uses scientific format.
c    - Character. The argument must be a single character value.
s    - ansistring. The argument must be a character, a ansistring, or a char* value. The ansistring or character is inserted in place of the format specifier.
       The precision specifier, if present in the format ansistring, specifies the maximum length of the resulting ansistring. If the argument is a ansistring
       that is longer than this maximum, the ansistring is truncated.
 ...
 * param psz_options the options (as a ansistring)
 *
 *)
 ...
 * return  ansistring with mrl of media descriptor object
 *)
 ...
 *
 * param mp the media player
 * param chroma a four-characters ansistring identifying the chroma
 *              (e.g. "RV32" or "YUYV")
 ...
 *
 * param mp the media player
 * param format a four-characters ansistring identifying the sample format
 *               (e.g. "S16N" or "FL32")
...
(**
 * Get a ansistring marquee option value
 *
 * param p_mi libvlc media player
...
(**
 * Set a marquee ansistring option
 *
 * param p_mi libvlc media player
...
function libvlc_media_record_str(fileName : WideString; mux : ansistring = 'mp4';
  vCodec : ansistring = 'h264'; vBitRate : Integer = 2048; vFps : Integer = 25; vScale : Integer = 1;
  aCodec : ansistring = 'mp3';  aBitRate : Integer = 128; aChannels : Integer = 2; aSampleRate : Integer = 44100;
...
function DirectoryExists(fileName : ansistring) : Boolean;
var
  sr : TSearchRec;
  re : Integer;
begin
...
{$HINTS OFF}
function libvlc_get_install_path() : ansistring;
...
{$IFDEF MACOS}
const
  pathLst : array[0..2] of ansistring = (
...
function libvlc_get_install_file(libvlc_install_path : ansistring) : ansistring;
{$IFDEF MSWINDOWS}
begin
  // look for libvlc.dll
...
procedure require_version(function_name, require_version: ansistring);
begin
  if IsConsole then
  begin
...
function libvlc_dynamic_dll_architecture(file_path_name : ansistring) : Integer;
{$IFDEF MSWINDOWS}
const
...
var
 vlcdir :ansistring;
begin
  vlcdir:=libvlc_get_install_path();
  vlcdir:=extractfilepath(paramstr(0))+'\vlcdll\';
  if (libvlc_handle <> 0) then exit;
  libvlc_dynamic_dll_init_with_path(vlcdir);
end;
