...
begin
  ...
  if comnt
    then FrSetTemplate.Caption:='Текстовые шаблоны (Комментарий)'
    else FrSetTemplate.Caption:='Текстовые шаблоны (Основной текст)';
  ...
  if FrSetTemplate.ModalResult = mrOk then begin
    if FrSetTemplate.checkbox1.Checked
      then result:='#' + FrSetTemplate.Edit1.Text
      else result:=FrSetTemplate.Edit1.Text;
  end;
end;

procedure TFrSetTemplate.GridMyListsDrawCell(Sender: TObject; ACol,
...
begin
  ...
  for i:=0 to GridMyLists.RowCount-1 do (GridMyLists.Objects[0,i] as TGridRows).MyCells[0].Mark:=false;
  Edit1.Text := (GridMyLists.Objects[0,rw] as TGridRows).MyCells[1].ReadPhrase('Template');
  (GridMyLists.Objects[0,rw] as TGridRows).MyCells[0].Mark:=true;
  if DblClickMyLists then begin
    if rw > 0
      then if (GridMyLists.Objects[0,rw] is TGridRows)
