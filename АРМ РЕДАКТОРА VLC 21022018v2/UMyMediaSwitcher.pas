...
Var MyMediaSwitcher : TMyMediaSwitcher;
...
var rt : trect;
    ...
    clrs : tcolor;
begin
  if (ColorSelect>=smoothColor(ProgrammColor,-8)) and (ColorSelect<=smoothColor(ProgrammColor,8))
    then clrs := smoothColor(ProgrammColor, 48) else clrs:=ColorSelect;

  Rect1.Left    := 0;
  Rect1.Right   := Rect1.Left + Width;
  Rect1.Top     := (cv.ClipRect.Bottom-cv.ClipRect.Top - 2*Height -interval) div 2;
  Rect1.Bottom  := Rect1.Top + Height;
  Rect2.Left    := 0;
  Rect2.Right   := Rect2.Left + Width;
  Rect2.Top     := Rect1.Bottom + Interval;
  ...
  if Select1 then cv.Brush.Color:=SmoothColor(ProgrammColor,24) else cv.Brush.Color:=ProgrammColor;
  ...
  if Select = 0 then cv.Brush.Color:=clrs else cv.Brush.Color:=ProgrammColor;
  cv.Pen.Width:=1;
  cv.Ellipse(rt);
  cv.Pen.Width:=2;
  ...
  cv.Brush.Style:=bsSolid;
  if Select2 then cv.Brush.Color:=SmoothColor(ProgrammColor,24) else cv.Brush.Color:=ProgrammColor;
  ...
  if Select = 1 then cv.Brush.Color:=clrs else cv.Brush.Color:=ProgrammColor;
  cv.Pen.Width:=1;
  cv.Ellipse(rt);
  cv.Pen.Width:=2;
...
begin
  if (X >= Rect1.Left) and  (X <= Rect1.Right) and (Y >= Rect1.Top) and (Y <= Rect1.Bottom)
    then Select1 := true else Select1 := false;
  if (X >= Rect2.Left) and  (X <= Rect2.Right) and (Y >= Rect2.Top) and (Y <= Rect2.Bottom)
    then Select2 := true else Select2 := false;
end;
