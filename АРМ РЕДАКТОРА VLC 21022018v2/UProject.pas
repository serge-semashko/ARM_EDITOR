...
begin
  ...
  if mode<0 then begin
    //if (trim(ProjectNumber)<>'') and IsProjectChanges then begin
    ...
    FNewProject.Edit1.Text:=pntlproj.GetText('ProjectName'); //Form1.lbProjectName.Caption;
    FNewProject.Memo1.Text:=pntlproj.GetText('CommentText'); //Form1.lbpComment.Caption;
    if trim(pntlproj.GetText('DateEnd'))=''
      then FNewProject.DateTimePicker1.DateTime:=now + DeltaDateDelete
      else FNewProject.DateTimePicker1.DateTime:=strtodate(pntlproj.GetText('DateEnd'));
  end;
