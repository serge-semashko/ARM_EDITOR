...
begin
  ...
  cv.TextOut(CurW+wdth + 5 * MaketOptions.mmHorz,CurH,pntlprep.GetText('ClipName'));
  ...
  CurH:=CurH + hght + 3 * MaketOptions.mmVert;
  hght := cv.TextHeight(pntlprep.GetText('SingerName'));// Form1.lbClipSinger.Caption);
...
begin
  ...
  if frMyPrint.cbDataPrint.ItemIndex=0
    then cv.TextOut(MaketOptions.X5,hght,PrintCol61)
    else cv.TextOut(MaketOptions.X5,hght,PrintCol62);
end;
...
begin
  ...
  for i:=1 to cnt do cbTypePrint.Items.Add(PrintDeviceName + ' ' + inttostr(i));
