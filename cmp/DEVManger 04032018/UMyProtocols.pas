...
type
  ...
  TOneField = class
    TypeField  : string; //byte, hbyte, lbyte, bit0, bit1, bit2, bit3, bit4, bit5, bit6, bit7
  ...
  TListTypeDevices = class
    ...
    procedure clear;
    procedure GetProtocol(TypeDevice,Vendor,Device,Prot : string; Protocol : TOneProtocol);
    constructor create;
    destructor destroy;
  end;

Procedure LoadProtocol(FileName,TypeTL,TypeDevice,Vendor,Device,Protocol : string);

Var ListTypeDevices : TListTypeDevices;
...
begin
  try
    ...
    ListTypeDevices.GetProtocol(TypeDevice, Vendor, Device, Protocol, MyProtocol);
  finally
...
var sindx, sphr, stph, stmp : string;
    indx : integer;
begin
  result := '';
  sindx := Index;
  ...
  if Source='value' then result:= Param
    ...
    else if ansilowercase(trim(param))='value' then result:=MyProtocol.Options.List[indx-1].Text;
  ...
  WriteLog('Translator','GetCommandValue: Source='+Source+' Param=' + Param
           + ' Phrase=' + Phrase + 'TypePhrase=' + TypePhrase + ' Index' + Index
           + 'Result=' + Result);
end;
...
begin
  clear;
  for i:=0 to TOB.Count-1 do begin
    Count:=Count+1;
    setlength(Fields,Count);
    ...
    if Fields[Count-1].Index<>nil then begin
      Fields[Count-1].Index.Freeinstance;
      Fields[Count-1].Index := nil;
    end;
    if TOB.Fields[i].Index<>nil then begin
...
begin
  ...
  for i:=0 to Count-1 do begin
    if Fields[i].Index=nil then sindx:='' else sindx:=Fields[i].Index.GetStr(evnt);
    btstr := GetCommandValue(evnt, Fields[i].TypeData, Fields[i].Param, Fields[i].Phrase, Fields[i].TypePhrase, sindx);

     WriteLog('Translator','TOneByte.GetValue Field=' + inttostr(i) + ' TypeField='
              + Fields[i].TypeField + ' VarData=' + Fields[i].VarData);

    if trim(Fields[i].VarData)<>'' then btstr := GetDigit(btstr, Fields[i].VarData);
    if trim(btstr)='' then btstr:='00';


    if Fields[i].TypeField='byte' then begin
      if length(trim(btstr))=1 then result:='0'+trim(btstr) else result := btstr;
    end else if Fields[i].TypeField='hbyte' then begin
      result[1]:=btstr[length(btstr)];
    end else if Fields[i].TypeField='lbyte' then begin
      result[2]:=btstr[length(btstr)];
    end else if Fields[i].TypeField='bit0' then begin
      bt:=StrToByte(result);
      res := MyStrToInt(btstr);
      if res>0 then bt:=bt or $01 else bt:=bt and $FE;
      result:=inttohex(bt);
    end else if Fields[i].TypeField='bit1' then begin
      bt:=StrToByte(result);
      res := MyStrToInt(btstr);
      if res>0 then bt:=bt or $02 else bt:=bt and $FD;
      result:=inttohex(bt);
    end else if Fields[i].TypeField='bit2' then begin
      bt:=StrToByte(result);
      res := MyStrToInt(btstr);
      if res>0 then bt:=bt or $04 else bt:=bt and $FB;
      result:=inttohex(bt);
    end else if Fields[i].TypeField='bit3' then begin
      bt:=StrToByte(result);
      res := MyStrToInt(btstr);
      if res>0 then bt:=bt or $08 else bt:=bt and $F7;
      result:=inttohex(bt);
    end else if Fields[i].TypeField='bit4' then begin
      bt:=StrToByte(result);
      res := MyStrToInt(btstr);
      if res>0 then bt:=bt or $10 else bt:=bt and $EF;
      result:=inttohex(bt);
    end else if Fields[i].TypeField='bit5' then begin
      bt:=StrToByte(result);
      res := MyStrToInt(btstr);
      if res>0 then bt:=bt or $20 else bt:=bt and $DF;
      result:=inttohex(bt);
    end else if Fields[i].TypeField='bit6' then begin
      bt:=StrToByte(result);
      res := MyStrToInt(btstr);
      if res>0 then bt:=bt or $40 else bt:=bt and $BF;
      result:=inttohex(bt);
    end else if Fields[i].TypeField='bit7' then begin
      bt:=StrToByte(result);
      res := MyStrToInt(btstr);
      if res>0 then bt:=bt or $80 else bt:=bt and $7F;
      result:=inttohex(bt);
    end;
  end;
...
begin
  clear;
  for i:=0 to TIP.Count-1 do begin
    Count := Count + 1;
    setlength(List,Count);
    List[Count-1] := TOneStringTable.create(TIP.List[i].Name,TIP.List[i].Text);
  end;
end;
...
begin
  clear;
  Count:=0;
  sparam := StringReplace(stri,'<'+trim(lstname)+'>','',[rfReplaceAll, rfIgnoreCase]);
  sparam := StringReplace(sparam,'</'+trim(lstname)+'>','',[rfReplaceAll, rfIgnoreCase]);
...
begin
  result := '';
  for i:=0 to Count-1 do begin
    if ansilowercase(trim(list[i].Name))=ansilowercase(trim(SName)) then begin
      result := list[i].Text;
      exit;
    end;
  end;
...
begin
  result:='';
  if (trim(TypeData)='0') or (trim(TypeData)='1')
    then for i:=0 to Count-1 do result:=result + Bytes[i].GetValue(evnt)
    else result:=GetCommand(evnt); //&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
end;
...
begin
  ...
  Name := TOC.Name;
  Command := TOC.Command;
  for i:=0 to TOC.Count-1 do begin
    Count:=Count+1;
    setlength(Bytes,count);
...
var sparam, scount, stxt, stmp, sprm, slnk : string;
...
begin
  clear;
  Count:=0;
  sparam := StringReplace(stri,'<Command='+trim(Name)+'>','',[rfReplaceAll, rfIgnoreCase]);
  sparam := StringReplace(sparam,'</Command='+trim(Name)+'>','',[rfReplaceAll, rfIgnoreCase]);
  if trim(TypeData)='2' then begin
    Command:=sparam;
    exit;
  end;
  ...
  if trim(scount)<>'' then begin
    ...
    for i:=0 to cnt-1 do begin
      ...
      for j:=0 to alst.Count-1 do begin
        ...
        for ic:=0 to 4 do cmd[ic]:='';
        if lst.Count>5 then sind:=5 else sind := lst.Count;
        for ic:=0 to sind-1 do cmd[ic]:=trim(lst.Strings[ic]);
          Bytes[rwbts].Fields[rwfld].TypeField:=ansilowercase(cmd[0]);
          Bytes[rwbts].Fields[rwfld].TypeData:=ansilowercase(cmd[1]);
          if Bytes[rwbts].Fields[rwfld].TypeData='phrase' then begin
            res := PhraseParam(cmd[2]);
            Bytes[rwbts].Fields[rwfld].Phrase:=res.Name;
            Bytes[rwbts].Fields[rwfld].TypePhrase:=res.Text;
          end else Bytes[rwbts].Fields[rwfld].Param:=trim(cmd[2]);

          slnk := cmd[3];
          if slnk<>'' then begin
            if slnk[1]='[' then begin
               slnk := copy(slnk,2,length(slnk));
               if slnk[length(slnk)]=']' then slnk := copy(slnk,1,length(slnk)-1);
               if Bytes[rwbts].Fields[rwfld].Index <> nil then begin
                 Bytes[rwbts].Fields[rwfld].Index.FreeInstance;
                 Bytes[rwbts].Fields[rwfld].Index := nil;
               end;
               Bytes[rwbts].Fields[rwfld].Index := TMyIndex.create(slnk);
            end else begin
              slnk:=ansilowercase(slnk);
              if (slnk='thousands') or (slnk='hundres') or (slnk='tens') or (slnk='ones')
                then Bytes[rwbts].Fields[rwfld].VarData:=slnk;
            end;
          end;

          if cmd[4]<>'' then Bytes[rwbts].Fields[rwfld].VarData:=cmd[4];
...
var i : integer;
begin
  for i:=CMDCount-1 downto 0 do begin
  ...
  try
    ...
    if lst.Count>0 then begin
      for i:=0 to lst.Count-1 do begin
        ...
        sprotocols := GetProtocolsStr(stri, 'Command='+trim(lst.Strings[i]));
...
var i : integer;
begin
  WriteLog('Translator','TCommandTemplates.GetCommand  Start Command=' + cmd);
  for i:=0 to CMDCount-1 do begin
    if ansilowercase(trim(CommandsList[i].Name))=ansilowercase(trim(cmd)) then begin
      result := CommandsList[i].GetValue(evnt, TypeData);
      WriteLog('Translator','TCommandTemplates.GetCommand  Command=' + cmd + ' Result=' + result);
      exit;
    end;
  end;
...
begin
  ...
  try
    ...
    lst.Clear;
    for i:=0 to clst.Count-1 do begin
...
begin
  ...
  for i:=0 to TCT.CMDCount-1 do begin
    CMDCount := CMDCount + 1;
    setlength(CommandsList,CMDCount);
    CommandsList[CMDCount-1] := TOneCommand.create;
...
begin
  ...
  try
    ...
    for i:=0 to 3 do cmd[i]:='';
    if lst.Count>4 then cnt:=4 else cnt:=lst.Count;
    for i:=0 to lst.Count-1 do cmd[i]:=lst.Strings[i];
    Source := ansilowercase(trim(cmd[0]));
    if Source='phrase' then begin
      res:=PhraseParam(cmd[1]);
      Phrase     := res.Name;
      TypePhrase := res.Text;
    end else Param:=trim(cmd[1]);
    ss := trim(cmd[2]);
    if ss<>'' then begin
      if ss[1]='[' then begin
        ss:=copy(ss,2,length(ss));
        if ss[length(ss)]=']' then ss:=copy(ss,1,length(ss)-1);
...
begin
  result := '';
  if index<>nil then sindx := Index.GetStr(evnt) else sindx:='';
...
begin
  ...
  freemem(@Phrase);
  freemem(@TypePhrase);
  if Index<>nil then index.Freeinstance;
  freemem(@Index);
end;
...
var sindx : string;
begin
  if Index=nil then sindx:='' else sindx:=Index.GetStr(evnt);
...
begin
  result := -1;
  for i:=0 to Count-1 do begin
    if ansilowercase(trim(TypeDevices[i].TypeDevice))=ansilowercase(trim(Name)) then begin
      result:=i;
      exit;
    end;
  end;
...
begin
  for i:=0 to Count-1 do begin
    result := setspace(2) + '<TypeDevices=' + trim(TypeDevices[i].TypeDevice) + '>' + #13#10;
    result := result + TypeDevices[i].GetString;
    result := result + setspace(2) + '</TypeDevices=' + trim(TypeDevices[i].TypeDevice) + '>' + #13#10;
  end;
end;
...
begin
  for i:=0 to Count-1 do begin
    lst.Add(setspace(2) + '<TypeDevices=' + trim(TypeDevices[i].TypeDevice) + '>');
...
begin
  ...
  try
    ...
    for i:=0 to lst.Count-1 do begin
     ...
     sprotocols := GetProtocolsStr(SrcStr, 'TypeDevices='+trim(lst.Strings[i]));
...
procedure TListTypeDevices.GetProtocol(TypeDevice,Vendor,Device,Prot : string; Protocol : TOneProtocol);
var i, j, n, m : integer;
begin
  for i:=0 to Count-1 do begin
    if ansilowercase(trim(TypeDevices[i].TypeDevice))=ansilowercase(trim(TypeDevice)) then begin
      for j:=0 to TypeDevices[i].Count-1 do begin
        if ansilowercase(trim(TypeDevices[i].Vendors[j].Vendor))=ansilowercase(trim(Vendor)) then begin
          with TypeDevices[i].Vendors[j] do begin
            for n :=0 to Count-1 do begin
              if ansilowercase(trim(FirmDevices[n].Device))=ansilowercase(trim(Device)) then begin
                for m:=0 to FirmDevices[n].Count-1 do begin
                  if ansilowercase(trim(FirmDevices[i].ListProtocols[m].Protocol))=ansilowercase(trim(Prot)) then begin
                    Protocol.AssignPart(FirmDevices[i].ListProtocols[m]);
                    exit;
                  end;
                end;
              end;
...
begin
  ...
  if not FileExists(FileName) then exit;
    ...
    for i:=0 to buff.Count-1 do ss:=ss + trim(buff.Strings[i]);
...
begin
  ...
  try
    ...
    GetListString(buff);
    buff.Add('</' + TypeDevices + '>');
    if ansilowercase(TypeDevices)='tldevices' then buff.Text:=buff.Text + stxt + smedia
    else if ansilowercase(TypeDevices)='tltext' then buff.Text:=sdev + buff.Text + smedia
    else if ansilowercase(TypeDevices)='tlmedia' then buff.Text:=sdev + stxt + buff.Text;
...
begin
  result := -1;
  for i:=0 to Count-1 do begin
    if ansilowercase(trim(ListProtocols[i].Protocol))=ansilowercase(trim(Name)) then begin
      result:=i;
      exit;
    end;
  end;
...
begin
  for i:=0 to Count-1 do begin
    result:=setspace(8)+'<Protocol=' + ListProtocols[i].Protocol + '>' + #13#10;
...
begin
  result := -1;
  for i:=0 to Count-1 do begin
    if ansilowercase(trim(Vendors[i].Vendor))=ansilowercase(trim(Name)) then begin
      result:=i;
      exit;
    end;
  end;
...
begin
  for i:=0 to Count-1 do begin
    ...
    result := result + setspace(4) + '</Firms=' + trim(Vendors[i].Vendor) + '>' + #13#10;
  end;
end;
...
begin
  ...
  if Condition.Index<>nil then begin
...
begin
  clear;
  CMDType:=TC.CMDType;
  if Condition=nil then Condition.create;
  Condition.Assign(TC.Condition);
  for i:=0 to TC.Count-1 do begin
    count := count + 1;
...
var i, j : integer;
    scond, sindx : string;
begin
  WriteLog('Translator','TCommands.GetListCommands  Start CMDType=' + inttostr(CMDType));
  lst.Clear;
  if CMDType=0 then begin
    if Count=0 then exit;
    //caseitems[0].List.Clear;
    for i:=0 to CaseItems[0].List.Count-1 do begin
      lst.Add(caseitems[0].List.Strings[i]);
      WriteLog('Translator','TCommands.GetListCommands  AddCommand=' + caseitems[0].List.Strings[i]);
    end;
  end else begin
    if Condition.Index=nil then sindx:='' else sindx:=Condition.Index.GetStr(evnt);
    scond:=GetCommandValue(evnt, Condition.Source, Condition.Param, Condition.Phrase, Condition.TypePhrase, sindx);
    //scond:='mix';
    WriteLog('Translator','TCommands.GetListCommands  Condition=' + scond);
    for i:=0 to Count-1 do begin
      if ansilowercase(trim(CaseItems[i].CaseName))=ansilowercase(trim(scond)) then begin
        for j:=0 to CaseItems[i].List.Count-1 do begin
          lst.Add(caseitems[i].List.Strings[j]);
          WriteLog('Translator','TCommands.GetListCommands  AddCommand=' + caseitems[i].List.Strings[j]);
        end;
        exit;
      end;
    end;
...
var sparam, scount, stype, ss, s1, s2 : string;
    i, j, k, cnt, sind : integer;
    res, srng : TListParam;
    rng : TRangeParam;
    lst : tstrings;
begin
  clear;
  Count:=0;
  sparam := StringReplace(stri,'<'+trim(SName)+'>','',[rfReplaceAll, rfIgnoreCase]);
  sparam := StringReplace(sparam,'</'+trim(SName)+'>','',[rfReplaceAll, rfIgnoreCase]);
  //sparam := GetProtocolsStr(stri, 'MainOptions');
  if trim(sparam)='' then exit;

  stype := trim(GetProtocolsParam(sparam, 'Type'));
  if ansilowercase(stype)='list' then begin
    CMDType:=0;
    scount := GetProtocolsParam(sparam, 'Count');
    if trim(scount)<>'' then begin
      cnt:=strtoint(scount);
  ...
  end else if ansilowercase(stype)='case' then begin
    CMDType:=1;
...
begin
  result := -1;
  for i:=0 to Count-1 do begin
    if ansilowercase(trim(FirmDevices[i].Device))=ansilowercase(trim(Name)) then begin
      result:=i;
      exit;
    end;
  end;
...
var i : integer;
begin
  if trim(FirmDevices[0].Device)='' then begin
    result:=FirmDevices[0].GetString;
  end else begin
    for i:=0 to Count-1 do begin
      result := setspace(6) + '<Device=' + trim(FirmDevices[i].Device) + '>' + #13#10;
...
begin
  clear;
  Name := TLP.Name;
  for i:=0 to TLP.Count-1 do begin
    count:=Count+1;
    setlength(List,Count);
    List[Count-1] := TOneStringTable.create(TLP.List[i].Name,TLP.List[i].Text);
  end;
end;
...
begin
  clear;
  Count:=0;
  sparam := StringReplace(stri,'<Param='+trim(Name)+'>','',[rfReplaceAll, rfIgnoreCase]);
  sparam := StringReplace(sparam,'</Param='+trim(Name)+'>','',[rfReplaceAll, rfIgnoreCase]);
  //sparam := GetProtocolsStr(stri, 'MainOptions');
  scount := GetProtocolsParam(sparam, 'Count');
  if trim(scount)<>'' then begin
    cnt:=strtoint(scount);
    for i:=0 to cnt-1 do begin
      ...
      if rng.Max<>-1 then begin
        srng := GetIndexStr(res.Text);
        if trim(srng.Text)='' then sind:=0 else sind:=strtoint(srng.Text);
        for j:=rng.Min to rng.Max do begin
          Count:=Count+1;
          setlength(List,Count);
          List[Count-1] := TOneStringTable.create(trim(rng.Text)+inttostr(j), GetParamData(srng.Name,sind));
          sind := sind + 1;
        end;
      end else begin
        Count:=Count+1;
...
begin
  clear;
  for i:=0 to TPP.Count-1 do begin
    Count := Count + 1;
    setlength(Params,Count);
...
begin
  result := '';
  for i:=0 to Count-1 do begin
    if ansilowercase(trim(Params[i].Name))=ansilowercase(trim(PName)) then begin
      for j:=0 to Params[i].Count-1 do begin
        if ansilowercase(trim(Params[i].List[j].Name))=ansilowercase(trim(SName)) then begin
...
begin
  ...
  try
    ...
    if lst.Count>0 then begin
      for i:=0 to lst.Count-1 do begin
        rw := Add(trim(lst.Strings[i]));
        sprotocols := GetProtocolsStr(Stri, 'Param='+trim(lst.Strings[i]));
