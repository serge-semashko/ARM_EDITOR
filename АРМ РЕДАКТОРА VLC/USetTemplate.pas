...
begin
  ...
  if FileExists(PathTemp + '\TextTemplates.lst') then
    LoadGridFromFile(PathTemp + '\TextTemplates.lst', GridMyLists)
  else if FileExists(PathLists + '\TextTemplates.lst') then
    LoadGridFromFile(PathLists + '\TextTemplates.lst', GridMyLists)
  else
    GridMyLists.Enabled := false;
end;
