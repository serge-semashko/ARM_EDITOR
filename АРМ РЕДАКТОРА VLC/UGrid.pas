    ...
    begin
      for i := 0 to grid.RowCount - 1 do
        ...
        begin
          ...
          if makelogging then
            WriteLog('TC', 'MarkRowPhraseInGrid 1=' + MyTimeToStr + ' ' + ev);
          if makelogging then
            WriteLog('MAIN', 'UGrid.MarkRowPhraseInGrid Grid=' + grid.Name +
              ' MarkCell=' + inttostr(MarkCell) + ' WorkCell=' +
              inttostr(WorkCell) + ' Name=' + Name + ' Text' + Text + ' Row=' +
              inttostr(i) + ' Mark:=false' + ' ' + ev);
        end;
      grid.Repaint;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          grid.Row := i;
          if makelogging then
            WriteLog('TC', 'MarkRowPhraseInGrid 2=' + MyTimeToStr + ' ' + ev);
          if makelogging then
            WriteLog('MAIN', 'UGrid.MarkRowPhraseInGrid Grid=' + grid.Name +
              ' MarkCell=' + inttostr(MarkCell) + ' WorkCell=' +
              inttostr(WorkCell) + ' Name=' + Name + ' Text' + Text + ' Row=' +
              inttostr(i) + ' Mark:=true' + ' ' + ev);
        end
        else
        begin
          (grid.Objects[0, i] as TGridRows).MyCells[MarkCell].Mark := false;
          if makelogging then
            WriteLog('TC', 'MarkRowPhraseInGrid 3=' + MyTimeToStr + ' ' + ev);
          if makelogging then
            WriteLog('MAIN', 'UGrid.MarkRowPhraseInGrid Grid=' + grid.Name +
              ' MarkCell=' + inttostr(MarkCell) + ' WorkCell=' +
              inttostr(WorkCell) + ' Name=' + Name + ' Text' + Text + ' Row=' +
