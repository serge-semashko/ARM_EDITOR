...
begin
  ...
  if makelogging then
    WriteLog('MAIN', 'USortGrid..SortMyListClear');
end;
...
begin
  ...
  for i := 0 to 4 do
  begin
    if trim(SortMyList[i].Name) = '' then
      exit;
    FrSortGrid.RadioGroup1.Items.Add(SortMyList[i].Name);
    if makelogging then
      WriteLog('MAIN', 'USortGrid..LoadListRadioButton Name=' +
        SortMyList[i].Name);
  end;
end;
...
begin
  case SortMyList[FrSortGrid.RadioGroup1.ItemIndex].TypeData of
    tstext:
      begin
        SortGridAlphabet(Grid, StartRow, ACol,
          SortMyList[FrSortGrid.RadioGroup1.ItemIndex].Field,
    ...
    tsdate:
      begin
        SortGridDate(Grid, StartRow, ACol,
          SortMyList[FrSortGrid.RadioGroup1.ItemIndex].Field,
          FrSortGrid.Direction);
        if makelogging then
  ...
  try
    if makelogging then
      WriteLog('MAIN', 'USortGrid.GridSort Start Grid=' + Grid.Name +
        ' StartRow=' + inttostr(StartRow) + ' Col=' + inttostr(ACol));
    LoadListRadioButton;
    if FrSortGrid.RadioGroup1.Items.Count > 0 then
      FrSortGrid.RadioGroup1.ItemIndex := 0;
    FrSortGrid.ShowModal;
    if FrSortGrid.ModalResult = mrok then
    begin
      if FrSortGrid.RadioGroup1.ItemIndex < 0 then
        exit;
...
begin
  ...
  if Direction then
    ...
    SpeedButton3.Glyph.Assign(Image2.Picture.Graphic);
  if makelogging then
    WriteLog('MAIN', 'USortGrid.TFrSortGrid.SpeedButton3Click');
end;
...
begin
  if makelogging then
    WriteLog('MAIN',
      'USortGrid.TFrSortGrid.SpeedButton2Click ModalResult := mrcancel');
...
begin
  if makelogging then
    WriteLog('MAIN',
      'USortGrid.TFrSortGrid.SpeedButton1Click ModalResult := mrOk');
  ModalResult := mrok;
end;
  ...
  case Key of
    #13:
      begin
        if makelogging then
          WriteLog('MAIN',
            'USortGrid.TFrSortGrid.FormKeyPress Key=13 ModalResult := mrOk');
        ModalResult := mrok;
      end;
    #27:
      begin
