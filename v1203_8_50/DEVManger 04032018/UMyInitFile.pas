...
function StringParameters(Baud,Bits,StopBit,Parity,Control : Cardinal) : string;
...
begin
  ...
  result:=result + ': Baud: ' + inttostr(Baud)+ '; Bits=' + inttostr(Bits) + '; Stop=';
     case StopBit of
  0 : result := result + '1; Parity=';
  1 : result := result + '1.5; Parity=';
  2 : result := result + '2; Parity=';
     end;
    case Parity of
  0 : result := result + 'Нет; Flow=';
  1 : result := result + 'Нечет.; Flow=';
  2 : result := result + 'Четн.; Flow=';
  3 : result := result + 'Маркер(1); Flow=';
  4 : result := result + 'Пробел(0); Flow=';
     end;
  //result := result + inttohex(Control);

  s:=IntToHex(Control);
  if (Control and $00000300) <> 0 then result := result + 'XOn/XOff'
  else if (Control and $00002004) <> 0 then result := result + 'Аппаратное'
  else if (Control and $00000011) <> 0 then result := result + 'Нет';
end;
...
function BitmapToIcon(const Bitmap: TBitmap; Icon: TIcon; MaskColor: TColor): Boolean;
