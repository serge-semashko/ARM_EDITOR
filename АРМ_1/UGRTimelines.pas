...
begin
  try
    ...
    cv.Brush.Style := bsSolid;
    cv.Brush.Color := evnt.Color;
    // (Form1.GridTimeLines.Objects[0,Index] as TTimelineOptions).TextColor; //TLParameters.ForeGround; //Events[i].Color;
    cv.FillRect(rt);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              TLEditor.EventsSelectFalse;
              TLEditor.Events[i].Select := true;
              // not TLEditor.Events[i].Select;
              Timelines[FindTimeline(TLEditor.IDTimeline)].Events[i].Select :=
                TLEditor.Events[i].Select;
            end;
...
begin
  ...
  try
    ...
    try
      ...
      if makelogging then
        WriteLog('TC', 'DrawTimelines Start=' + MyTimeToStr);
      ...
      if makelogging then
        WriteLog('TC', 'DrawTimelines bitblt=' + MyTimeToStr);

      DrawCursorStart(form1.imgLayer1.Canvas);
      if makelogging then
        WriteLog('TC', 'DrawTimelines DrawCursorStart=' + MyTimeToStr);

      DrawCursorEnd(form1.imgLayer1.Canvas);
      if makelogging then
        WriteLog('TC', 'DrawTimelines DrawCursorEnd=' + MyTimeToStr);
      ...
      // Form1.imgTimelines.Canvas.Lock;
      if makelogging then
        WriteLog('TC', 'DrawTimelines stretchblt start=' + MyTimeToStr);
      ...
      // Form1.imgTimelines.Canvas.UnLock;
      if makelogging then
        WriteLog('TC', 'DrawTimelines stretchblt finish=' + MyTimeToStr);
      ...
      if makelogging then
        WriteLog('TC', 'DrawTimelines Finish=' + MyTimeToStr);
