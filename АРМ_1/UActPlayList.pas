...
begin
  try
    ...
    // (Form1.GridActPlayList.Objects[0,GridPlayerRow] as TGridRows).MyCells[3].ReadPhrase('Song');
    Form1.lbSongSinger.Caption := '';
    // (Form1.GridActPlayList.Objects[0,GridPlayerRow] as TGridRows).MyCells[3].ReadPhrase('Singer');
    ...
    // Trim((Form1.GridActPlayList.Objects[0,GridPlayerRow] as TGridRows).MyCells[3].ReadPhrase('TypeMedia'));
  ...
  try
    ...
    begin
      ...
      begin
        ...
        WriteLog('MAIN', 'UActPlayList.LoadClipsToPlayer Load FileName=' + fn +
          ' Error:' + GraphErrorToStr(err));
        exit;
      end;
...
begin
  ...
  begin
    ...
    if cnt = 0 then
      MyTextMessage('Сообщение', 'В плей-лист не загружен ни один клип.', 1)
    else
      MyTextMessage('Сообщение', 'Выделено клипов ' + inttostr(ttl) +
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        txt := (GridActPlayList.Objects[0, i] as TGridRows).MyCells[3]
          .ReadPhrase('ClipID');
        if not ClipExists(GridClips, txt) then
        begin
          if trim(txt) = trim(lbActiveClipID.Caption) then
            TLZone.ClearZone;
