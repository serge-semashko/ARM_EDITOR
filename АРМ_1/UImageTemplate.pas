...
begin
  try
    ...
    begin
      LoadGridFromFile(PathTemp + '\ImageTemplates.lst',
        FGRTemplate.GridImgTemplate);
...
begin
  try
    if MyTextMessage('', 'Сохранить cделанные изменения?' + #10#13 + #10#13 +
      '[Да]-Сохранить изменения, [Нет]-Выйти без сохранения.', 2) then
      SaveGridToFile(PathTemp + '\ImageTemplates.lst', GridImgTemplate);
    UpdateGridTemplate := true;
