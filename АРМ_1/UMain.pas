...
begin
  try
    ...
    begin
      ...
      if (MyStartPlay <= ftm) and (mode <> play) { and MyStartReady } then
      begin
        Form1.lbTypeTC.Font.Color := ProgrammFontColor;
        MyRemainTime := -1;
        MyShiftOld := MyShift;
    ...
    begin
      ...
      begin
        ...
        begin
          WriteLog('MAIN',
            'TMyThread.DoWork Изменение Тайм-код LTC | Системное время =' +
            TimeToTimeCodeStr(now) + ' Смещение=' + TimeToTimeCodeStr(MyShift));
        end
        else
        begin
          WriteLog('MAIN',
            'TMyThread.DoWork Изменение системного времени | Системное время ='
            + TimeToTimeCodeStr(now));
        end;
      end;
      if (TimeToFrames(msd) >= SynchDelay) and (mode = play) then
      begin
    ...
    begin
      ...
      if MakeLogging then
        WriteLog('TC', 'TimeCode=' + MyTimeToStr);

      if OldTCPosition < TLParameters.Position then
      begin
        // if (db0-OldTCTime > 0.02) then begin
        crpos := TLZone.TLEditor.CurrentEvents;

        if MakeLogging then
          WriteLog('TC', 'CurrentEvents=' + MyTimeToStr);
        ...
        MyPanelAir.SetValues;
        if MakeLogging then
          WriteLog('TC', 'SetValues=' + MyTimeToStr);
        ...
        if MakeLogging then
          WriteLog('TC', 'MarkRowPhraseInGrid=' + MyTimeToStr);
        ...
        if MakeLogging then
          WriteLog('TC', 'DrawFlash=' + MyTimeToStr);
        ...
        // dbld2 := MyTimer.ReadTimer;//========
        if MakeLogging then
          WriteLog('TC', 'DrawTimelines=' + MyTimeToStr);
        ...
        begin
          ...
          begin
            ...
            // dbld2 := MyTimer.ReadTimer;//========
            if MakeLogging then
              WriteLog('TC', 'MyPanelAir=' + MyTimeToStr);
          end;
          MyPanelAir.AirEvents.DrawTimeCode(Form1.imgEvents.Canvas,
            TLParameters.Position - TLParameters.ZeroPoint);
        end;
...
begin
  try
    ...
    pStart1 := 0;
    if FramesToDouble(TLParameters.Position - TLParameters.Preroll) > dur then
...
begin
  try
    ...
    (GridLists.Objects[0, ListBox1.ItemIndex + 1] as TGridRows).MyCells[2]
      .Mark := true;
    lbplcomment.Caption := (GridLists.Objects[0, ListBox1.ItemIndex + 1]
      as TGridRows).MyCells[3].ReadPhrase('Comment');
    if fileexists(PathPlayLists + '\' + FLName) then
    begin
      LoadGridFromFile(PathPlayLists + '\' + FLName, GridActPlayList);
      CheckedActivePlayList;
      WriteLog('MAIN', 'TForm1.ListBox1Click FileName=' + PathPlayLists + '\'
        + FLName);
    end
    else
    begin
      ...
      WriteLog('MAIN',
        'TForm1.ListBox1Click GridClear(GridActPlayList, RowGridClips)');
    end;
    GridActvePLToPanel(GridActPlayList.Row);
...
begin
  Try
    ...
    LoadGridFromFile(AppPath + DirProjects + '\' + 'ListProjects.prjt',
      GridProjects);
...
begin
  try
    ...
    if GridClickRow(GridProjects, Y) = 0 then
      exit;
    if GridProjects.Objects[0, GridProjects.Row] is TGridRows then
    begin
      if i = 3 then
      begin
        if (RowDownGridProject <> GridProjects.Row) and (RowDownGridProject > 0)
          and (GridProjects.Row > 0) then
        begin
          TempGridRow.Clear;
          TempGridRow.Assign((GridProjects.Objects[0, RowDownGridProject]
            as TGridRows));
          if GridProjects.Row < RowDownGridProject then
          begin
            for cnt := RowDownGridProject downto GridProjects.Row + 1 do
            begin
              (GridProjects.Objects[0, cnt] as TGridRows)
                .Assign((GridProjects.Objects[0, cnt - 1] as TGridRows));
            end;
          end
          else
          begin
            for cnt := RowDownGridProject to GridProjects.Row - 1 do
            begin
              (GridProjects.Objects[0, cnt] as TGridRows)
                .Assign((GridProjects.Objects[0, cnt + 1] as TGridRows));
            end;
        ...
        if DblClickProject then
        begin
          if GridProjects.Row > 0 then
          begin
            if (GridProjects.Objects[0, GridProjects.Row] is TGridRows) then
            begin
              ...
              then
                ...
                CreateProject(GridProjects.Row);
            end;
          end;
        end;
      ...
      if (GridProjects.Objects[0, GridProjects.Row] as TGridRows).ID <= 0 then
        exit;
      case i of
        0, 1:
          (GridProjects.Objects[0, GridProjects.Row] as TGridRows).MyCells[i]
            .Mark := not(GridProjects.Objects[0, GridProjects.Row] as TGridRows)
            .MyCells[i].Mark;
        2:
          begin
            ...
            for i := 1 to GridProjects.RowCount - 1 do
              (GridProjects.Objects[0, i] as TGridRows).MyCells[2].Mark
                := FALSE;
            (GridProjects.Objects[0, GridProjects.Row] as TGridRows)
              .MyCells[2].Mark := true;
            loadoldproject;
    ...
    on E: Exception do
      WriteLog('MAIN', 'TForm1.GridProjectsMouseUp | ' + E.Message);
  end;
end;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if clpid <> '' then
                  SaveGridToFile(clpid, GridActPlayList);
              end;
            end;
            ListBox1.Clear;
            for i := 1 to GridLists.RowCount - 1 do
...
begin
  ...
  if mode = play then
  begin
    pVideoWindow.SetWindowPosition(0, 0, pnMovie.ClientRect.Right,
      pnMovie.ClientRect.Bottom);
  end;
end;
    ...
    begin
      ...
      begin
        ...
        frSetTC.Top := (Form1.Height - Form1.ClientHeight - 2 *
          Form1.BevelWidth) + Form1.PanelControl.Height + GridClips.RowHeights
          [0] + (GridClips.Row - 1) * GridClips.RowHeights[GridClips.Row] - 5;
        ...
        txt := (GridClips.Objects[0, GridClips.Row] as TGridRows).MyCells[3]
          .ReadPhrase('TypeMedia');
        clpid := (GridClips.Objects[0, GridClips.Row] as TGridRows)
          .MyCells[3].ReadPhrase('ClipID');
        txt := Trim(SetTimeCode(txt));
        (GridClips.Objects[0, GridClips.Row] as TGridRows).MyCells[3]
          .updatephrase('TypeMedia', txt);
        ...
        ps := (GridClips.Objects[0, GridClips.Row] as TGridRows).MyCells[3]
          .PositionName('TypeMedia');
        (GridClips.Objects[0, GridClips.Row] as TGridRows).MyCells[3].Rows[ps.X]
          .Phrases[ps.Y].SubFontColor := clLime;
        (GridClips.Objects[0, GridClips.Row] as TGridRows).MyCells[3]
          .updatephrase('TypeTTL', 'Время старта:');
      end;
...
begin
  try
    try
      ...
      if (TLZone.DownTimeline) and (mode <> play) then
      begin
      ...
      if (TLZone.DownViewer) and (mode <> play) then
      begin
...
begin
  try
    ...
    on E: Exception do
      WriteLog('MAIN', 'TForm1.imgBlockProjectsClick | ' + E.Message);
  end;
end;
    ...
    begin
      ...
      begin
        ...
        txt := (GridActPlayList.Objects[0, GridActPlayList.Row] as TGridRows)
          .MyCells[3].ReadPhrase('TypeMedia');
        ...
        (GridActPlayList.Objects[0, GridActPlayList.Row] as TGridRows)
          .MyCells[3].updatephrase('TypeMedia', txt);
        if (Trim(lbActiveClipID.Caption) = Trim(clpid)) and (txt <> '') then
        begin
          MyStartPlay := StrTimeCodeToFrames(txt);
          MyStartReady := true;
        ...
        ps := (GridActPlayList.Objects[0, GridActPlayList.Row] as TGridRows)
          .MyCells[3].PositionName('TypeMedia');
        (GridActPlayList.Objects[0, GridActPlayList.Row] as TGridRows)
...
begin
  try
    ...
    If (not(ssAlt In Shift)) and (not(ssCtrl In Shift)) and (ssShift In Shift)
      and (Key = 81) then
    begin
    ...
    begin
      ...
      If (ssCtrl In Shift) and (not(ssAlt In Shift)) and (not(ssShift In Shift))
        and (Key = 79) then
      begin
      ...
      If (ssAlt In Shift) and (ssCtrl In Shift) and (not(ssShift In Shift)) and
        (Key = 77) then
      begin
        if mode <> play then
          SetLTC(2);
      ...
      If (not(ssAlt In Shift)) and (ssCtrl In Shift) and (not(ssShift In Shift))
        and (Key = 90) then
      begin
      ...
      If (not(ssAlt In Shift)) and (not(ssCtrl In Shift)) and
        (not(ssShift In Shift)) and (Key = VK_LEFT) then
      begin
        if mode <> play then
      ...
      If (not(ssAlt In Shift)) and (not(ssCtrl In Shift)) and
        (not(ssShift In Shift)) and (Key = VK_RIGHT) then
      begin
        if mode <> play then
      ...
      If (not(ssAlt In Shift)) and (not(ssCtrl In Shift)) and
        (not(ssShift In Shift)) and (Key = VK_HOME) then
      begin
      ...
      If (not(ssAlt In Shift)) and (not(ssCtrl In Shift)) and
        (not(ssShift In Shift)) and (Key = VK_END) then
      begin
      ...
      If (not(ssAlt In Shift)) and (not(ssCtrl In Shift)) and
        (not(ssShift In Shift)) and (Key = 67) then
      begin
      ...
      If (not(ssAlt In Shift)) and (not(ssCtrl In Shift)) and
        (not(ssShift In Shift)) and (Key = VK_DELETE) then
      begin
      ...
      If (not(ssAlt In Shift)) and (not(ssCtrl In Shift)) and
        (not(ssShift In Shift)) and (Key = 86) then
      begin
      ...
      begin
        WriteLog('MAIN', 'TForm1.FormKeyUp pnMediaTL.Visible Key=' +
          ShiftToStr(Shift, Key));
        ...
        If (not(ssAlt In Shift)) and (ssCtrl In Shift) and
          (not(ssShift In Shift)) and (Key = 77) then
        begin
          ButtonsControlMedia(4);
          exit;
        end;

        If (ssAlt In Shift) and (not(ssCtrl In Shift)) and
          (not(ssShift In Shift)) and (Key = 77) then
        begin
      ...
      begin
        WriteLog('MAIN', 'TForm1.FormKeyUp pnTextTL.Visible Key=' +
          ShiftToStr(Shift, Key));
        If (ssAlt In Shift) and (not(ssCtrl In Shift)) and
          (not(ssShift In Shift)) and (Key = 67) then
        begin
      ...
      If (not(ssAlt In Shift)) and (not(ssCtrl In Shift)) and
        (not(ssShift In Shift)) and (Key = 73) then
      begin
        if TLParameters.Position < TLParameters.Finish then
        begin
          SaveToUNDO;
      ...
      If (not(ssAlt In Shift)) and (not(ssCtrl In Shift)) and
        (not(ssShift In Shift)) and (Key = 79) then
      begin
      ...
      If (ssCtrl In Shift) And (Key in [49 .. 57, 97 .. 105]) then
      begin
        SaveToUNDO;
        if Key >= 97 then
          InsertEventToEditTimeline(Key - 87)
        else
          InsertEventToEditTimeline(Key - 39);
        exit;
      end;

      If (ssCtrl In Shift) And ((Key = 48) or (Key = 96)) then
      begin
        SaveToUNDO;
      ...
      If (ssShift In Shift) And (Key in [49 .. 57, 97 .. 105]) then
      begin
        SaveToUNDO;
        if Key >= 97 then
          InsertEventToEditTimeline(Key - 77)
        else
          InsertEventToEditTimeline(Key - 29);
        exit;
      end;

      If (ssShift In Shift) And ((Key = 48) or (Key = 96)) then
      begin
        SaveToUNDO;
      ...
      If (ssAlt In Shift) And ((Key = 49) or (Key = 97)) then
      begin
        SaveToUNDO;
      ...
      If (ssAlt In Shift) And ((Key = 50) or (Key = 98)) then
      begin
        SaveToUNDO;
      ...
      If Key in [49 .. 57, 97 .. 105] then
      begin
        SaveToUNDO;
        if Key >= 97 then
          InsertEventToEditTimeline(Key - 97)
        else
    ...
    begin
      ...
      If (ssAlt In Shift) and (not(ssCtrl In Shift)) and (not(ssShift In Shift))
        and (Key = 83) then
      begin
        ButtonPlaylLists(2);
        exit;
      ...
      if ActiveControl = GridProjects then
      begin
        If (not(ssAlt In Shift)) and (not(ssCtrl In Shift)) and
        ...
        begin
          ...
          for i := 1 to GridProjects.RowCount - 1 do
            (GridProjects.Objects[0, i] as TGridRows).MyCells[2].Mark := FALSE;
          (GridProjects.Objects[0, GridProjects.Row] as TGridRows).MyCells[2]
            .Mark := true;
          loadoldproject;
          ProjectToPanel(GridProjects.Row);
          exit;
