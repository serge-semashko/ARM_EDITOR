...
Begin {Execute}
  try
    ...
    while not Terminated do QueryPort;
...
Begin {InitPort}
  ...
  else if ansilowercase(trim(Port422Parity))='нечет' then DCB.Parity := 1
  else if ansilowercase(trim(Port422Parity))='нет' then DCB.Parity := 0
  else if ansilowercase(trim(Port422Parity))='чет' then DCB.Parity := 2
  else if ansilowercase(trim(Port422Parity))='маркер' then DCB.Parity := 3
  else if ansilowercase(trim(Port422Parity))='пробел' then DCB.Parity := 4;
  ...
  else if ansilowercase(trim(Port422Flow))='нет' then DCB.Flags := DCB.Flags and $00000011
  else if ansilowercase(trim(Port422Flow))='XOn/XOff' then DCB.Flags := DCB.Flags and $00000311
  else if ansilowercase(trim(Port422Flow))='Аппаратное' then DCB.Flags := DCB.Flags and $00002015;
...
Begin {StartService}
  ...
  if result then begin
    StartComThread; //если порт проинициализировался, то запускаем поток
    PortRSStoped := false;
  end;
