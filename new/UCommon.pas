...
begin
  ...
  for i:=1 to Grid.RowCount-1 do begin
    ...
    if stime='' then begin
      ListErrors.Add('Отстутствует время старта:   [' + inttostr(i) + '] ' + Clp);
    end else begin
...
begin
  if GridPlayer=grClips then begin
    ...
    if form1.listbox1.ItemIndex=-1
      then form1.lbusesclpidlst.Caption := 'Плей-лист: '
      else form1.lbusesclpidlst.Caption := 'Плей-лист: ' + trim(form1.ListBox1.Items.Strings[form1.ListBox1.ItemIndex]);
  end;
end;
         ...
         txt := (GridClips.Objects[0,i] as TGridRows).MyCells[3].ReadPhrase('File');
           ...
           if Trim(txt)<>'' then begin
             if not FileExists(txt) then begin
               ...
               WriteLog('MAIN', 'UCommon.CheckedClipsInList-1 GridClips File=' + txt + 'Не существует');
             end else WriteLog('MAIN', 'UCommon.CheckedClipsInList GridClips File=' + txt + 'Найден');
           end else begin
             (GridClips.Objects[0,i] as TGridRows).MyCells[3].SetPhraseColor('Clip', PhraseErrorColor);
             WriteLog('MAIN', 'UCommon.CheckedClipsInList-2 GridClips File=' + txt + 'Не существует');
           end;
        end;
      end;
