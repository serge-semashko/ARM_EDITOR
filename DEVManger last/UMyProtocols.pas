...
begin
  ...
  WriteLog('Translator', 'GetCommandValue: Source=' + Source + ' Param=' + Param
    + ' Phrase=' + Phrase + 'TypePhrase=' + TypePhrase + ' Index' +
    Index + 'Result=' + result);
end;
...
begin
  ...
  begin
    ...
    WriteLog('Translator', 'TOneByte.GetValue Field=' + inttostr(i) +
      ' TypeField=' + Fields[i].TypeField + ' VarData=' + Fields[i].VarData);
...
function TOneCommand.GetValue(evnt: TMyEvent): string;
var
  i: integer;
begin
  result := '';
...
procedure TOneCommand.SetString(stri: string);
var
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if (slnk = 'thousands') or (slnk = 'hundres') or (slnk = 'tens')
                  or (slnk = 'ones') then
  ...
  try
    ...
    begin
      ...
      begin
        ...
        CommandsList[rw].SetString(sprotocols);
      end;
    end
    else
    begin
...
begin
  ...
  TypeDate := GetProtocolsParam(ss, 'TypeDate');
...
begin
  ...
  begin
    if ansilowercase(trim(CommandsList[i].Name)) = ansilowercase(trim(cmd)) then
    begin
      result := CommandsList[i].GetValue(evnt);
      WriteLog('Translator', 'TCommandTemplates.GetCommand  Command=' + cmd +
        ' Result=' + result);
      exit;
    end;
  end;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  if ansilowercase
                    (trim(FirmDevices[i].ListProtocols[m].Protocol))
                    = ansilowercase(trim(Prot)) then
                  begin
...
begin
  ...
  begin
    ...
    for i := 0 to Count - 1 do
    begin
      if ansilowercase(trim(CaseItems[i].CaseName)) = ansilowercase(trim(scond))
      then
      begin
      ...
      begin
        ...
        begin
          ...
          List[Count - 1] := TOneStringTable.create
            (trim(rng.Text) + inttostr(j), GetParamData(srng.Name, sind));
          sind := sind + 1;
        end;
      end
      else
