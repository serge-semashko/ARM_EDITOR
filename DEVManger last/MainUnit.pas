    ...
    begin
      ...
      if InfoWEB.Options[0].Text <> '' then
        TLParameters.Position := TLParameters.Preroll +
          StrTimeCodeToFrames(InfoWEB.Options[1].Text);
      ...
      begin
        strchron := FramesToStr(MyTLEdit.Events[MyTLEdit.Count - 1].Finish -
          MyTLEdit.Events[0].Finish);
        ...
        begin
          ...
          crdr := inttostr(MyTLEdit.Events[MyTLEdit.Count - 1].ReadPhraseData
            ('Duration'));
          crst := inttostr(MyTLEdit.Events[MyTLEdit.Count - 1]
            .ReadPhraseData('Set'));
          crcmd := MyTLEdit.Events[MyTLEdit.Count - 1].ReadPhraseCommand
            ('Command');
        ...
        begin
          ...
          crdr := inttostr(MyTLEdit.Events[crpos.Number].ReadPhraseData
            ('Duration'));
          ...
          cmdcmd := MyTLEdit.Events[crpos.Number + 1].ReadPhraseCommand
            ('Command');
          cmdc := MyTLEdit.Events[crpos.Number + 1].ReadPhraseText('Command');
          sdur := inttostr(MyTLEdit.Events[crpos.Number + 1].ReadPhraseData
            ('Duration'));
          ...
          DurFrame := MyTLEdit.Events[crpos.Number].Start + MyTLEdit.Events
            [crpos.Number].ReadPhraseData('Duration');
          ...
          nDurFrame := MyTLEdit.Events[crpos.Number + 1].Start + MyTLEdit.Events
            [crpos.Number + 1].ReadPhraseData('Duration');
          if nDurFrame > MyTLEdit.Events[crpos.Number + 1].Finish then
            DurFrame := MyTLEdit.Events[crpos.Number + 1].Finish - 1;
        end;
        sstart := FramesToStr(framestostart - 1);

        InfoWEB.SetData(3, inttostr(MyTLEdit.Count) + '  (' +
          inttostr(crpos.Number + 1) + ')'); // 'Кол-во событий (текущее):'
        InfoWEB.SetData(4, ''); // 'Режим воспроизведения:'
        InfoWEB.SetData(5, FramesToStr(MyTLEdit.Events[crpos.Number].Finish -
        ...
        InfoWEB.SetData(11, next + '  S=' + inttostr(nStartFrame) + '  T=' +
          inttostr(nDurFrame) + '  F=' + inttostr(nEndFrame));
        // 'След. устройство:'
        ...
        begin
          ...
          for icmd := 0 to ListCommands.Count - 1 do
            WriteBuffToPort(DataToBuffIn(trim(ListCommands.Strings[icmd])));
        ...
        begin
          ...
          if myprotocol <> nil then
            myprotocol.CMDTemplates.GetCMDStart(MyTLEdit.Events[crpos.Number],
              ListCommands);
          for icmd := 0 to ListCommands.Count - 1 do
            WriteBuffToPort(DataToBuffIn(trim(ListCommands.Strings[icmd])));
...
begin
  try
    ...
    begin
      tempport := CreateFile(PChar(port422name), GENERIC_READ or GENERIC_WRITE,
        FILE_SHARE_READ or FILE_SHARE_WRITE, nil, OPEN_EXISTING,
        FILE_ATTRIBUTE_NORMAL, 0);
      if tempport <= 0 then
      begin
        MessageDlg('Не удалось активировать порт ' + port422name + '.' + #10#13
          + 'Возможно он отрыт в другой программе.', mtInformation,
          [mbOk], 0, mbOk);
      end
      else
      begin;
    ...
    MessageDlg('Ошибка обращения к порту ' + port422name + '.', mtInformation,
      [mbOk], 0, mbOk);
  end;
end;
