...
begin
  try
  if MakeLogging then WriteLog('MAIN', 'UDrawTimelines.TTLNames.Draw Start');
  ...
  if MakeLogging then WriteLog('MAIN', 'UDrawTimelines.TTLNames.Draw Finish');
  except
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TTLNames.Draw | ' + E.Message);
  end;
end;
...
begin
  ...
  if MakeLogging then WriteLog('MAIN', 'UDrawTimelines.TTLNames.SetDefaultFonts;');
end;
...
var i, res, APos, GPos, MyY : integer;
begin
  try
  if MakeLogging then WriteLog('MAIN', 'UDrawTimelines.TTLNames.ClickTTLNames Start Grid=' + Grid.Name);
  ...
  if Result.Position >= 0 then begin
     for i:=0 to NamesTL.Count-1 do NamesTL.Names[i].Editing:=false;
  ...
  if MakeLogging then WriteLog('MAIN', 'UDrawTimelines.TTLNames.ClickTTLNames Finish Position=' + inttostr(Result.Position) + ' IDTimeline=' + inttostr(Result.ID));
  except
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TTLNames.ClickTTLNames | ' + E.Message);
  end;
end;
...
begin
  ...
  Review.WriteToStream(F);
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TTLNames.WriteToStream | ' + E.Message);
  end;
end;
...
begin
  ...
  Review.ReadFromStream(F);
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TTLNames.ReadFromStream | ' + E.Message);
  end;
end;
...
begin
  ...
  F.WriteBuffer(Interval, SizeOf(Interval));
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TTLHeights.WriteToStream | ' + E.Message);
  end;
end;
...
begin
  ...
  F.ReadBuffer(Interval, SizeOf(Interval));
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TTLHeights.ReadFromStream | ' + E.Message);
  end;
end;
...
begin
  try
  if MakeLogging then WriteLog('MAIN', 'UDrawTimelines.TZoneReview.Draw Top=' + inttostr(Top) + ' Height=' + inttostr(height));
  cbr := cv.Brush.Color;
  cpn := cv.Pen.Color;
  wpn := cv.Pen.Width;
  sbr := cv.Brush.Style;
  ...
  if FontBold then cv.Font.Style:=cv.Font.Style + [fsBold] else cv.Font.Style:=cv.Font.Style - [fsBold];
  if FontItalic then cv.Font.Style:=cv.Font.Style + [fsItalic] else cv.Font.Style:=cv.Font.Style - [fsItalic];
  if FontBold then cv.Font.Style:=cv.Font.Style + [fsUnderline] else cv.Font.Style:=cv.Font.Style - [fsUnderline];
  cv.Pen.Color:=FontColor;
  ...
  cv.Pen.Mode     := mpn;
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TZoneReview.Draw | ' + E.Message);
  end;
end;
...
begin
  ...
  if (X > ImgRect.Left) and (X < ImgRect.Right) and (Y > ImgRect.Top) and (Y < ImgRect.Bottom) then begin
    Result.Button:=btreview;
    WriteLog('MAIN', 'UDrawTimelines.TZoneReview.ClickViewer Button:=btreview');
  end else WriteLog('MAIN', 'UDrawTimelines.TZoneReview.ClickViewer Button:=btnone');;
  except
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TZoneReview.ClickViewer | ' + E.Message);
  end;
end;
...
begin
  ...
  F.WriteBuffer(stopview, SizeOf(stopview));
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TZoneReview.WriteToStream | ' + E.Message);
  end;
end;
...
begin
  ...
  F.ReadBuffer(stopview, SizeOf(stopview));
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TZoneReview.ReadFromStream | ' + E.Message);
  end;
end;
...
begin
  ...
  FontItalic := TLZoneNamesFontItalic;
  FontUnderline := TLZoneNamesFontUnderline;
  for i:=0 to Count-1 do Names[i].ColorUpdate;
end;
...
begin
  ...
  result := Count-1;
  if MakeLogging then WriteLog('MAIN', 'UDrawTimelines.TNamesTL.Add Result=' + inttostr(Result) + ' IDTimeline=' + inttostr(Timeline.IDTimeline));
end;
...
begin
  ...
  setlength(Names,0);
  Count:=0;
  if MakeLogging then WriteLog('MAIN', 'UDrawTimelines.TNamesTL.Clear');
end;
...
begin
  try
  if MakeLogging then WriteLog('MAIN', 'UDrawTimelines.TNamesTL.Draw Grid=' + Grid.Name + ' Top=' + inttostr(Top) + ' Height=' + inttostr(height));
  tp:=Top;
  Rect.Left:=cv.ClipRect.Left;
  Rect.Right:=cv.ClipRect.Right;
  Rect.Top:=Top;
  ...
  for i:=0 to Count-1 do begin
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TNamesTL.Draw | ' + E.Message);
  end;
end;
...
begin
  ...
  //Draw(cv,Rect.Top,Rect.Bottom-Rect.Top);
  if makeLogging then WriteLog('MAIN', 'UDrawTimelines.TNamesTL.ClickNamesTL Position=' + inttostr(Result.Position) + 'IDTimeline=' + inttostr(Result.ID));
  except
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TNamesTL.ClickNamesTL | ' + E.Message);
  end;
end;
...
begin
  ...
  for i:=0 to Count-1 do Names[i].WriteToStream(F);
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TNamesTL.WriteToStream | ' + E.Message);
  end;
end;
...
begin
  ...
  for i:=0 to Count-1 do begin
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TNamesTL.ReadFromStream | ' + E.Message);
  end;
end;
...
begin
  try
  if MakeLogging then WriteLog('MAIN', 'UDrawTimelines.TTimeLineName.Draw Position=' + inttostr(Apos));
  cbr := cv.Brush.Color;
  cpn := cv.Pen.Color;
  wpn := cv.Pen.Width;
  sbr := cv.Brush.Style;
  ...
  if Selection
    then cv.Font.Color:=FontColorSelect
    else if IDTimeline=ZoneNames.Edit.IDTimeline
           then cv.Font.Color:=FontColor
           else cv.Font.Color:=SmoothColor(FontColor,32);
  cv.Font.Name:=FontName;
  if FontBold then cv.Font.Style:=cv.Font.Style + [fsBold] else cv.Font.Style:=cv.Font.Style - [fsBold];
  if FontItalic then cv.Font.Style:=cv.Font.Style + [fsItalic] else cv.Font.Style:=cv.Font.Style - [fsItalic];
  if FontUnderline then cv.Font.Style:=cv.Font.Style + [fsUnderline] else cv.Font.Style:=cv.Font.Style - [fsUnderline];
 ...
 //   else
  if IDTimeline=ZoneNames.Edit.IDTimeline
    then cv.Brush.Color:=Color
    else cv.Brush.Color:=SmoothColor(Color,48);
  ...
  if (Grid.Objects[0,APos+1] is TTimelineOptions) then begin
    ...
    if TLZone.Count > APos
      then cv.Brush.Color:= StatusColor[TLZone.Timelines[APos].Status]
      else cv.Brush.Color:= StatusColor[4];
    cv.Pen.Color:=FontColor;
    ...
    if TLZone.Count > APos then begin
      if TLZone.Timelines[APos].Block//(Grid.Objects[0,APos+1] as TTimelineOptions).Block
        then LoadBMPFromRes(cv, BlockRect, 20, 20, 'Lock')
        else LoadBMPFromRes(cv, BlockRect, 20, 20, 'Unlock');
    end else LoadBMPFromRes(cv, BlockRect, 20, 20, 'Unlock');
  end;
  ...
  cv.Pen.Mode     := mpn;
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TTimeLineName.Draw| ' + E.Message);
  end;
end;
...
begin
  ...
  F.WriteBuffer(StatusRect.Right, SizeOf(integer));
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TTimeLineName.WriteToStream| ' + E.Message);
  end;
end;
...
begin
  ...
  F.ReadBuffer(StatusRect.Right, SizeOf(StatusRect.Right));
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TTimeLineName.ReadFromStream | ' + E.Message);
  end;
end;
...
begin
  try
  if MakeLogging then WriteLog('MAIN', 'UDrawTimelines.TZoneEditTL.Draw Grid=' + Grid.Name + ' Top=' + inttostr(Top) + ' Height=' + inttostr(height));
  cbr := cv.Brush.Color;
  cpn := cv.Pen.Color;
  wpn := cv.Pen.Width;
  sbr := cv.Brush.Style;
  ...
  if (Grid.Objects[0,Apos] is TTimelineOptions) then begin
    ...
    with (Grid.Objects[0,Apos] as TTimelineOptions) do begin
      ...
      cv.Pen.Color:=FontColor;
      if TLZone.Count > APos-1 then begin
        if StatusSelect then begin
          cv.Brush.Color:=SmoothColor(StatusColor[TLZone.Timelines[APos-1].Status],32);
      ...
      if TLZone.Count > APos-1 then begin
        if TLZone.Timelines[APos-1].Block then begin
        ...
        end else LoadBMPFromRes(cv, BlockRect, 25, 25, 'Unlock');
      end else LoadBMPFromRes(cv, BlockRect, 25, 25, 'Unlock');
    end;
  end;
  ...
  cv.Pen.Mode     := mpn;
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TZoneEditTL.Draw | ' + E.Message);
  end;
end;
...
begin
  ...
  for i:=1 to Grid.RowCount-1 do begin
    if Grid.Objects[0,i] is TTimelineOptions then begin
      s := inttostr((Grid.Objects[0,i] as TTimelineOptions).IDTimeline); //Warning
      if (Grid.Objects[0,i] as TTimelineOptions).IDTimeline=IDTimeline then begin
        result := i;
        if MakeLogging then WriteLog('MAIN', 'UDrawTimelines.TZoneEditTL.GridPosition Grid=' + Grid.Name + ' Position=' + inttostr(result) + ' IDTimeline=' + inttostr(IDTimeline));
        exit;
      end;
    end;
  end;
  if MakeLogging then WriteLog('MAIN', 'UDrawTimelines.TZoneEditTL.GridPosition Grid=' + Grid.Name + ' Position=' + inttostr(result) + ' IDTimeline=' + inttostr(IDTimeline));
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TZoneEditTL.GridPosition | ' + E.Message);
  end;
end;
...
begin
  ...
  for i:=0 to NM.Count-1 do begin
    if NM.Names[i].IDTimeline=IDTimeline then begin
      result:=i;
      if MakeLogging then WriteLog('MAIN', 'UDrawTimelines.TZoneEditTL.TLPosition Position=' + inttostr(result) + ' IDTimeline=' + inttostr(IDTimeline));
      exit;
    end;
  end;
  if MakeLogging then WriteLog('MAIN', 'UDrawTimelines.TZoneEditTL.TLPosition Position=' + inttostr(result) + ' IDTimeline=' + inttostr(IDTimeline));
  except
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TZoneEditTL.TLPosition | ' + E.Message);
  end;
end;
...
begin
  ...
  if APos > 0 then begin
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TZoneEditTL.AssignTL | ' + E.Message);
  end;
end;
...
begin
  ...
  if Grid.Objects[0,GPos] is TTimelineOptions then begin
    if (X > StatusRect.Left) and (X < StatusRect.Right) and (Y > StatusRect.Top) and (Y < StatusRect.Bottom) then begin
      ...
      TLZone.Timelines[GPos-1].Status:=TLZone.Timelines[GPos-1].Status+1;
      If TLZone.Timelines[GPos-1].Status > 4 then TLZone.Timelines[GPos-1].Status:=0;
      ...
      TLZone.Timelines[GPos-1].Block:=not TLZone.Timelines[GPos-1].Block;
    end;
  end;
  if MakeLogging then WriteLog('MAIN', 'UDrawTimelines.TZoneEditTL.ClickEditTl Grid=' + Grid.Name + ' Position=' + inttostr(GPos) + ' IDTimeline=' + inttostr(IDTimeline));
  except
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TZoneEditTL.ClickEditTl | ' + E.Message);
  end;
end;
...
begin
  ...
  F.WriteBuffer(StatusSelect, SizeOf(StatusSelect));
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TZoneEditTL.WriteToStream | ' + E.Message);
  end;
end;
...
begin
  ...
  F.ReadBuffer(StatusSelect, SizeOf(StatusSelect));
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TZoneEditTL.ReadFromStream | ' + E.Message);
  end;
end;
...
//  cv.Rectangle(PlusW);
  ...
  cv.Pen.Mode     := mpn;
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TZoneScaler.Draw | ' + E.Message);
  end;
end;
...
begin
  ...
  result.Position:=-1;
  result.Button:=btnone;
  if (X > PlusH.Left) and (X < PlusH.Right) and (Y > PlusH.Top) and (Y < PlusH.Bottom) then begin
    result.Button := btplusH;
    if makelogging then WriteLog('MAIN', 'UDrawTimelines.TZoneScaler.ClickScaler btplusH');
  end else if (X > minusH.Left) and (X < minusH.Right) and (Y > minusH.Top) and (Y < minusH.Bottom) then begin
    result.Button := btminusH;
    if makelogging then WriteLog('MAIN', 'UDrawTimelines.TZoneScaler.ClickScaler btminusH');
  end else if (X > minusW.Left) and (X < minusW.Right) and (Y > minusW.Top) and (Y < minusW.Bottom) then begin
    result.Button := btminusW;
    if makelogging then WriteLog('MAIN', 'UDrawTimelines.TZoneScaler.ClickScaler btminusW');
  end else if (X > PlusW.Left) and (X < PlusW.Right) and (Y > PlusW.Top) and (Y < PlusW.Bottom) then begin
    result.Button := btplusW;
    if makelogging then WriteLog('MAIN', 'UDrawTimelines.TZoneScaler.ClickScaler btplusW');
  end else if (X > procent.Left) and (X < procent.Right) and (Y > procent.Top) and (Y < procent.Bottom) then begin
    result.Button := btprocent;
    if makelogging then WriteLog('MAIN', 'UDrawTimelines.TZoneScaler.ClickScaler btprocent');
  end else if makelogging then WriteLog('MAIN', 'UDrawTimelines.TZoneScaler.ClickScaler btnone');;
  except
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TZoneScaler.ClickScaler | ' + E.Message);
  end;
end;
...
begin
  ...
  F.WriteBuffer(procentSelect, SizeOf(procentSelect));
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TZoneScaler.WriteToStream | ' + E.Message);
  end;
end;
...
begin
  ...
  F.ReadBuffer(procentSelect, SizeOf(procentSelect));
    ...
    on E: Exception do WriteLog('MAIN', 'UDrawTimelines.TZoneScaler.ReadFromStream | ' + E.Message);
  end;
end;
