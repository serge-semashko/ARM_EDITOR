...
type
  ...
  TTimelineOptions = Class(TObject)
    ...
    Status : integer;
    CountDev   : Integer;
    DevNames : array[0..31] of string;
    DevColors : array[0..31] of tcolor;
    MediaColor : tcolor;
    TextColor : tcolor;
...
uses UMain, ucommon, UButtonOptions, umyevents, uinitforms, umymessage;
...
begin
  ...
  for i:=0 to cntbtns-1
    do if cv.TextWidth((obj as TTimelineOptions).DevNames[i]) > szbtns
         then szbtns:=cv.TextWidth((obj as TTimelineOptions).DevNames[i]);
  szbtns:=szbtns + 20;
  ...
  for i:=0 to cnty-1 do begin
    ...
    for j:=0 to cntx-1 do begin
      ABtn := ARow * cntx + j;
      BTNSDEVICE.Rows[ARow].AddButton((obj as TTimelineOptions).DevNames[ABtn],imnone);
    end;
    //Rows[i].Count:=cntx;
  end;
  ...
  for i:=0 to BTNSDEVICE.Count -1 do begin
    ...
    for j:=0 to BTNSDEVICE.Rows[i].Count-1 do begin
      ...
      BTNSDEVICE.Rows[i].Btns[j].Color:=(obj as TTimelineOptions).DevColors[i*cntx + j];
...
var i : integer;
begin
  for i:=0 to 31 do OPTTimeline.DevNames[i]:=Trim(Edit2.Text) + OPTTimeline.DevNames[i];
  InitBTNSDEVICE(FEditTimeline.Image1.Canvas, OPTTimeline, BTNSDEVICE);
end;
...
var i : integer;
begin
  for i:=0 to 31 do OPTTimeline.DevNames[i]:=IntToStr(i+1);
  InitBTNSDEVICE(FEditTimeline.Image1.Canvas, OPTTimeline, BTNSDEVICE);
end;
...
var i : integer;
begin
   for i:=0 to 31 do OPTTimeline.DevNames[i]:=StringReplace(OPTTimeline.DevNames[i],Edit2.Text,'', [ rfReplaceAll, rfIgnoreCase ]);
  InitBTNSDEVICE(FEditTimeline.Image1.Canvas, OPTTimeline, BTNSDEVICE);
end;
