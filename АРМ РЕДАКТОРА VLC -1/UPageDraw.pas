...
Var MaketOptions : TMaketOptions;
...
var i : integer;
begin
  for i:=0 to Count-1 do Rows[i].FreeInstance;
  Count:=0;
...
begin
  ...
  if shps = 1 then begin
    if TLZone.Timelines[tlnom].Events[evnom].Start < TLParameters.Start
      then Rows[Count-1].Start := TLParameters.Start - TLParameters.Preroll
      else Rows[Count-1].Start := TLZone.Timelines[tlnom].Events[evnom].Start - TLParameters.Preroll;
  end else Rows[Count-1].Start := TLZone.Timelines[tlnom].Events[evnom].Start - TLParameters.Preroll;
  if TLZone.Timelines[tlnom].Events[evnom].Finish > TLParameters.Finish
    then Rows[Count-1].Finish := TLParameters.Finish - TLParameters.Preroll
    else Rows[Count-1].Finish := TLZone.Timelines[tlnom].Events[evnom].Finish - TLParameters.Preroll;
  Rows[Count-1].Duration := Rows[Count-1].Finish - Rows[Count-1].Start;
  Rows[Count-1].Device := TLZone.Timelines[tlnom].Events[evnom].ReadPhraseData('Device');
  Rows[Count-1].Text := TLZone.Timelines[tlnom].Events[evnom].ReadPhraseText('Text');
  Rows[Count-1].Comment := TLZone.Timelines[tlnom].Events[evnom].ReadPhraseText('Comment');
  result := count-1;
end;
...
begin
  ...
  for i := 0 to Count - 1 do begin
    if (tc >= Rows[i].Start) and (tc <= Rows[i].Finish) then begin
      result := i;
      exit;
    end;
  end;
...
var strt, fnsh, dur, dur1 : longint;
...
begin
  ...
  if (TLZone.Timelines[tlnom].Events[evnom].Finish <= TLParameters.Start) then exit;
  if (TLZone.Timelines[tlnom].Events[evnom].Start > TLParameters.Finish) then exit;
  ...
  if (ps1=-1) and (ps2=-1) then begin
     if Rows[0].Start >= fnsh then exit;
     if fnsh >= Rows[Count-1].Finish then begin
  ...
  if (ps1=-1) and (ps2>=0) then ps1:=0;

  if (ps1>=0) and (ps2=-1) then ps2:=Count-1;
...
//  end;
  ...
  if ps1 < ps2 then begin
   i := ps1;
    while i < ps2 do begin
      dur1 := Rows[i].Finish - strt;
      cch  := trunc(dur1 / wch);
      txt1 := copy(txt,1,cch);
      txt  := copy(txt,cch+1, length(txt));
      if i = ps1
        then Rows[i].Lyrics:=Trim(Rows[i].Lyrics + ' ' + txt1 + '..')
        else Rows[i].Lyrics:=Trim(Rows[i].Lyrics + ' ..' + txt1 + '..');
      i := i + 1;
      strt := Rows[i].Start;
    end;
...
var i, ps, dlt : integer;
    strt : longint;
begin
  Clear;
  for i:=0 to TLZone.Timelines[tldev].Count-1 do begin
    if TLZone.Timelines[tldev].Events[i].Finish<=TLParameters.Start then continue;
    if (TLZone.Timelines[tldev].Events[i].Start<=TLParameters.Start)
         and (TLZone.Timelines[tldev].Events[i].Finish>TLParameters.Start) then begin
      dlt := i;
      AddDevice(tldev,i, i-dlt+1);
      continue;
    end;
    if TLZone.Timelines[tldev].Events[i].Start < TLParameters.Finish
      then AddDevice(tldev,i, i-dlt+1) else break;
  end;
  if whatload then begin
    for i:=0 to TLZone.Timelines[tltxt].Count-1
      do if TLZone.Timelines[tltxt].Events[i].Start < TLParameters.Finish
           then AddLyrics(tltxt,i);
  end;
  if Count=0 then exit;
  for i:=0 to Count-1 do Rows[i].Start := Rows[i].Start - Rows[0].Start;
end;
...
begin
  ...
  if tm = tmScreen then begin
    ...
    Left := trunc(PrintPageParameters.SpaceLeft * mmX) + PrintPageParameters.PixPrinterOffset.X;
    Top := trunc(PrintPageParameters.SpaceTop * mmY) + PrintPageParameters.PixPrinterOffset.Y;
    Right := trunc(Width - PrintPageParameters.SpaceRight * mmX) - PrintPageParameters.PixPrinterOffset.X;
    Bottom := trunc(Height - PrintPageParameters.SpaceBottom * mmY) - PrintPageParameters.PixPrinterOffset.Y;
  end;
