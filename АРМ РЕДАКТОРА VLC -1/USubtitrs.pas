...
//  Первый проход проверяем правильно ли заданы данные
    ...
    for i:=0 to lst.Count-1 do begin
       ...
       if trim(lst.Strings[i])<>'' then begin
           case nom of
        0: if not isInteger(lst.Strings[i]) then err.Add('Строка ' + IntToStr(i) + ': Невозможно преобразовать данные в чиловое значение');
        1: if not SelectTimeCodes(lst.Strings[i], sbt) then err.Add('Строка ' + IntToStr(i) + ': Неправильно задан формат тайм-кода');
...
begin
  ...
  try
    ...
    for i:=0 to lst.Count-1 do begin
      ...
      if s1<>'' then begin
        lengthfr := Round(length(s1) * (Form1.GridTimeLines.Objects[0, pstl+1] as TTimelineOptions).CharDuration / 40);
        if lengthfr < (Form1.GridTimeLines.Objects[0, pstl+1] as TTimelineOptions).EventDuration
          then lengthfr := (Form1.GridTimeLines.Objects[0, pstl+1] as TTimelineOptions).EventDuration;
       // TLZone.Timelines[pstl].Count:=TLZone.Timelines[pstl].Count+1;
       // setlength(TLZone.Timelines[pstl].Events,TLZone.Timelines[pstl].Count);
       // TLZone.Timelines[pstl].Events[TLZone.Timelines[pstl].Count-1] := TMyEvent.Create;

        APos := TLZone.TLEditor.AddEvent(starttc,pstl+1,0); //InsertEventToEditTimeline(-1);
...
begin
  ...
  if trim(ansilowercase(ext))=trim(ansilowercase('.srt'))
    then LoadSubtitrsSRT(Form1.OpenDialog2.FileName)
    else LoadSubtitrsTXT(Form1.OpenDialog2.FileName);
