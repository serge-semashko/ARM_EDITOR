...
begin
  ...
  WorkDirTextTemplate:=IncludeTrailingBackslash(extractfilepath(Opendialog1.FileName));
  CheckListBox1.Items.LoadFromFile(Opendialog1.FileName);
  For i:=CheckListBox1.Count-1 downto 0 do begin
    if trim(CheckListBox1.Items[i])='' then CheckListBox1.Items.Delete(i)
    else begin
...
var i, ps : integer;
begin
  if WorkDirTextTemplate<>'' then FTextTemplate.OpenDialog1.InitialDir:=WorkDirTextTemplate;
  ...
  if rg then begin
    ...
    if ARow= -1 then begin
      ...
      FTextTemplate.Edit1.Text:=(Form1.GridLists.Objects[0,ARow] as TGridRows).MyCells[1].ReadPhrase('Template');
  ...
  if FTextTemplate.ModalResult = mrOk then begin
    if rg then begin
      Result:=FTextTemplate.SelString;
    end else begin
      if ARow=-1 then begin
         if FTextTemplate.CCount > 0 then begin
           for i:=0 to FTextTemplate.CheckListBox1.Count-1 do begin
             if FTextTemplate.CheckListBox1.Checked[i] then begin
               ps := GridAddRow(Form1.GridLists, RowGridListTxt);
               IDTXTTmp:=IDTXTTmp+1;
               (Form1.GridLists.Objects[0,ps] as TGridRows).ID:=IDTXTTmp;
               (Form1.GridLists.Objects[0,ps] as TGridRows).MyCells[1].UpdatePhrase('Template',FTextTemplate.CheckListBox1.Items[i]);
             end
           end;
           exit;
         end;
         if trim(FTextTemplate.Edit1.Text) <> '' then begin
           ps := GridAddRow(Form1.GridLists, RowGridListTxt);
           IDTXTTmp:=IDTXTTmp+1;
           (Form1.GridLists.Objects[0,ps] as TGridRows).ID:=IDTXTTmp;
           (Form1.GridLists.Objects[0,ps] as TGridRows).MyCells[1].UpdatePhrase('Template',FTextTemplate.Edit1.Text)
         end
      end else begin
        (Form1.GridLists.Objects[0,ARow] as TGridRows).MyCells[1].UpdatePhrase('Template',FTextTemplate.Edit1.Text);
      end;
    end;
  end;
end;
...
begin
  ...
  if CRow=-1 then begin
    ...
    if CheckListBox1.Count > 0 then begin
      for i:=0 to CheckListBox1.Count-1 do if CheckListBox1.Checked[i] then CCount:=CCount+1;
...
begin
  ...
  Checked:=not Checked;
  if Checked then SpeedButton4.Caption:='Выделить всё' else SpeedButton4.Caption:='Снять выделение';
end;
