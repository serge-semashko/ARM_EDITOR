...
uses
  ...
  Dialogs, StdCtrls, Buttons, Vcl.ExtCtrls, Grids;
...
begin
  ...
  if frLTC.ModalResult=mrOk then begin
    if tp=1 then begin
      ...
      if ps>=1 then (Form1.GridClips.Objects[0,ps] as tgridrows).MyCells[3].UpdatePhrase('StartTime',trim(frLTC.Edit1.Text));
      ps := FindClipInGrid(Form1.GridActPlayList,trim(Form1.lbActiveClipID.Caption));
      if ps>=1 then (Form1.GridActPlayList.Objects[0,ps] as tgridrows).MyCells[3].UpdatePhrase('StartTime',trim(frLTC.Edit1.Text));
...
begin
  if TextTimeCodeOK(Edit1.Text) then ModalResult:=mrOk
    ...
    MyTextMessage('Предупреждение','Не правильно задан тайм-код,'
                   + #10#13 + 'формат должен быть HH:MM:SS:FF'
...
begin
  ...
  with form1 do begin
    for i:=1 to Grid.RowCount-1 do begin
      txt := trim((Grid.Objects[0,i] as TGridRows).MyCells[3].ReadPhrase('ClipId'));
...
begin
  if trim(Form1.lbActiveClipID.Caption)<>'' then begin
    ...
    txt := trim((Form1.GridClips.Objects[0,ps] as TGridRows).MyCells[3].ReadPhrase('StartTime'));
...
procedure TfrLTC.FormKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if (ssAlt In Shift) and ((Key=ord('a')) or (Key=ord('A')) or (Key=ord('ф')) or (Key=ord('Ф'))) then begin
    edit1.SelStart := 0;
