...
begin
  ...
  try
    ...
    addVariableToJson(json,'sf5', CTHTML(ColorToRgb(SmoothColor(ForeGround, 5))));
    addVariableToJson(json,'sb5', CTHTML(SmoothColor(BackGround, 5)));
    addVariableToJson(json,'sct25', CTHTML(ColorToRgb(SmoothColor(ColorTimeline, 25))));
    addVariableToJson(json,'sct56', CTHTML(ColorToRgb(SmoothColor(ColorTimeline, 56))));
...
begin
  inherited;
  BackGround := AirBackGround;//ProgrammColor;
  if airBackGround=TimeForeGround
    then ForeGround := SmoothColor(AirBackGround, 12)
    else ForeGround := TimeForeGround;
...
begin
  inherited;
  BackGround := AirBackGround;//ProgrammColor;
  if airBackGround=airForeGround
    then ForeGround := SmoothColor(AirBackGround, 12)
    else ForeGround := AirForeGround;
