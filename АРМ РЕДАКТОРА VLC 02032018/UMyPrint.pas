...
begin
  ...
  for i:=0 to TLZone.Count-1 do begin
    if TLZone.Timelines[i].TypeTL=typetl then begin
      if tl = ps then begin
        result := i;
        exit;
      end;
...
var wdth, hght, hght1, CurW, CurH : integer;
    txt : string;
begin
  ...
  cv.Font.Size:=Form1.lbNomClips.Font.Size;
  CurW:=MaketOptions.Left;
  CurH:=MaketOptions.Top;
  txt := '[' + Form1.lbNomClips.Caption + ']';
  wdth:=cv.TextWidth(txt);
  hght:=cv.TextHeight(txt);
  cv.TextOut(CurW,CurH,txt);
  cv.TextOut(CurW+wdth + 5 * MaketOptions.mmHorz,CurH,Form1.lbClipName.Caption);

  cv.Font.Size:=Form1.lbNomClips.Font.Size + 2;
  ...
  hght := cv.TextHeight(Form1.lbSongName.Caption);
  cv.TextOut(CurW, CurH, Form1.lbSongName.Caption);

  cv.Font.Size:=Form1.lbClipSinger.Font.Size;
  CurH:=CurH + hght + 3 * MaketOptions.mmVert;
  hght := cv.TextHeight(Form1.lbClipSinger.Caption);
  cv.TextOut(CurW, CurH, Form1.lbClipSinger.Caption);
...
begin
  ...
  hght := posy - hght;
  cv.TextOut(MaketOptions.Left,hght,'Съёмка');
  cv.TextOut(MaketOptions.X1,hght,'Начало');
  cv.TextOut(MaketOptions.X2,hght,'Хронометраж');
  cv.TextOut(MaketOptions.X3,hght,'Камера');
  cv.TextOut(MaketOptions.X4,hght,'Текст');
  if frMyPrint.cbDataPrint.ItemIndex=0
    then cv.TextOut(MaketOptions.X5,hght,'Комментарий')
    else cv.TextOut(MaketOptions.X5,hght,'Текст песни');
end;

function DrawMaketTableRow(cv : tcanvas; pstl, psev, Top, Bottom : integer) : integer;
var Rect : trect;
...
//==============================================================================
    ...
    if frMyPrint.cbDataPrint.ItemIndex=0
      then txt := PrintListEvents.Rows[psev].Comment
      else txt := PrintListEvents.Rows[psev].Lyrics;
...
//==============================================================================
    if hght1>hght then hght:=hght1;
    if hght<MaketOptions.RowHeight then hght := MaketOptions.RowHeight;
...
begin
  bmp := tbitmap.Create;
  try
    frMyPrint.Panel3.Height := trunc((PrintPageParameters.Height / PrintPageParameters.Width) * frMyPrint.Panel3.Width);
    frMyPrint.Panel3.Left := (frMyPrint.Panel2.Width - frMyPrint.Panel3.Width) div 2;
    frMyPrint.Panel3.Top := (frMyPrint.Panel2.Height - frMyPrint.Panel3.Height) div 2;
    ...
    for i:=0 to PrintListEvents.Count-1 do begin
            case tp of
      1: CurH := DrawMaketTableRow(bmp.Canvas, 0, i, CurH, MaketOptions.Bottom);
      2..34 : begin
                dvs := tp - 1;
                if PrintListEvents.Rows[i].Device=dvs
                  then CurH := DrawMaketTableRow(bmp.Canvas, 0, i, CurH, MaketOptions.Bottom);
              end;
            end;
      if CurH > MaketOptions.LowBorder - 1 then break;
    end;

    DrawMaketLowTitle(bmp.Canvas);

    stretchblt(frMyPrint.Image3.Canvas.Handle, frMyPrint.Image3.Left, frMyPrint.Image3.Top, frMyPrint.Image3.Width, frMyPrint.Image3.Height,
...
begin
  ...
  try
    ...
    for i:=posi to PrintListEvents.Count-1 do begin
            case tp of
      1: CurH := DrawMaketTableRow(bmp.Canvas, 0, i, CurH, MaketOptions.Bottom);
      2..34 : begin
                dvs := tp - 1;
      ...
      if CurH > MaketOptions.LowBorder - 1 then begin
        DrawMaketLowTitle(bmp.Canvas);
        if i < PrintListEvents.Count-1 then begin
...
begin
    ...
    for i:=posi to PrintListEvents.Count-1 do begin
           case tp of
      1: CurH := DrawMaketTableRow(Printer.Canvas, 0, i, CurH, MaketOptions.Bottom);
      2..34 : begin
                dvs := tp - 1;
                if PrintListEvents.Rows[i].Device=dvs
                  then CurH := DrawMaketTableRow(Printer.Canvas, 0, i, CurH, MaketOptions.Bottom);
              end;
            end;
...
Procedure PrintTimelines;
var i, rw, tp, cnt, tld, tlt : integer;
begin
  ...
  for i:=1 to Form1.GridTimeLines.RowCount-1 do begin
    ...
    tldevice : frMyPrint.cbDevices.Items.Add((Form1.GridTimeLines.Objects[0,i] as TTimelineOptions).Name);
    tltext   : frMyPrint.cbTexts.Items.Add((Form1.GridTimeLines.Objects[0,i] as TTimelineOptions).Name);
       end;
  ...
  tp := frMyPrint.cbTypePrint.ItemIndex;
  if frMyPrint.cbDataPrint.ItemIndex = 0
    then frmyprint.cbTexts.Enabled := false else frmyprint.cbTexts.Enabled := true;
...
begin
   ...
   if PageSetup(PrintPageParameters) then begin
   ...
   for j := 1 to SpinEdit1.Value do begin
     ...
     if tp = 0 then begin
       ...
       for i:=0 to cnt - 1 do begin
         ...
         if i<>cnt -1 then printer.NewPage;
       end;
     end else PrintMaketPages(0, tp);
     printer.EndDoc;
   end;
end;

procedure TfrMyPrint.cbDevicesChange(Sender: TObject);
var i, tp, tld, tlt, cnt, inx : integer;
begin
  Image3.Canvas.FillRect(Image3.Canvas.ClipRect);
  tld := findpostimeline(frMyPrint.cbDevices.ItemIndex, tldevice);
  cnt := (Form1.GridTimeLines.Objects[0,tld + 1] as TTimelineOptions).CountDev;
  ...
  for i:=1 to cnt do cbTypePrint.Items.Add('Устройство ' + inttostr(i));
  if inx < cbTypePrint.Items.Count then cbTypePrint.ItemIndex := inx else cbTypePrint.ItemIndex := 0;
