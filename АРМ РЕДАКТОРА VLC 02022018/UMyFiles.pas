...
var TempStream : TFileStream;
    ...
    renm, Filename : string;
begin
  if not DirectoryExists(DirName) then ForceDirectories(PathProject);
...
begin
  ...
  if not FileExists(FileName) then begin
    MessageDlg('Файл '+FileName+' не найден.', mtInformation, [mbOk], 0, mbOk);
    exit;
  end;
  ...
  frSaveProject.Label2.Caption:='Загрузка проекта' + #13#10 + extractfilename(FileNameProject);
  frSaveProject.ProgressBar1.Position:=0;
  Delay(50);
  ...
  stemp:='';
  ReadBufferStr(Stream,stemp);
  Form1.lbDateStart.Caption:=stemp;
  stemp:='';
  ReadBufferStr(Stream,stemp);
  Form1.lbDateEnd.Caption:=stemp;
  ...
  WriteLog('MAIN', 'UMyFiles.ReadProjectFromFile Finish FileName=' + FileName);
    ...
    FreeAndNil(Stream);
    frSaveProject.Close;
    MyTextMessage('Сообщение','Ошибка при восстановлении проекта ' + FileName,1);
  end;
end;
