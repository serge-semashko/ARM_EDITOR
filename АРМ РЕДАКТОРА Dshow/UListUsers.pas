...
begin
  result := '';
  for i:=0 to length(Src) do result := result + chr(ord(Src[i])-Step);
end;
...
var i : integer;
begin
  for i:=Count-1 to 0 do Users[i].FreeInstance;
  Count:=0;
...
begin
  ...
  for i := 0 to Count-1 do begin
    if (trim(Users[i].Login)=trim(Login)) and (trim(Users[i].Password)=trim(Password)) then begin
      result:=true;
      exit;
    end;
  end;
...
begin
  ...
  try
    ...
    for i:=0 to Count-1 do Users[i].WriteToStream(Stream);
  finally
    FreeAndNil(Stream);
  end;
  except
...
begin
  for i := 0 to Count-1 do begin
    if trim(Users[i].Login)=trim(Login) then begin
      apos:=i;
      break;
    end;
  end;
  if apos<count-1 then for i:=apos to Count-1 do Users[apos]:=Users[apos+1];
  Count:=Count-1;
  Setlength(Users,Count);
end;
