    ...
    if ModalResult=mrOk then begin
      if CheckBox1.Checked then begin
        for i:=0 to TLZone.TLEditor.Count-1 do begin
          TLZone.TLEditor.Events[i].Start:=TLZone.TLEditor.Events[i].Start + SpinEdit1.Value;
          TLZone.TLEditor.Events[i].Finish:=TLZone.TLEditor.Events[i].Finish + SpinEdit1.Value;
        end;
        for i:=0 to TLZone.Count-1 do begin
          for j:=0 to TLZone.Timelines[i].Count-1 do begin
            TLZone.Timelines[i].Events[j].Start:=TLZone.Timelines[i].Events[j].Start + SpinEdit1.Value;
            TLZone.Timelines[i].Events[j].Finish:=TLZone.Timelines[i].Events[j].Finish + SpinEdit1.Value;
          end;
        end;
      end;
      ...
      for i:=0 to TLZone.Timelines[ps].Count-1 do begin
        TLZone.Timelines[ps].Events[i].Start:=TLZone.Timelines[ps].Events[i].Start + SpinEdit1.Value;
        TLZone.Timelines[ps].Events[i].Finish:=TLZone.Timelines[ps].Events[i].Finish + SpinEdit1.Value;
      end;
    end;
  end;
end;
