...
begin
  ...
  if Form1.PanelPrepare.Visible then begin
    crpos := TLZone.TLEditor.CurrentEvents;
    if crpos.Number <> -1 then MarkRowPhraseInGrid(Form1.GridGRTemplate, 0, 2, 'File', crpos.Image, 'TEvSwapBuffer.Cut');
    TemplateToScreen(crpos);
    if Form1.pnImageScreen.Visible then Form1.Image3.Repaint;
  end;
  if makelogging then WriteLog('MAIN', 'UEvSwapBuffer.TEvSwapBuffer.Cut');
  except
    on E: Exception do WriteLog('MAIN', 'UEvSwapBuffer.TEvSwapBuffer.Cut | ' + E.Message);
  end;
end;
...
begin
  ...
  if Form1.PanelPrepare.Visible then begin
    crpos := TLZone.TLEditor.CurrentEvents;
    if crpos.Number <> -1 then MarkRowPhraseInGrid(Form1.GridGRTemplate, 0, 2, 'File', crpos.Image, 'TEvSwapBuffer.Copy');
    TemplateToScreen(crpos);
    if Form1.pnImageScreen.Visible then Form1.Image3.Repaint;
  end;
  if makelogging then WriteLog('MAIN', 'UEvSwapBuffer.TEvSwapBuffer.Copy');
  except
    on E: Exception do WriteLog('MAIN', 'UEvSwapBuffer.TEvSwapBuffer.Copy | ' + E.Message);
  end;
end;
...
begin
  ...
  if Form1.PanelPrepare.Visible then begin
    crpos := TLZone.TLEditor.CurrentEvents;
    if crpos.Number <> -1 then MarkRowPhraseInGrid(Form1.GridGRTemplate, 0, 2, 'File', crpos.Image, 'TEvSwapBuffer.Paste');
    TemplateToScreen(crpos);
    if Form1.pnImageScreen.Visible then Form1.Image3.Repaint;
  end;
  if makelogging then WriteLog('MAIN', 'UEvSwapBuffer.TEvSwapBuffer.Paste');
  except
    on E: Exception do WriteLog('MAIN', 'UEvSwapBuffer.TEvSwapBuffer.Paste | ' + E.Message);
  end;
end;
