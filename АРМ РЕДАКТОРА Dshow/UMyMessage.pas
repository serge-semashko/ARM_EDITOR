...
begin
  try
  WriteLog('MAIN', 'TForm1.MyTextMessage UMyMessage=' + Msg + ' Buttons=' + inttostr(btns));
  result:=false;
  ...
  if FMyMessage.ModalResult = mrOk then begin
    ...
    on E: Exception do WriteLog('MAIN', 'UMyMessage.MyTextMessage Message=' + Msg + ' Buttons=' + inttostr(btns) + ' | ' + E.Message);
  end;
end;
