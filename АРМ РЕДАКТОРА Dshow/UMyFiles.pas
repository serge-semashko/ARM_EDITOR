  ...
  if FileExists(AppPath+DirProjects + '\List.tsk') then begin
    ...
    try
      ...
      if lst.Count<=0 then begin
        ...
        WriteLog('MAIN', 'UMyFiles.ExecTaskOnDelete - Не все задачи выполнены');
      end;
    finally
      lst.Free;
    end;
...
begin
  ...
  if RemoveRoot then begin// если необходимо удалить корень удаляем
...
begin
  try
  WriteLog('MAIN', 'UMyFiles.SaveGridTimelinesToFile Start FileName=' + FileName + ' Grid=' + Grid.Name);
  if FileExists(FileName) then begin
  ...
  try
    ...
    FreeAndNil(Stream);
    WriteLog('MAIN', 'UMyFiles.SaveGridTimelinesToFile Finish FileName=' + FileName + ' Grid=' + Grid.Name);
  end;
  except
    on E: Exception do begin
      WriteLog('MAIN', 'UMyFiles.SaveGridTimelinesToFile FileName=' + FileName + ' Grid=' + Grid.Name + ' | ' + E.Message);
      FreeAndNil(Stream);
    end else FreeAndNil(Stream);
  end;
end;
...
begin
  try
  WriteLog('MAIN', 'UMyFiles.LoadGridTimelinesFromFile Start FileName=' + FileName + ' Grid=' + Grid.Name);
  if not FileExists(FileName) then exit;
  Stream := TFileStream.Create(FileName, fmOpenReadWrite or fmShareDenyNone);
  try
  Stream.ReadBuffer(cnt, SizeOf(integer));
  ...
  for i:=1 to grid.RowCount-1 do begin
    ...
    FreeAndNil(Stream);
    WriteLog('MAIN', 'UMyFiles.LoadGridTimelinesFromFile Finish FileName=' + FileName + ' Grid=' + Grid.Name);
  end;
  except
    on E: Exception do begin
      WriteLog('MAIN', 'UMyFiles.LoadGridTimelinesFromFile FileName=' + FileName + ' Grid=' + Grid.Name + ' | ' + E.Message);
      FreeAndNil(Stream);
    end else FreeAndNil(Stream);
  end;
end;
...
begin
  ...
  if FileExists(PathTemp + '\PlayLists.lst') then begin
     ...
     if FileExists(PathTemp + '\PlayLists.lst')
       then CopyFile(PWideChar(PathTemp + '\PlayLists.lst'), PWideChar(PathLists + '\PlayLists.lst'), true);
  end;
  ...
  if FileExists(PathTemp + '\ImageTemplates.lst') then begin
     ...
     if FileExists(PathTemp + '\ImageTemplates.lst')
       then CopyFile(PwideChar(PathTemp + '\ImageTemplates.lst'), PwideChar(PathLists + '\ImageTemplates.lst'), true);
  end;
  ...
  if FileExists(PathTemp + '\TextTemplates.lst') then begin
     ...
     if FileExists(PathTemp + '\TextTemplates.lst')
       then CopyFile(PWideChar(PathTemp + '\TextTemplates.lst'), PwideChar(PathLists + '\TextTemplates.lst'), true);
  end;
  ...
  if FileExists(PathTemp + '\Clips.lst') then begin
     ...
     if FileExists(PathTemp + '\Clips.lst')
       then CopyFile(PwideChar(PathTemp + '\Clips.lst'), PwideChar(PathLists + '\Clips.lst'), true);
  end;
  WriteLog('MAIN', 'UMyFiles.SaveProjectToDisk Finish');
  except
    on E: Exception do WriteLog('MAIN', 'UMyFiles.SaveProjectToDisk | ' + E.Message);
  end;
end;
...
begin
  ...
  WriteLog('MAIN', 'UMyFiles.LoadProjectFromDisk Start');
    ...
    if FileExists(pathlists + '\PlayLists.lst')
       then CopyFile(PwideChar(pathlists + '\PlayLists.lst'), PwideChar(pathtemp + '\PlayLists.lst'), true);

    if FileExists(pathlists + '\ImageTemplates.lst')
       then CopyFile(PwideChar(pathlists + '\ImageTemplates.lst'), PwideChar(pathtemp + '\ImageTemplates.lst'), true);

    if FileExists(pathlists + '\TextTemplates.lst')
       then CopyFile(PwideChar(pathlists + '\TextTemplates.lst'), PwideChar(pathtemp + '\TextTemplates.lst'), true);

    if FileExists(pathlists + '\Clips.lst')
       then CopyFile(PwideChar(pathlists + '\Clips.lst'), PwideChar(pathtemp + '\Clips.lst'), true);
  WriteLog('MAIN', 'UMyFiles.LoadProjectFromDisk Finish');
  except
    on E: Exception do WriteLog('MAIN', 'UMyFiles.LoadProjectFromDisk | ' + E.Message);
  end;
end;
