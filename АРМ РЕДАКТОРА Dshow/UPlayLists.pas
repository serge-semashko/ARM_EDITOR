...
begin
  try
  WriteLog('MAIN', 'Uplaylists.findclipposition ClipID=' + ClipID);
  result:=-1;
  for i:=1 to Form1.GridClips.RowCount-1 do begin
    if trim((Form1.GridClips.Objects[0,i] as TGridRows).MyCells[3].ReadPhrase('ClipID'))=trim(ClipID) then begin
      result:=i;
      WriteLog('MAIN', 'Uplaylists.findclipposition ClipID=' + ClipID + ' Position=' + inttostr(i));
      exit;
    end;
  end;
  except
    on E: Exception do WriteLog('MAIN', 'Uplaylists.findclipposition ClipID=' + ClipID + ' | ' + E.Message);
  end;
end;
  ...
  with FPlayLists do begin
    ...
    for i:=0 to cnt-1 do begin
      ...
      if i>0 then begin
        ...
        WriteLog('MAIN', 'Uplaylists.findclipposition Clip=' + clp + ' ClipID=' + clpid);
      end;
    end;
    finally
      FreeAndNil(Stream);
