...
type
  ...
  TProgOptions = class
    Count : integer;
    Options : array of TOneOption;       // [1] nomer manager
...
begin
  ...
  for i:=1 to Count-1 do begin
    if (X>Options[i].RTT.Left) and (X<Options[i].RTT.Right) and (Y>Options[i].RTT.Top)
       and (Y<Options[i].RTT.Bottom)
    then begin
...
begin
  ...
  Add('Скорость порта (бит/сек):',Port422Speed,'1200|2400|4800|9600|14400|19200|38400|57600|115200');  //: string = '38400';
  Add('Количество бит:',Port422Bits,'8|7|6|5');   //: string = '8';
  Add('Четность:',Port422Parity,'нет|чет|нечет|маркер|пробел'); //: string = 'нечет';
  Add('Количество стоп бит:',Port422Stop,'1|1.5|2');   //: string = '1';
  Add('Управление потоком:',Port422Flow,'нет|аппаратный|XOn/XOff');   //: string = 'нет';
  ...
  Add('URL Управляющего WEB сервера:',URLServer,'');     //: string = 'http://localhost:9090/GET_TLEDITOR';
  if AutoStart then ss:='Да' else ss:='Нет';
  Add('Авто запуск программы:',ss,'Да|Нет');     //: boolean = false;
  if MakeLogging then ss:='Да' else ss:='Нет';
...
begin
  ...
  if trim(ss)='' then ManagerNumber:=0 else ManagerNumber:=strtoint(ss);
  ss := Get('Порт передачи данных:');
  if trim(ss)='RS232/422' then Port422select:=true else Port422select:=false;

  Port422Name := trim(Get('Последовательный порт:'));

  if trim(Port422Name)='' then Port422Number:=-1 else Port422Number:=ListComports.IndexOf(Port422Name);
  ...
  if ansilowercase(trim(ss))='да' then AutoStart:=true else AutoStart:=false;

  ss := Get('Выполнять логирование:');
  if ansilowercase(trim(ss))='да' then MakeLogging:=true else MakeLogging:=false;

  ss := Get('Высота шрифта:');
  if trim(ss)='' then MTFontSize:=MTFontSize else MTFontSize:=strtoint(ss);
end;
...
begin
   if indxoption=-1 then exit;
   if ProgOptions.Options[indxoption].EditType>=0 then if key=46 then key:=0;
end;
...
procedure TfrOptions.ImgOptionsMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
...
begin
  ...
  ProgOptions.draw(ImgOptions.Canvas,ComboBox1.Height);
  ImgOptions.Repaint;
  if AutoStart then AutoStartEnable else AutoStartDisable;
