...
uses
  ...
  Dialogs, ComCtrls, Buttons, StdCtrls, ExtCtrls, Grids, ImgList, UGrid, ucommon;
...
function GridDeleteRow(typegrid : ttypegrid) : integer;
...
var i : integer;
begin
      for i:=0 to 15 do FDelGridRow.ListDelTasks[i]:=delnone;
...
projects    : begin
                ...
                RadioGroup1.Items.Add('Удалить проекты время создания которых меньше указанного');
                FDelGridRow.ListDelTasks[2]:=delstart;
                RadioGroup1.Items.Add('Удалить проекты время окончания которых меньше указанного');
...
clips       : begin
                ...
                FDelGridRow.ListDelTasks[1]:=delins;
                RadioGroup1.Items.Add('Удалить клипы время создания которых меньше указанного');
                FDelGridRow.ListDelTasks[2]:=delstart;
                RadioGroup1.Items.Add('Удалить клипы время окончания которых меньше указанного');
                FDelGridRow.ListDelTasks[3]:=delend;
...
txttemplate : begin
                RadioGroup1.Items.Add('Удалить выбранный текстовый шаблон');
                FDelGridRow.ListDelTasks[0]:=delone;
                RadioGroup1.Items.Add('Удалить все выделенные текстовые шаблоны');
                FDelGridRow.ListDelTasks[1]:=delins;
                RadioGroup1.Items.Add('Очистить список текстовых шаблонов');
                FDelGridRow.ListDelTasks[2]:=delall;
              end;
      end;
end;
...
begin
  ...
  If FDelGridRow.ModalResult=mrOk then begin
    currtypegrid:=empty;
  end;
end;
...
begin
  ...
  tschoise : begin
               ...
               delall   : begin
                        ...
                        else FDelGridRow.ModalResult:=mrOk;
                  end;
             end;
  tslabel  : begin
                     case SelectTypeDeletion(CurrTypeGrid) of
  ...
  tsend     : FDelGridRow.ModalResult:=mrOk;
       end;
end;
