...
uses umain, uproject, uinitforms, umyfiles, utimeline, udrawtimelines, ugrtimelines,
     ...
     ushifttl, UShortNum, UEvSwapBuffer, UMyMessage, uairdraw, UMyMediaSwitcher, math,
    ...
    if Grid.Objects[0,i] is TGridRows then begin
       ...
       if Trim(txt)<>'' then begin
         if not FileExists(txt) then begin
           (Grid.Objects[0,i] as TGridRows).MyCells[3].SetPhraseColor('Clip', PhraseErrorColor);
           WriteLog('MAIN', 'UCommon.CheckedClipsInList-1 Grid=' + Grid.Name + ' File=' + txt + 'Не существует');
         end else WriteLog('MAIN', 'UCommon.CheckedClipsInList Grid=' + Grid.Name + ' File=' + txt + 'Найден');;
       end else begin
         (Grid.Objects[0,i] as TGridRows).MyCells[3].SetPhraseColor('Clip', PhraseErrorColor);
         WriteLog('MAIN', 'UCommon.CheckedClipsInList-2 Grid=' + Grid.Name + ' File=' + txt + 'Не существует');
       end;
    end;
  end;
  WriteLog('MAIN', 'UCommon.CheckedClipsInList Finish Grid=' + Grid.Name);
  except
    on E: Exception do WriteLog('MAIN', 'UCommon.CheckedClipsInList Grid=' + Grid.Name + ' | ' + E.Message);
  end;
end;
    ...
    if Form1.OpenDialog1.Execute then begin
      if trim(Form1.opendialog1.FileName)<>'' then begin
        ...
        if Err<>0 then begin
          ...
          WriteLog('MAIN', 'UCommon.ReloadClipInList Grid=' + Grid.Name + ' ARow' + inttostr(ARow) + ' File=' + trim(Form1.opendialog1.FileName) + ' Медиа-данные отсутствуют');
          exit;
        end;
        ...
        WriteLog('MAIN', 'UCommon.ReloadClipInList Grid=' + Grid.Name + ' ARow' + inttostr(ARow) + ' File=' + trim(Form1.opendialog1.FileName) + ' Загружен');
      end;
    end;
  end;
  except
...
begin
  ...
  if Form1.ListBox1.ItemIndex<>-1 then begin
    txt := (Form1.ListBox1.Items.Objects[Form1.ListBox1.ItemIndex] as TMyListBoxObject).ClipId;
    SaveGridToFile(PathPlayLists + '\' + txt, Form1.GridActPlayList);
  end;
  SaveProjectToDisk;
  if Form1.GridClips.Objects[0,1] is TGridRows then begin
    txt := (Form1.GridClips.Objects[0,1] as TGridRows).MyCells[3].ReadPhrase('ClipId');
    if (Form1.GridClips.RowCount=2) and (Trim(txt)='') then exit;
    SaveGridToFile(PathLists + '\Clips.lst', Form1.GridClips);
  end;
  WriteLog('MAIN', 'UCommon.saveoldproject Finish');
  except
    on E: Exception do WriteLog('MAIN', 'UCommon.saveoldproject | ' + E.Message);
  end;
end;
...
begin
  ...
  if MakeLogging then begin
    ...
    On E : Exception do  WriteLog('MAIN', 'UCommon.CreateDirectories NewProject=' + NewProject + ' | ' + E.Message);
  end;
end;
...
begin
    ...
    for i:=0 to TLZone.Count-1 do begin
      if TLZone.Timelines[i].Count > 0 then begin
        if MyTextMessage('Вопрос','Сохранить редактируемые данные в списке клипов?',2) then begin
          ...
          EditClip(-100);
          WriteLog('MAIN', 'UCommon.SetMainGridPanel - Сохранение пустого клипа');
  ...
  actplaylist : begin
                  ...
                  if listbox1.ItemIndex=-1
                    then lbusesclpidlst.Caption := 'Плей-лист: '
                    else lbusesclpidlst.Caption := 'Плей-лист: ' + trim(ListBox1.Items.Strings[ListBox1.ItemIndex]);
                  if trim(Form1.lbActiveClipID.Caption)<>''   then begin
                    GridPlayer:=grPlayList;
