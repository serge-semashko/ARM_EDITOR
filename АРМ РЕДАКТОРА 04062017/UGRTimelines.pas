...
uses
  ...
  umyevents, OpenGL,WINAPI.MMSYSTEM;
...
var rect1, rect2, rect3 : trect;
    ...
    sz, st, en : longint;
    DCs : HDC;
    freq, sttime, fintime :int64;
begin
  QueryPerformanceFrequency(freq);
  QueryPerformanceCounter(sttime);
 // SetStretchBltMode(Cv.Handle, COLORONCOLOR);
  QueryPerformanceCounter(fintime);
  if makelogging then WriteLog('TCblt', 'DrawTimelines set stretchblt mode=' + floatToStr((FINTIME-STTIME)*1000000/freq));
  try
  ...
  QueryPerformanceCounter(fintime);
  if makelogging then WriteLog('TCblt', 'DrawTimelines stretchblt duration=' + floatToStr((FINTIME-STTIME)*1000/freq));
