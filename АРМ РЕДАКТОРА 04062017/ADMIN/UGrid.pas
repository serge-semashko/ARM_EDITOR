...
Var
  ...
  RowGridUsers,  RowGridSample, TempGridRow : TGridRows;
...
function MarkRowPhraseInGrid(Grid : tstringgrid; MarkCell, WorkCell : integer; Name, Text : string) : integer;
    ...
    if grid.Objects[0,rw] is tgridrows then begin
       for i:=0 to grid.ColCount-1 do begin
         if (grid.Objects[0,rw] as tgridrows).MyCells[i].CellType=tsImage then begin
           ...
           if FileExists(pathtemplates + '\' +nm)
             then (grid.Objects[0,rw] as tgridrows).MyCells[i].LoadJpeg(pathtemplates + '\' +nm,grid.ColWidths[i],(grid.Objects[0,rw] as tgridrows).HeightRow);//grid.RowHeights[rw]);
         end;
  ...
  if Trim(Text)='' then begin
    for i:=0 to Grid.RowCount-1
      do if Grid.Objects[0,i] is TGridRows
           then (Grid.Objects[0,i] as TGridRows).MyCells[MarkCell].Mark:=false;
    grid.Repaint;       
    exit;
  end;
  for i:=0 to Grid.RowCount-1 do begin
    if Grid.Objects[0,i] is TGridRows then begin
      ...
      if trim(lowercase(txt))=trim(lowercase(Text)) then begin
        ...
        Grid.Row:=i;
      end else (Grid.Objects[0,i] as TGridRows).MyCells[MarkCell].Mark:=false;
    end;
  end;
  grid.Repaint;
end;
...
RowGridClips.MyCells[ACell].Rows[ARow].Phrases[APhr].leftprocent:=60;
RowGridClips.MyCells[ACell].Rows[ARow].Phrases[APhr].UsedSubFont:=true;
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//Создаем шаблон списка Пользователей
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

RowGridUsers := TGridRows.Create;

RowGridUsers .HeightTitle:=PLHeightTitle;
RowGridUsers .HeightRow:=PLHeightRow;

ACell:=RowGridUsers .AddCells;
//ACell:=RowGridUsers .AddCells;
RowGridUsers .MyCells[ACell].CellType:=tsText;
RowGridUsers .MyCells[ACell].Title:='Список плей-листов';
//RowGridUsers .MyCells[ACell].Mark:=true;
RowGridUsers .MyCells[ACell].Width:=-1;
RowGridUsers .MyCells[ACell].Procents:=100;
RowGridUsers .MyCells[ACell].TopPhrase:=2;
RowGridUsers .MyCells[ACell].Interval:=4;

ARow:=RowGridUsers .MyCells[ACell].AddRow;
RowGridUsers .MyCells[ACell].Rows[ARow].Top:=PLRowsTop;
RowGridUsers .MyCells[ACell].Rows[ARow].Height:=PLRowsHeight;

APhr:=RowGridUsers .MyCells[ACell].AddPhrase(ARow,'Family','');
RowGridUsers .MyCells[ACell].Rows[ARow].Phrases[APhr].leftprocent:=2;
RowGridUsers.MyCells[ACell].Rows[ARow].Phrases[APhr].Bold:=true;
RowGridUsers.MyCells[ACell].Rows[ARow].Phrases[APhr].UsedSubFont:=False;

APhr:=RowGridUsers .MyCells[ACell].AddPhrase(ARow,'Name','');
RowGridUsers .MyCells[ACell].Rows[ARow].Phrases[APhr].leftprocent:=35;
RowGridUsers.MyCells[ACell].Rows[ARow].Phrases[APhr].Bold:=true;
RowGridUsers.MyCells[ACell].Rows[ARow].Phrases[APhr].UsedSubFont:=False;

APhr:=RowGridUsers .MyCells[ACell].AddPhrase(ARow,'SubName','');
RowGridUsers .MyCells[ACell].Rows[ARow].Phrases[APhr].leftprocent:=70;
RowGridUsers.MyCells[ACell].Rows[ARow].Phrases[APhr].Bold:=true;
RowGridUsers.MyCells[ACell].Rows[ARow].Phrases[APhr].UsedSubFont:=False;

ARow:=RowGridUsers .MyCells[ACell].AddRow;
RowGridUsers .MyCells[ACell].Rows[ARow].Top:=PLRowsTop + PLRowsHeight + PLRowsInterval;
RowGridUsers .MyCells[ACell].Rows[ARow].Height:=PLRowsHeight;

APhr:=RowGridUsers .MyCells[ACell].AddPhrase(ARow,'ShortNameTxt','');
RowGridUsers.MyCells[ACell].Rows[ARow].Phrases[APhr].leftprocent:=2;
RowGridUsers.MyCells[ACell].Rows[ARow].Phrases[APhr].UsedSubFont:=true;

APhr:=RowGridUsers .MyCells[ACell].AddPhrase(ARow,'ShortName','');
RowGridUsers.MyCells[ACell].Rows[ARow].Phrases[APhr].leftprocent:=16;
RowGridUsers.MyCells[ACell].Rows[ARow].Phrases[APhr].UsedSubFont:=true;

APhr:=RowGridUsers .MyCells[ACell].AddPhrase(ARow,'LoginTxt','');
RowGridUsers.MyCells[ACell].Rows[ARow].Phrases[APhr].leftprocent:=35;
RowGridUsers.MyCells[ACell].Rows[ARow].Phrases[APhr].UsedSubFont:=true;

APhr:=RowGridUsers .MyCells[ACell].AddPhrase(ARow,'Login','');
RowGridUsers.MyCells[ACell].Rows[ARow].Phrases[APhr].leftprocent:=43;
RowGridUsers.MyCells[ACell].Rows[ARow].Phrases[APhr].UsedSubFont:=true;

RowGridUsers .MyCells[ACell].Rows[ARow].Top:=PLRowsTop + PLRowsHeight + PLRowsInterval;
RowGridUsers .MyCells[ACell].Rows[ARow].Height:=PLRowsHeight;
APhr:=RowGridUsers .MyCells[ACell].AddPhrase(ARow,'password','');
RowGridUsers.MyCells[ACell].Rows[ARow].Phrases[APhr].leftprocent:=80;
RowGridUsers.MyCells[ACell].Rows[ARow].Phrases[APhr].Visible:=false;

//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//Создаем шаблон списка Пользователей
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

RowGridSample := TGridRows.Create;

RowGridSample .HeightTitle:=25;
RowGridSample .HeightRow:=PLHeightRow;

ACell:=RowGridSample .AddCells;
//ACell:=RowGridSample .AddCells;
RowGridSample .MyCells[ACell].CellType:=tsText;
RowGridSample .MyCells[ACell].Title:='Таблица';
RowGridSample .MyCells[ACell].TitleColorFont:=ProgrammFontColor;
//RowGridSample .MyCells[ACell].Background:=ProgrammColor;
//RowGridSample .MyCells[ACell].Mark:=true;
RowGridSample .MyCells[ACell].Width:=-1;
RowGridSample .MyCells[ACell].Procents:=100;
RowGridSample .MyCells[ACell].TopPhrase:=2;
RowGridSample .MyCells[ACell].Interval:=4;

ARow:=RowGridSample .MyCells[ACell].AddRow;
RowGridSample .MyCells[ACell].Rows[ARow].Top:=PLRowsTop;
RowGridSample .MyCells[ACell].Rows[ARow].Height:=PLRowsHeight;

APhr:=RowGridSample .MyCells[ACell].AddPhrase(ARow,'Text','');
RowGridSample .MyCells[ACell].Rows[ARow].Phrases[APhr].leftprocent:=2;
RowGridSample.MyCells[ACell].Rows[ARow].Phrases[APhr].Bold:=true;
RowGridSample.MyCells[ACell].Rows[ARow].Phrases[APhr].UsedSubFont:=False;
...
RowGridUsers.FreeInstance;
RowGridSample.FreeInstance;
