...
begin
  if ARow=-1 then begin
    ...
    if not (Form1.GridUsers.Objects[0,ARow] is TGridRows) then begin
      Form1.GridUsers.Objects[0,ARow] := TGridRows.Create;
      (Form1.GridUsers.Objects[0,ARow] as TGridRows).Assign(RowGridUsers);
    end;
    frNewUser.edFamily.Text    := (Form1.GridUsers.Objects[0,ARow] as TGridRows).MyCells[0].ReadPhrase('Family');
    frNewUser.edName.Text      := (Form1.GridUsers.Objects[0,ARow] as TGridRows).MyCells[0].ReadPhrase('Name');
    frNewUser.edSubName.Text   := (Form1.GridUsers.Objects[0,ARow] as TGridRows).MyCells[0].ReadPhrase('SubName');
    frNewUser.edShortName.Text := (Form1.GridUsers.Objects[0,ARow] as TGridRows).MyCells[0].ReadPhrase('ShortName');
    frNewUser.edLogin.Text     := (Form1.GridUsers.Objects[0,ARow] as TGridRows).MyCells[0].ReadPhrase('Login');
    frNewUser.edPassword.Text  := (Form1.GridUsers.Objects[0,ARow] as TGridRows).MyCells[0].ReadPhrase('Password');
  end;
  frNewUser.ShowModal;
  if frNewUser.ModalResult = mrOk then begin
    rw := ARow;
    if rw=-1 then begin
      if Form1.GridUsers.RowCount=2 then begin
        if not (Form1.GridUsers.Objects[0,1] is TGridRows) then begin
          Form1.GridUsers.Objects[0,1] := TGridRows.Create;
          (Form1.GridUsers.Objects[0,1] as TGridRows).Assign(RowGridUsers);
        end;
        s := (Form1.GridUsers.Objects[0,1] as TGridRows).MyCells[0].ReadPhrase('LoginTxt');
        if Trim(s)='' then rw:=1
        else begin
          Form1.GridUsers.RowCount := Form1.GridUsers.RowCount + 1;
          Form1.GridUsers.Objects[0,Form1.GridUsers.RowCount-1] := TGridRows.Create;
          (Form1.GridUsers.Objects[0,Form1.GridUsers.RowCount-1] as TGridRows).Assign(RowGridUsers);
          rw:=Form1.GridUsers.RowCount-1;
        end;
      end else begin
        Form1.GridUsers.RowCount := Form1.GridUsers.RowCount + 1;
        Form1.GridUsers.Objects[0,Form1.GridUsers.RowCount-1] := TGridRows.Create;
    ...
    (Form1.GridUsers.Objects[0,rw] as TGridRows).ID:=1;
    (Form1.GridUsers.Objects[0,rw] as TGridRows).MyCells[0].UpdatePhrase('Family', frNewUser.edFamily.Text);
    (Form1.GridUsers.Objects[0,rw] as TGridRows).MyCells[0].UpdatePhrase('Name', frNewUser.edName.Text);
    (Form1.GridUsers.Objects[0,rw] as TGridRows).MyCells[0].UpdatePhrase('SubName', frNewUser.edSubName.Text);
    (Form1.GridUsers.Objects[0,rw] as TGridRows).MyCells[0].UpdatePhrase('ShortNameTxt', 'Коротокое имя:');
    (Form1.GridUsers.Objects[0,rw] as TGridRows).MyCells[0].UpdatePhrase('ShortName', frNewUser.edShortName.Text);
    (Form1.GridUsers.Objects[0,rw] as TGridRows).MyCells[0].UpdatePhrase('LoginTxt', 'Логин:');
    (Form1.GridUsers.Objects[0,rw] as TGridRows).MyCells[0].UpdatePhrase('Login', frNewUser.edLogin.Text);
    (Form1.GridUsers.Objects[0,rw] as TGridRows).MyCells[0].UpdatePhrase('Password', frNewUser.edPassword.Text);
    WriteUsersToFile('workingdata.dll',Form1.GridUsers);
  end;

end;

procedure TfrNewUser.FormCreate(Sender: TObject);
begin
  initfrNewUser;
end;

procedure TfrNewUser.FormKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
begin
  if trim(edFamily.Text)='' then begin
...
begin
  s := '';
  if src<>'' then for i:=0 to length(Src) do s := s + chr(ord(Src[i])-Step);
  result := s;
end;
...
begin
  ...
  if cnt < 2 then begin
    initgrid(Grid, RowGridUsers, Form1.Width - Form1.pnctrlusers.Width);
  end else begin
    Grid.RowCount:=cnt;
