...
uses
  ...
  Dialogs, ExtCtrls, ComCtrls, StdCtrls, MMSystem, Menus, Buttons;
...
begin
 ...
 if trim(lowercase(ParamStr(1)))='test' then begin
   Edit1.Visible := true;
...
function StrTimeCodeToFrames(tc : string) : double;
var HH,MM,SS,ms : word;
begin
  ...
  result := EncodeTime(hh, mm, ss, ms*40);
end;
...
begin
  dt := Now;
  myTime := StrTimeCodeToFrames(Edit1.Text);
  myshiftsample := dt - (Trunc(dt) + myTime);
  compartido^.State:=CheckBox4.Checked;
  Compartido^.Shift :=myshiftsample;
